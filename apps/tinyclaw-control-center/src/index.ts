...
// - command mode (APP_COMMAND / argv)
// - UI mode (UI_MODE=true)
...
const UI_HOST = (process.env.UI_HOST ?? "127.0.0.1").trim() || "127.0.0.1";
...
async function start(): Promise<void> {
  ...
  // Startup diagnostics intentionally expose only configuration posture
  // and never print local token values.
  ...
  if (COMMAND) {
    // Command mode runs one explicit operation and exits.
    await runCommand(COMMAND, adapter, COMMAND_ARG);
    return;
  }

  // Monitor mode runs at least once, then optionally in a fixed interval.
  ...
  if (UI_MODE) {
    // UI server can run together with monitor mode.
    startUiServer(UI_PORT, client, UI_HOST);
  }
}
...
async function runCommand(
  ...
  // All command-mode mutations are guarded by the local-token gate.
...
function assertCommandOperationGate(
  ...
  // If local token auth is enabled, protected commands require an explicit token.
