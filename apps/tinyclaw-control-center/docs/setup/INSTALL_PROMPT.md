# TinyClaw Control Center 安装 Prompt

默认语言：**中文** | [English](INSTALL_PROMPT.en.md)

把下面整段原样交给你自己的 OpenClaw，让它帮你把 `TinyClaw Control Center` 安装并接到这台机器自己的 OpenClaw 环境上。

```text
你现在要帮我把 TinyClaw Control Center 安装并接到这台机器自己的 OpenClaw 环境上。
