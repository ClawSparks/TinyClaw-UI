...
// Safety defaults: production-like posture remains read-only unless explicitly opened.
...
function parsePositiveInt(input: string | undefined, fallback: number): number {
  // Invalid or non-positive values never override the fallback.
...
function readStringEnv(input: string | undefined, fallback: string): string {
  // Empty strings are treated as unset to keep behavior predictable.
