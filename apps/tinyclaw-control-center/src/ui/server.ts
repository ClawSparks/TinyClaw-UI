...
const ADMIN_AUTH_COOKIE_NAME = "openclaw_admin_auth";
const ADMIN_AUTH_COOKIE_MAX_AGE_SECONDS = 60 * 60 * 24 * 30;
...
const GATEWAY_UI_SETTINGS_PATH = join(process.cwd(), "runtime", "gateway-ui-settings.json");
...
const DASHBOARD_SECTION_LINKS_EN: DashboardSectionLink[] = [
  ...
  { key: "chat", label: "Chat", blurb: "LLM conversations" },
  { key: "ai-model-config", label: "AI Models", blurb: "Model providers and API keys" },
  { key: "agent-management", label: "Agents", blurb: "Create and manage agents" },
  { key: "skills", label: "Skills", blurb: "Installed skills and setup" },
  { key: "chat-channel-config", label: "Channels", blurb: "Connected chat channels" },
...
interface GatewayRuntimeSnapshot {
  endpoint: string;
  protocol: string;
  reachability: "connected" | "not_connected" | "partial";
  latencyMs?: number;
  certStatus: "ok" | "invalid" | "not_applicable" | "unknown";
  certExpiryLabel?: string;
}

interface ChatChannelsSnapshot {
...
let renderSessionPreviewCache:
  | { snapshotAt: string; historyLimit: number; value: SessionConversationListResult; expiresAt: number }
  | undefined;
...
let recentToolCallsCache: { value: number; expiresAt: number; inflight?: Promise<void> } = {
  value: 0,
  expiresAt: 0,
};
let readModelSnapshotCache:
  | {
      value: ReadModelSnapshot;
      expiresAt: number;
    }
  | undefined;
let readModelSnapshotInFlight: Promise<ReadModelSnapshot> | undefined;
...
export function startUiServer(port: number, toolClient: ToolClient, host: string = "127.0.0.1"): Server {
  const approvalActions = new ApprovalActionService(toolClient);

  // Single HTTP server entrypoint:
  // - renders TinyClaw pages
  // - exposes control APIs
  // - reverse-proxies chat-gateway under /chat-gateway/*
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      const isLegacyCcChatHost = hostHeader.includes("cc-43-155-180-158.sslip.io") || hostHeader.startsWith("cc-");
      if (
        method === "GET" &&
        (path === "/chat-gateway" || path === "/chat-gateway/") &&
        fetchDest !== "iframe" &&
        !url.searchParams.has("ccv") &&
        !isLegacyCcChatHost
      ) {
        return redirect(res, 302, buildHomeHref({ quick: "all" }, true, "chat", "zh"));
      }
      const isChatGatewayPath = path === "/chat-gateway" || path.startsWith("/chat-gateway/");
      const isChatGatewayChildPath = (
        (path.startsWith("/api/") && !path.startsWith("/api/admin/") && !path.startsWith("/api/config")) ||
        ...
        path === "/logo192.png" ||
        path === "/manifest.json"
      ) && (referer.includes("/chat-gateway/") || path.startsWith("/openclaw/"));

      if (isChatGatewayPath || isChatGatewayChildPath) {
        const upstreamPath = isChatGatewayPath
          ? (path === "/chat-gateway" ? "/" : path.slice("/chat-gateway".length))
          : path;
        const chatGatewayAssetRequest = upstreamPath.startsWith("/assets/");
        const upstreamUrl = `http://127.0.0.1:3115${upstreamPath}${url.search}`;
        ...
        for (const [headerName, headerValue] of upstreamResponse.headers.entries()) {
          ...
          if (normalized === "content-security-policy") {
            const rewritten = headerValue.replace(/frame-ancestors[^;]*;?/i, "").trim();
            if (rewritten.length > 0) {
              res.setHeader(headerName, rewritten);
            }
            continue;
          }
          res.setHeader(headerName, headerValue);
        }
        if (chatGatewayAssetRequest) {
        ...
        let bodyBuffer = Buffer.from(await upstreamResponse.arrayBuffer());
        if (contentType.includes("text/html")) {
          // Inject gateway defaults into chat HTML so remote clients do not
          // accidentally connect to their own localhost.
          const gatewayConfig = await loadGatewayUiConfigPayload();
          let autoGatewayUrl = GATEWAY_URL || gatewayConfig.gatewayUrl || "ws://127.0.0.1:18789";
          try {
            const parsedGateway = new URL(autoGatewayUrl);
            const hostFromRequest = firstNonEmptyString(req.headers["x-forwarded-host"], req.headers.host);
            const requestHost =
              typeof hostFromRequest === "string" ? hostFromRequest.split(",")[0].trim().replace(/:\d+$/, "") : "";
            const localHosts = new Set(["127.0.0.1", "localhost", "::1"]);
            if (requestHost && localHosts.has(parsedGateway.hostname.toLowerCase())) {
              const wsProtocol = parsedGateway.protocol === "wss:" ? "wss:" : "ws:";
              autoGatewayUrl = `${wsProtocol}//${requestHost}:${parsedGateway.port || "18789"}`;
            }
          } catch {}
          const autoGatewayToken = gatewayConfig.token || "";
          const autoConnectScript = `<script>(function(){try{var gatewayUrl=${JSON.stringify(autoGatewayUrl)};var token=${JSON.stringify(autoGatewayToken)};var settingsKey='openclaw.control.settings.v1';var tokenPrefix='openclaw.control.token.v1:';function norm(url){try{var x=new URL(url,location.origin);var p=x.pathname==='/'?'':(x.pathname.replace(/\\/+$/,'')||x.pathname);return x.protocol+'//'+x.host+p;}catch{return url;}}var n=norm(gatewayUrl);var current={};try{var raw=localStorage.getItem(settingsKey);if(raw){current=JSON.parse(raw)||{};}}catch{}current.gatewayUrl=gatewayUrl;try{localStorage.setItem(settingsKey,JSON.stringify(current));if(token){localStorage.setItem(tokenPrefix+n,token);}}catch{}}catch{}})();</script>`;
          const rewritten = bodyBuffer
            .toString("utf8")
            .replace(/(["'])\/assets\//g, "$1/chat-gateway/assets/")
            .replace(/(["'])\/favicon\.ico(["'])/g, "$1/chat-gateway/favicon.ico$2")
            .replace(/(["'])\/logo192\.png(["'])/g, "$1/chat-gateway/logo192.png$2")
            .replace(/(["'])\/manifest\.json(["'])/g, "$1/chat-gateway/manifest.json$2")
            .replace(/<\/head>/i, `${autoConnectScript}</head>`);
      ...
      if (method === "HEAD" && (path === "/" || legacySection)) {
        res.statusCode = 200;
      ...
      if (method === "GET" && path === "/docs") {
        ...
        const html = `<!doctype html><html><head><meta charset="utf-8" /><title>${escapeHtml(t("TinyClaw Docs", "TinyClaw 文档"))}</title></head><body><h1>${escapeHtml(t("TinyClaw Docs", "TinyClaw 文档"))}</h1><ul>${links}</ul><p><a href="${escapeHtml(docsHref)}">${escapeHtml(t("Open document workbench", "打开文档工作台"))}</a> · <a href="${escapeHtml(homeHref)}">${escapeHtml(t("Back to control center", "返回控制中心"))}</a></p></body></html>`;
        return writeText(res, 200, html, "text/html; charset=utf-8");
      }
      ...
      if (method === "GET" && path.startsWith("/img/")) {
        ...
        const relativeAssetPath = path.slice(1);
        const assetPath = resolve(process.cwd(), relativeAssetPath);
        const insideImgDir = !relative(IMG_DIR, assetPath).startsWith("..");
        if (!insideImgDir) {
          return writeApiError(res, 404, "NOT_FOUND", "Image not found.");
        }
        let fileBody: Buffer;
        try {
          fileBody = await readFile(assetPath);
        } catch {
          return writeApiError(res, 404, "NOT_FOUND", "Image not found.");
        }
            ...
            : ext === ".jpg" || ext === ".jpeg"
              ...
              : ext === ".webp"
                ...
                : ext === ".svg"
                  ...
                  : undefined;
        if (!contentType) {
          return writeApiError(res, 415, "UNSUPPORTED_MEDIA_TYPE", "Unsupported image format.");
        }
        return writeBinary(res, 200, fileBody, contentType);
      }
      ...
      if (method === "GET" && path === "/api/model-config") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const summary = await loadModelProviderSummary();
        return writeJson(res, 200, {
          ok: true,
          sourcePath: summary.sourcePath,
          totalProviders: summary.providers.length,
          totalModels: summary.providers.reduce((sum, provider) => sum + provider.models.length, 0),
          providers: summary.providers,
          defaultPrimaryModel: summary.defaultPrimaryModel,
        });
      }

      if (method === "POST" && path === "/api/model-config/providers") {
        assertMutationAuthorized(req, "/api/model-config/providers");
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "model provider payload");
        const baseUrl = boundedTextField(payload.baseUrl, "baseUrl", 1024).trim();
        const apiKey = boundedTextField(payload.apiKey, "apiKey", 2048).trim();
        const modelName = boundedTextField(payload.modelName, "modelName", 256).trim();
        if (!baseUrl) {
          throw new RequestValidationError("baseUrl is required.", 400);
        }
        if (!apiKey) {
          throw new RequestValidationError("apiKey is required.", 400);
        }
        if (!modelName) {
          throw new RequestValidationError("modelName is required.", 400);
        }
        const providerAlias = optionalBoundedString(payload.providerAlias, "providerAlias", 128)?.trim();
        const updated = await appendModelProviderConfig({
          baseUrl,
          apiKey,
          modelName,
          providerAlias: providerAlias && providerAlias.length > 0 ? providerAlias : undefined,
        });
        return writeJson(res, 200, {
          ok: true,
          ...
          syncedDefaultAgentId: updated.syncedDefaultAgentId,
          defaultPrimaryModel: updated.defaultPrimaryModel,
      ...
      if (method === "GET" && path === "/api/endpoints") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const payload = await readCompatGatewayConfigPayload();
        return writeJson(res, 200, {
          success: true,
      ...
      if (method === "POST" && path === "/api/endpoints") {
        assertMutationAuthorized(req, "/api/endpoints");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const body = expectObject(await readJsonBody(req), "endpoints payload");
        const id = requiredBoundedString(body.id, "id", 128).trim();
        const baseUrl = requiredBoundedString(body.baseUrl, "baseUrl", 1024).trim();
        const api = optionalBoundedString(body.api, "api", 128)?.trim() || "openai";
        const apiKey = optionalBoundedString(body.apiKey, "apiKey", 2048)?.trim() ?? "";
        const result = await upsertCompatEndpoint({ id, baseUrl, api, apiKey });
        return writeJson(res, 200, {
          success: true,
      ...
      if (method === "DELETE" && path === "/api/endpoints/manage") {
        assertMutationAuthorized(req, "/api/endpoints/manage");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const body = expectObject(await readJsonBody(req), "endpoints delete payload");
        const endpoint = requiredBoundedString(body.endpoint, "endpoint", 128).trim();
        const result = await deleteCompatEndpoint(endpoint);
        return writeJson(res, 200, {
          success: true,
          endpoints: result.endpoints,
      ...
      if (method === "POST" && path === "/api/models/manage") {
        assertMutationAuthorized(req, "/api/models/manage");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const body = expectObject(await readJsonBody(req), "models create payload");
        const endpoint = requiredBoundedString(body.endpoint, "endpoint", 128).trim();
        const modelName = requiredBoundedString(body.modelName, "modelName", 256).trim();
        const alias = optionalBoundedString(body.alias, "alias", 256)?.trim();
        const result = await createCompatModel({ endpoint, modelName, alias: alias && alias.length > 0 ? alias : undefined });
        return writeJson(res, 200, {
          success: true,
          models: result.models,
        });
      ...
      if (method === "PUT" && path === "/api/models/manage") {
        assertMutationAuthorized(req, "/api/models/manage");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const body = expectObject(await readJsonBody(req), "models update payload");
        const id = requiredBoundedString(body.id, "id", 256).trim();
        const alias = optionalBoundedString(body.alias, "alias", 256)?.trim();
        const result = await updateCompatModel({ id, alias: alias && alias.length > 0 ? alias : undefined });
        return writeJson(res, 200, {
          success: true,
          models: result.models,
        });
      ...
      if (method === "DELETE" && path === "/api/models/manage") {
        assertMutationAuthorized(req, "/api/models/manage");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const body = expectObject(await readJsonBody(req), "models delete payload");
        const id = requiredBoundedString(body.id, "id", 256).trim();
        const result = await deleteCompatModel(id);
        return writeJson(res, 200, {
          success: true,
          models: result.models,
        });
      ...
      if (method === "PUT" && path === "/api/models/manage/default") {
        assertMutationAuthorized(req, "/api/models/manage/default");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const body = expectObject(await readJsonBody(req), "models default payload");
        const id = requiredBoundedString(body.id, "id", 256).trim();
        const result = await setCompatModelDefault(id);
        return writeJson(res, 200, {
          success: true,
          models: result.models,
      ...
      if (method === "GET" && path === "/api/skills") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const summary = await loadSkillsStatusSummary({ timeoutMs: 12_000 });
        return writeJson(res, 200, {
          ok: true,
          totalSkills: summary.totalSkills,
          installedSkills: summary.installedSkills,
          bundledSkills: summary.bundledSkills,
          skills: summary.skills,
        });
      }

      if (method === "POST" && path === "/api/skills/install") {
        assertMutationAuthorized(req, "/api/skills/install");
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "skill install payload");
        const source = boundedTextField(payload.source, "source", 1024).trim();
        if (!source) {
          throw new RequestValidationError("source is required.", 400);
        }
        const summary = await installSkillBySource(source);
        return writeJson(res, 200, {
          ok: true,
      ...
      if (method === "POST" && path === "/api/skills/update") {
        assertMutationAuthorized(req, "/api/skills/update");
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "skill update payload");
        const skillKey = boundedTextField(payload.skillKey, "skillKey", 256).trim();
        const disabled = payload.disabled === true;
        if (!skillKey) {
          throw new RequestValidationError("skillKey is required.", 400);
        }
        const summary = await updateSkillDisabledState(skillKey, disabled);
        return writeJson(res, 200, {
          ok: true,
      ...
      if (method === "POST" && path === "/api/skills/uninstall") {
        assertMutationAuthorized(req, "/api/skills/uninstall");
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "skill uninstall payload");
        const skillKey = boundedTextField(payload.skillKey, "skillKey", 256).trim();
        if (!skillKey) {
          throw new RequestValidationError("skillKey is required.", 400);
        }
        const summary = await uninstallSkill(skillKey);
        return writeJson(res, 200, {
          ok: true,
      ...
      if (method === "GET" && path === "/api/channel-config") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const summary = await loadChannelConfigSummary();
        return writeJson(res, 200, {
          ok: true,
          sourcePath: summary.sourcePath,
      ...
      if (method === "POST" && path === "/api/channel-config/channels") {
        assertMutationAuthorized(req, "/api/channel-config/channels");
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "channel payload");
        const channelKeyRaw = boundedTextField(payload.channelKey, "channelKey", 64).trim().toLowerCase();
        if (!channelKeyRaw) {
          throw new RequestValidationError("channelKey is required.", 400);
        }
        if (!/^[a-z0-9_-]{2,64}$/.test(channelKeyRaw)) {
          throw new RequestValidationError("channelKey must match /^[a-z0-9_-]{2,64}$/.", 400);
        }
        const configPayload = payload.config;
        if (!configPayload || typeof configPayload !== "object" || Array.isArray(configPayload)) {
          throw new RequestValidationError("config must be a JSON object.", 400);
        }
        const result = await appendChannelConfig({
          channelKey: channelKeyRaw,
          config: configPayload as Record<string, unknown>,
        });
        return writeJson(res, 200, {
          ok: true,
          sourcePath: OPENCLAW_CONFIG_PATH,
      ...
      if (method === "GET" && path === "/api/admin/session") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const decision = evaluateRequestTokenGate(req, "/api/admin/session");
        return writeJson(res, 200, {
          ok: true,
          required: LOCAL_TOKEN_AUTH_REQUIRED,
          authenticated: decision.ok,
        });
      }
      ...
      if (method === "POST" && path === "/api/admin/login") {
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "admin login payload");
        const password = requiredBoundedString(payload.password, "password", 512);
        const token = normalizeToken(password) ?? "";
        const decision = evaluateLocalTokenGate({
          gateRequired: LOCAL_TOKEN_AUTH_REQUIRED,
          configuredToken: LOCAL_API_TOKEN,
          providedToken: token,
          routeLabel: "/api/admin/login",
        });
        if (!decision.ok) {
      ...
      if (method === "GET" && path === "/api/openclaw/gateway/status") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const status = await readOpenClawGatewayRuntimeStatus();
        return writeJson(res, 200, {
          ok: true,
          status,
        });
      ...
      if (method === "POST" && path === "/api/openclaw/gateway/toggle") {
        assertMutationAuthorized(req, "/api/openclaw/gateway/toggle");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "openclaw gateway toggle payload");
        const actionRaw = requiredBoundedString(payload.action, "action", 12).toLowerCase();
        if (actionRaw !== "start" && actionRaw !== "stop") {
          throw new RequestValidationError("action must be either 'start' or 'stop'.", 400);
        }
        const result = await executeOpenClawGatewayAction(actionRaw);
        return writeJson(res, result.ok ? 200 : 500, {
          ok: result.ok,
      ...
      if (method === "GET" && path === "/api/config") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const config = await loadGatewayUiConfigPayload();
        return writeJson(res, 200, config);
      }

      if (method === "POST" && path === "/api/config") {
        assertMutationAuthorized(req, "/api/config");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "gateway config payload");
        const saved = await saveGatewayUiConfigPayload(payload);
        return writeJson(res, 200, {
          success: true,
          ...saved,
        });
      ...
      if (method === "GET" && path === "/api/config/detect-all") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const detected = await detectGatewayUiConfigPayload();
        return writeJson(res, 200, {
          success: true,
          data: detected,
        });
      ...
      if (method === "GET" && path === "/api/config/max-permissions") {
        assertAllowedQueryParams(url.searchParams, [], true);
        const settings = await readGatewayUiSettings();
        return writeJson(res, 200, {
          success: true,
          enabled: settings.maxPermissions,
        });
      }

      if (method === "POST" && path === "/api/config/max-permissions") {
        assertMutationAuthorized(req, "/api/config/max-permissions");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "gateway max permissions payload");
        if (typeof payload.enabled !== "boolean") {
          throw new RequestValidationError("enabled must be a boolean.", 400);
        }
        const settings = await writeGatewayUiSettings({ maxPermissions: payload.enabled });
        return writeJson(res, 200, {
          success: true,
          enabled: settings.maxPermissions,
        });
      ...
      if (method === "POST" && path === "/api/config/test") {
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const payload = expectObject(await readJsonBody(req), "gateway test payload");
        const result = await testGatewayConnection(payload);
        return writeJson(res, result.success ? 200 : 400, result);
      }

      if (method === "POST" && path === "/api/config/restart") {
        assertMutationAuthorized(req, "/api/config/restart");
        assertAllowedQueryParams(url.searchParams, [], true);
      ...
      if (method === "GET" && (path === "/sessions" || path === "/api/sessions")) {
        const snapshot = await readCachedReadModelSnapshot(toolClient);
      ...
      if (method === "POST" && path.startsWith("/api/sessions/") && path.endsWith("/messages")) {
        assertMutationAuthorized(req, "/api/sessions/:sessionKey/messages");
        assertAllowedQueryParams(url.searchParams, [], true);
        assertJsonContentType(req);
        const sessionKey = decodeRouteParam(path, /^\/api\/sessions\/([^/]+)\/messages$/, "sessionKey");
        const payload = expectObject(await readJsonBody(req), "session message payload");
        const message = requiredBoundedString(payload.message, "message", 4000);
...
async function countRecentToolCalls(snapshot: ReadModelSnapshot, toolClient: ToolClient): Promise<number> {
  ...
  const now = Date.now();
  if (recentToolCallsCache.expiresAt > now) return recentToolCallsCache.value;
  if (!recentToolCallsCache.inflight) {
    recentToolCallsCache.inflight = (async () => {
      try {
        ...
        const value = recentSessions.items.reduce((sum, item) => {
          if (typeof item.toolEventCount === "number") return sum + item.toolEventCount;
          return sum + (item.latestKind === "tool_event" ? 1 : 0);
        }, 0);
        recentToolCallsCache.value = value;
      } catch {
      } finally {
        recentToolCallsCache.expiresAt = Date.now() + 30_000;
        recentToolCallsCache.inflight = undefined;
      }
    })();
  }
  return recentToolCallsCache.value;
}

async function readCachedReadModelSnapshot(toolClient: ToolClient): Promise<ReadModelSnapshot> {
  const now = Date.now();
  if (readModelSnapshotCache && readModelSnapshotCache.expiresAt > now) {
    return readModelSnapshotCache.value;
  }
  if (!readModelSnapshotInFlight) {
    readModelSnapshotInFlight = readReadModelSnapshotWithLiveSessions(toolClient)
      .then((value) => {
        readModelSnapshotCache = {
          value,
      ...
      .finally(() => {
        readModelSnapshotInFlight = undefined;
      });
  }
  return await readModelSnapshotInFlight;
}
...
function dashboardSectionLinks(language: UiLanguage): DashboardSectionLink[] {
  return DASHBOARD_SECTION_LINKS_EN.map((item) => {
    ...
    if (item.key === "chat") {
      return { ...item, label: "聊天", blurb: "会话与对话" };
    }
    if (item.key === "ai-model-config") {
      return { ...item, label: "AI模型", blurb: "模型接口与密钥" };
    }
    if (item.key === "agent-management") {
      return { ...item, label: "智能体管理", blurb: "创建与管理智能体" };
    }
    if (item.key === "skills") {
      return { ...item, label: "技能", blurb: "已安装技能与接入配置" };
    }
    if (item.key === "chat-channel-config") {
      return { ...item, label: "渠道配置", blurb: "聊天渠道接入" };
    }
    if (item.key === "team") {
...
async function loadCachedSessionPreview(
  ...
  const now = Date.now();
  const normalizedHistoryLimit = Math.max(0, Math.min(200, Math.trunc(historyLimit)));
  if (
    renderSessionPreviewCache &&
    renderSessionPreviewCache.snapshotAt === snapshot.generatedAt &&
    renderSessionPreviewCache.historyLimit === normalizedHistoryLimit &&
...
async function renderHtml(
  ...
  const activeSection =
    options.section === "office-space"
      ...
      : options.section === "calendar"
        ? "projects-tasks"
        : options.section;
  const overviewWireframeOnly = activeSection === "overview";
  ...
  const sectionLeadText =
    activeSection === "overview"
      ...
      : activeSection === "projects-tasks"
        ...
        : sectionMeta.blurb;
  const needsSessionPreview = activeSection === "overview";
  const needsTaskEvidence = false;
  const needsTeamSnapshot = activeSection === "team";
  const needsTeamConfigSnapshot = activeSection === "overview";
  const needsGatewaySnapshot = activeSection === "overview";
  const needsChatChannelsSnapshot = activeSection === "overview";
  ...
  const sessionPreview = needsSessionPreview
    ? await loadCachedSessionPreview(snapshot, toolClient, activeSection === "overview" ? 0 : 2)
    : {
  ...
  const [teamSnapshot, gatewaySnapshot, chatChannelsSnapshot, memoryFiles, memoryFacetOptions, workspaceFiles, workspaceFacetOptions, taskEvidenceItems] = await Promise.all([
    needsTeamSnapshot || needsTeamConfigSnapshot
    ...
    needsGatewaySnapshot
      ? loadGatewayRuntimeSnapshot()
      : Promise.resolve<GatewayRuntimeSnapshot>({
    ...
    needsChatChannelsSnapshot
      ? loadChatChannelsSnapshot(snapshot, usageCost)
      : Promise.resolve<ChatChannelsSnapshot>({
          connectedChannels: [],
          connectedSessions: 0,
          sourcePath: OPENCLAW_CONFIG_PATH,
        }),
  ...
  const globalVisibilityModel = await buildGlobalVisibilityViewModel(snapshot, toolClient, options.language, {
    ...
    toolCallsCount: overviewWireframeOnly ? 0 : undefined,
  });
  ...
  const usageWireframeCards = renderUsageWireframeSummaryCards(usageCost.periods, options.language);
  const usageWireframeLegend = renderUsageWireframeLegend(usageSessionTypeRows, options.language);
  const usageWireframeTrendChart = renderUsageWireframeTrendChart(usageSessionTypeRows, options.language);
  ...
  const aiModelConfigHref = buildHomeHref({ quick: "all" }, options.compactStatusStrip, "ai-model-config", options.language, options.usageView);
  const agentManagementHref = buildHomeHref({ quick: "all" }, options.compactStatusStrip, "agent-management", options.language, options.usageView);
  const skillsHref = buildHomeHref({ quick: "all" }, options.compactStatusStrip, "skills", options.language, options.usageView);
  const gatewayConfigHref = buildHomeHref({ quick: "all" }, options.compactStatusStrip, "settings", options.language, options.usageView);
  const chatChannelConfigHref = buildHomeHref({ quick: "all" }, options.compactStatusStrip, "chat-channel-config", options.language, options.usageView);
  ...
  const wireframeModelSet = new Set<string>();
  for (const model of teamSnapshot.configuredModels) {
    const normalized = model.trim();
    if (!normalized) continue;
    wireframeModelSet.add(normalized);
  }
  if (wireframeModelSet.size === 0) {
    for (const member of teamSnapshot.members) {
      const normalized = member.model?.trim() ?? "";
      if (!normalized || normalized === "未标注") continue;
      wireframeModelSet.add(normalized);
    }
  }
  const wireframeModelList = Array.from(wireframeModelSet).slice(0, 3);
  if (wireframeModelList.length === 0) {
    wireframeModelList.push(t("No model configured", "未配置模型"));
  }
  const cachedSkillsSummary = options.section === "overview" ? getSkillsStatusSummaryFromCache(10 * 60 * 1000) : undefined;
  const installedSkillsCountLabel = cachedSkillsSummary
    ? `${formatInt(cachedSkillsSummary.installedSkills)}`
    : t("Loading", "加载中");
  const gatewayProtocolLabel = gatewaySnapshot.protocol || t("Unknown", "未知");
  const gatewayLatencyLabel =
    gatewaySnapshot.latencyMs === undefined
      ? t("Unknown", "未知")
      : gatewaySnapshot.reachability === "connected"
        ? `${gatewaySnapshot.latencyMs} ms`
        : t("Unreachable", "不可达");
  const gatewayCertExpiryLabel =
    gatewaySnapshot.certExpiryLabel ??
    (gatewaySnapshot.certStatus === "invalid"
      ? t("expired", "已过期")
      : "-");
  const gatewayRuntimeRunning = gatewaySnapshot.reachability === "connected";
  const gatewayRuntimeLines = [
    `endpoint: ${gatewaySnapshot.endpoint}`,
    `protocol: ${gatewayProtocolLabel}`,
    `latency: ${gatewayLatencyLabel}`,
    `cert-expiry: ${gatewayCertExpiryLabel}`,
  ];
  const chatEndpointRuntimeLines =
    chatChannelsSnapshot.connectedChannels.length > 0
      ? chatChannelsSnapshot.connectedChannels
      : [t("No connected channels", "未检测到已连接渠道")];
  const chatWorkspaceHref = buildHomeHref(filters, options.compactStatusStrip, "chat", options.language, options.usageView);
  const zhSwitchHref = buildHomeHref(filters, options.compactStatusStrip, options.section, "zh", options.usageView);
  const enSwitchHref = buildHomeHref(filters, options.compactStatusStrip, options.section, "en", options.usageView);
  const currentLanguageFlag = options.language === "en" ? "🇺🇸" : "🇨🇳";
  const wireframeStats = [
    {
      title: t("Today's usage", "今日消耗"),
      value:
        usageToday?.sourceStatus === "not_connected"
          ? t("Not connected", "未连接")
    ...
    {
      title: t("Sessions", "会话"),
      value: `${formatInt(liveSessionCount)}`,
      iconClass: "fa-solid fa-comments",
      href: chatWorkspaceHref,
    },
    {
      title: t("Tasks", "任务"),
      value: `${formatInt(snapshot.tasksSummary.tasks)}`,
      iconClass: "fa-solid fa-list-check",
      href: taskHubHref,
    },
    {
      title: t("Skills", "技能"),
      value: installedSkillsCountLabel,
      iconClass: "fa-solid fa-person",
      href: skillsHref,
    },
  ];
  const wireframeConfigCards = [
    {
      title: t("Model config", "AI模型配置"),
      accent: "wf-accent-red",
      lines: wireframeModelList,
      iconClass: "fa-solid fa-brain",
      href: aiModelConfigHref,
    },
    {
      title: t("Gateway config", "网关配置"),
      accent: "wf-accent-yellow",
      lines: gatewayRuntimeLines,
      iconClass: "fa-solid fa-network-wired",
      href: gatewayConfigHref,
    },
    {
      title: t("Chat channels config", "聊天渠道配置"),
      accent: "wf-accent-green",
      lines: chatEndpointRuntimeLines,
      iconClass: "fa-solid fa-comment-dots",
      href: chatChannelConfigHref,
    },
    {
      title: t("Agent management", "智能体管理"),
      accent: "wf-accent-blue",
      lines: [t("Create agents", "创建智能体"), t("Persona templates", "人格模板"), t("Independent model", "独立模型")],
      iconClass: "fa-solid fa-robot",
      href: agentManagementHref,
    },
  ];
  const overviewWireframeSection = `
    <section class="overview-wireframe-shell" id="overview-wireframe-home">
      <div class="wf-top-logo"><img src="/img/logo-v2.png" alt="TinyClaw" /></div>
      <div class="wf-mode-strip">
        <button
          id="wf-gateway-toggle"
          class="wf-mode-dot ${gatewayRuntimeRunning ? "wf-mode-red" : "wf-mode-green"} wf-mode-dot-btn"
          type="button"
          data-running="${gatewayRuntimeRunning ? "true" : "false"}"
          aria-label="${escapeHtml(gatewayRuntimeRunning ? t("Stop OpenClaw gateway", "停止 OpenClaw 网关") : t("Start OpenClaw gateway", "启动 OpenClaw 网关"))}"
          title="${escapeHtml(gatewayRuntimeRunning ? t("Stop OpenClaw gateway", "停止 OpenClaw 网关") : t("Start OpenClaw gateway", "启动 OpenClaw 网关"))}">
          <i class="fa-solid ${gatewayRuntimeRunning ? "fa-stop" : "fa-play"}" aria-hidden="true"></i>
        </button>
        <a
          class="wf-mode-dot wf-mode-blue"
          href="${escapeHtml(chatWorkspaceHref)}"
          aria-label="${escapeHtml(t("Open chat workspace", "打开聊天工作台"))}"
          title="${escapeHtml(t("Open chat workspace", "打开聊天工作台"))}">
          <i class="fa-solid fa-comments" aria-hidden="true"></i>
        </a>
        <div class="wf-mode-dot wf-mode-gray"><i class="fa-solid fa-user" aria-hidden="true"></i></div>
        <details class="wf-flag-menu">
          <summary class="wf-mode-dot wf-mode-flag" title="${escapeHtml(t("Switch language", "切换语言"))}" aria-label="${escapeHtml(t("Switch language", "切换语言"))}">${currentLanguageFlag}</summary>
          <div class="wf-flag-list">
            <a class="wf-flag-item${options.language === "zh" ? " active" : ""}" href="${escapeHtml(zhSwitchHref)}">🇨🇳</a>
            <a class="wf-flag-item${options.language === "en" ? " active" : ""}" href="${escapeHtml(enSwitchHref)}">🇺🇸</a>
          </div>
        </details>
      </div>
      <section class="wf-grid wf-grid-stats">
        ${wireframeStats
          .map((item) => {
            const cardBody = `<article class="wf-card wf-stat-card">
              <div class="wf-card-title">${escapeHtml(item.title)}</div>
              <div class="wf-card-main">
                <div class="wf-card-value">${escapeHtml(item.value)}</div>
                <div class="wf-card-icon"><i class="${escapeHtml(item.iconClass)}" aria-hidden="true"></i></div>
              </div>
            </article>`;
            return item.href
              ? `<a class="wf-card-link" href="${escapeHtml(item.href)}">${cardBody}</a>`
              : cardBody;
          })
          .join("")}
      </section>
      <section class="wf-grid wf-grid-config">
        ${wireframeConfigCards
          .map((card) => {
            const cardBody = `<article class="wf-card wf-config-card">
              <div class="wf-config-title ${escapeHtml(card.accent)}">${escapeHtml(card.title)}</div>
              <div class="wf-config-lines">${card.lines
                .map((line) => `<div>${escapeHtml(line)}</div>`)
                .join("")}</div>
              <div class="wf-config-icon ${escapeHtml(card.accent)}"><i class="${escapeHtml(card.iconClass)}" aria-hidden="true"></i></div>
            </article>`;
            return card.href ? `<a class="wf-card-link" href="${escapeHtml(card.href)}">${cardBody}</a>` : cardBody;
          })
          .join("")}
      </section>
    </section>
  ...
  `;
  const usageSection = `
    <section class="chat-page-shell usage-page-shell">

      <section class="usage-wf-shell">
        <div class="usage-wf-head">
          <h2>${escapeHtml(t("Token usage", "Token用量"))}</h2>
          <div class="usage-wf-controls">${usageViewSwitchHtml}</div>
        </div>
        <div class="meta">${usageViewRangeText}</div>
        <div class="meta">${usageViewRangeDetail}</div>
        <div class="usage-wf-kpi-grid">
          ${usageWireframeCards}
        </div>
        <div class="usage-wf-legend">${usageWireframeLegend}</div>
        <div class="usage-wf-chart-wrap">
          ${usageWireframeTrendChart}
        </div>
      </section>
      <details class="card compact-details" open>
        <summary>${escapeHtml(t("Details and data source", "明细与数据来源"))}</summary>
        <div class="fold-body">
          <h3 style="margin:0 0 6px 0;">${escapeHtml(t("Subscription windows", "订阅窗口"))}</h3>
          ${subscriptionStatusHtml}
          <div style="height:10px;"></div>
          <h3 style="margin:0 0 6px 0;">${escapeHtml(t("AI usage mix (all sessions)", "AI 用量构成（全部会话）"))}</h3>
          ...
          <div style="height:10px;"></div>
          <h3 style="margin:0 0 6px 0;">${escapeHtml(t("Timed-job usage share", "定时任务用量占比"))}</h3>
          ...
          <div style="height:10px;"></div>
          <h3 style="margin:0 0 6px 0;">${escapeHtml(t("Connector checklist", "连接器检查项"))}</h3>
  ...
  const cronDetailRows = allCronRows
    .slice(0, 20)
    .map((item) => {
      ...
      return `<tr>
        ...
        <td>${escapeHtml(sanitizeCronPurposeText(item.purpose, options.language, 56))}</td>
        <td><span class="wf-status-toggle ${enabled ? "is-on" : "is-off"}"><span class="wf-status-toggle-dot"></span></span></td>
        <td>${escapeHtml(item.nextRun)}</td>
      </tr>`;
    })
    .join("");
  const projectsSection = `
    <section class="card task-wireframe-shell">
      <div class="task-wireframe-summary">
        <article class="task-wireframe-stat">
          <div class="task-wireframe-stat-head"><span class="task-wireframe-icon">⏱</span>${escapeHtml(t("Timed jobs", "定时任务总数"))}</div>
          <strong>${allCronRows.length}</strong>
        </article>
        <article class="task-wireframe-stat">
          <div class="task-wireframe-stat-head"><span class="task-wireframe-icon">▶</span>${escapeHtml(t("Running jobs", "执行中的任务"))}</div>
          <strong>${allCronRows.filter((item) => item.status === "running").length}</strong>
        </article>
        <article class="task-wireframe-stat">
          <div class="task-wireframe-stat-head"><span class="task-wireframe-icon">!</span>${escapeHtml(t("Upcoming due", "即将截止的任务"))}</div>
          <strong>${upcomingTaskDueCount}</strong>
        </article>
      </div>
      <div class="task-wireframe-table-head">${escapeHtml(t("Cron detail", "Cron 任务明细"))}</div>
      <table class="task-wireframe-table">
        <thead><tr><th>${escapeHtml(t("Task", "任务"))}</th><th>${escapeHtml(t("Agent", "智能体"))}</th><th>${escapeHtml(t("Purpose", "任务目的"))}</th><th>${escapeHtml(t("Status", "状态"))}</th><th>${escapeHtml(t("Next run", "下次运行"))}</th></tr></thead>
        <tbody>${cronDetailRows || `<tr><td colspan="5">${escapeHtml(t("No timed jobs.", "暂无定时任务。"))}</td></tr>`}</tbody>
      </table>
    </section>
  `;
  const alertsSection = `
  ...
  const settingsSection = `
    <section class="chat-page-shell model-config-page-shell gateway-config-page-shell" data-gateway-config-root data-language="${escapeHtml(options.language)}">
      <section class="model-config-workspace">
        <section class="gateway-config-grid">
          <section class="model-config-block gateway-config-main">
            <div class="model-config-block-head">
              <h2>${escapeHtml(t("Connection settings", "连接设置"))}</h2>
              <button class="btn wf-add-btn wf-add-btn-blue" type="button" data-gateway-detect>${escapeHtml(t("Auto detect", "自动检测"))}</button>
            </div>
            <div class="meta" data-gateway-source>${escapeHtml(t("Loading gateway configuration...", "正在读取网关配置..."))}</div>
            <form class="model-config-form" data-gateway-form>
              <label>${escapeHtml(t("Gateway URL", "网关地址"))}
                <input type="text" name="gatewayUrl" required placeholder="ws://127.0.0.1:18789" />
              </label>
              <label>${escapeHtml(t("Public URL", "公网地址"))}
                <input type="text" name="publicGatewayUrl" readonly />
              </label>
              <label>${escapeHtml(t("Token", "Token"))}
                <input type="text" name="token" />
              </label>
              <label>${escapeHtml(t("Password", "Password（可选）"))}
                <input type="password" name="password" placeholder="${escapeHtml(t("Optional", "可选"))}" />
              </label>
              <label>${escapeHtml(t("OpenClaw workspace path", "OpenClaw 工作区路径"))}
                <input type="text" name="openclawWorkspace" placeholder="/root/.openclaw/workspace-main" />
              </label>
            </form>
          </section>
          <section class="gateway-config-side">
            <section class="model-config-block">
              <h2>${escapeHtml(t("Permission settings", "权限设置"))}</h2>
              <div class="meta">${escapeHtml(t("Enable max permissions for OpenClaw tools.", "开启后将解锁浏览器自动化、命令执行（免确认）、文件操作等全部工具权限。关闭则使用默认 coding 预设。"))}</div>
              <div class="gateway-toggle-row">
                <span>${escapeHtml(t("Max permissions", "最大化权限"))}</span>
                <button class="gateway-toggle-btn" type="button" data-gateway-max-permission><span data-gateway-max-permission-label>${escapeHtml(t("Disabled", "关闭"))}</span></button>
              </div>
            </section>
            <section class="model-config-block">
              <h2>${escapeHtml(t("Domain management", "域名管理"))}</h2>
              <div class="meta">${escapeHtml(t("Manage allowed domains for reverse proxy whitelist.", "管理允许访问此网页的域名（用于反向代理安全白名单）。"))}</div>
              <div class="gateway-host-row">
                <input type="text" data-gateway-host-input placeholder="${escapeHtml(t("e.g. openclaw.example.com", "例如: openclaw.abc.com"))}" />
                <button class="btn wf-confirm-btn" type="button" data-gateway-host-add>${escapeHtml(t("Add", "添加"))}</button>
              </div>
              <div class="gateway-host-list" data-gateway-host-list></div>
            </section>
          </section>
        </section>
        <section class="gateway-config-actions">
          <div class="gateway-action-row">
            <button class="btn wf-add-btn wf-add-btn-blue" type="button" data-gateway-test>${escapeHtml(t("Test connection", "测试连接"))}</button>
            <button class="btn wf-cancel-btn" type="button" data-gateway-restart>${escapeHtml(t("Restart gateway", "重启网关"))}</button>
            <button class="btn wf-confirm-btn" type="button" data-gateway-save>${escapeHtml(t("Save settings", "保存设置"))}</button>
          </div>
          <span class="meta" data-gateway-status></span>
        </section>
        <section class="model-config-block" style="display:none;" id="tool-connectors">
  ...
  const aiModelSection = `
    <section class="chat-page-shell model-config-page-shell ai-model-page-shell" data-model-config-root data-language="${escapeHtml(options.language)}">
      <section class="model-config-workspace">
        <section class="model-config-block">
          <div class="model-config-block-head">
            <h2>${escapeHtml(t("Model management", "模型管理"))}</h2>
            <button class="btn wf-add-btn" type="button" data-open-model-modal="add">${escapeHtml(t("Add model", "添加模型"))}</button>
          </div>
          <div class="meta" data-model-config-source>${escapeHtml(t("Loading configuration...", "正在读取配置..."))}</div>
          <div class="model-config-table-wrap">
            <table class="model-config-grid-table" data-model-table>
              <thead>
                <tr>
                  <th>${escapeHtml(t("Model alias", "模型别名"))}</th>
                  <th>${escapeHtml(t("Model ID", "模型ID"))}</th>
                  <th>${escapeHtml(t("Default", "是否默认"))}</th>
                  <th>${escapeHtml(t("Price", "价格"))}</th>
                  <th>${escapeHtml(t("Actions", "操作"))}</th>
                </tr>
              </thead>
              <tbody>
                <tr><td colspan="5" class="meta">${escapeHtml(t("Loading models...", "正在加载模型..."))}</td></tr>
              </tbody>
            </table>
          </div>
        </section>
        <section class="model-config-block">
          <div class="model-config-block-head">
            <h2>${escapeHtml(t("Endpoint management", "节点管理"))}</h2>
            <button class="btn wf-add-btn wf-add-btn-blue" type="button" data-open-endpoint-modal="add">${escapeHtml(t("Add endpoint", "添加节点"))}</button>
          </div>
          <div class="model-config-table-wrap">
            <table class="model-config-grid-table" data-endpoint-table>
              <thead>
                <tr>
                  <th>${escapeHtml(t("Endpoint name", "节点名"))}</th>
                  <th>${escapeHtml(t("Base URL", "调用地址"))}</th>
                  <th>${escapeHtml(t("API type", "接口类型"))}</th>
                  <th>${escapeHtml(t("Models", "模型数"))}</th>
                  <th>${escapeHtml(t("Actions", "操作"))}</th>
                </tr>
              </thead>
              <tbody>
                <tr><td colspan="5" class="meta">${escapeHtml(t("Loading endpoints...", "正在加载节点..."))}</td></tr>
              </tbody>
            </table>
          </div>
        </section>
        <div class="model-config-modal-backdrop" data-model-modal-backdrop hidden>
          <div class="model-config-modal" role="dialog" aria-modal="true">
            <button class="model-config-modal-close" type="button" data-model-modal-close><i class="fa-solid fa-xmark"></i></button>
            <h3 data-model-modal-title>${escapeHtml(t("Add/Edit model", "添加/修改模型"))}</h3>
            <form class="model-config-form" data-model-modal-form>
              <label>${escapeHtml(t("Endpoint", "节点选择"))}
                <select name="endpoint" data-model-endpoint-select required></select>
              </label>
              <label>${escapeHtml(t("Model ID", "模型ID"))}
                <input type="text" name="modelName" required placeholder="${escapeHtml(t("Please input", "单行输入"))}" />
              </label>
              <label>${escapeHtml(t("Model alias", "模型别名"))}
                <input type="text" name="alias" placeholder="${escapeHtml(t("Please input", "单行输入"))}" />
              </label>
              <div class="model-config-form-row">
                <label>${escapeHtml(t("Input token price", "模型输入价格（元/M）"))}
                  <input type="text" name="priceIn" placeholder="${escapeHtml(t("Please input", "单行输入"))}" />
                </label>
                <label>${escapeHtml(t("Output token price", "模型输出价格（元/M）"))}
                  <input type="text" name="priceOut" placeholder="${escapeHtml(t("Please input", "单行输入"))}" />
                </label>
              </div>
              <div class="model-config-modal-actions">
                <button class="btn wf-cancel-btn" type="button" data-model-modal-cancel>${escapeHtml(t("Cancel", "取消"))}</button>
                <button class="btn wf-confirm-btn" type="submit">${escapeHtml(t("Confirm", "确认"))}</button>
              </div>
            </form>
          </div>
        </div>
        <div class="model-config-modal-backdrop" data-endpoint-modal-backdrop hidden>
          <div class="model-config-modal" role="dialog" aria-modal="true">
            <button class="model-config-modal-close" type="button" data-endpoint-modal-close><i class="fa-solid fa-xmark"></i></button>
            <h3 data-endpoint-modal-title>${escapeHtml(t("Add/Edit endpoint", "添加/修改节点"))}</h3>
            <form class="model-config-form" data-endpoint-modal-form>
              <label>${escapeHtml(t("Endpoint key", "节点名"))}
                <input type="text" name="endpointId" required placeholder="${escapeHtml(t("Please input", "单行输入"))}" />
              </label>
              <label>${escapeHtml(t("Base URL", "调用地址"))}
                <input type="url" name="baseUrl" required placeholder="${escapeHtml(t("https://api.example.com/v1", "https://api.example.com/v1"))}" />
              </label>
              <label>${escapeHtml(t("API key", "API-key"))}
                <input type="password" name="apiKey" placeholder="${escapeHtml(t("Optional when unchanged", "不修改可留空"))}" />
              </label>
              <label>${escapeHtml(t("API type", "接口类型"))}
                <select name="apiType">
                  ...
                  <option value="cohere-chat">Cohere</option>
                  <option value="mistral-chat">Mistral</option>
                  <option value="ollama">Ollama</option>
                </select>
              </label>
              <div class="model-config-modal-actions">
                <button class="btn wf-cancel-btn" type="button" data-endpoint-modal-cancel>${escapeHtml(t("Cancel", "取消"))}</button>
                <button class="btn wf-confirm-btn" type="submit">${escapeHtml(t("Confirm", "确认"))}</button>
              </div>
            </form>
          </div>
        </div>
        <span class="meta" data-model-config-status></span>
      </section>
    </section>
  `;
  const agentManagementSection = `
    <section class="chat-page-shell model-config-page-shell agent-management-page-shell" data-agent-management-root data-language="${escapeHtml(options.language)}">
      <section class="model-config-workspace">
        <section class="model-config-block">
          <div class="model-config-block-head">
            <h2>${escapeHtml(t("Agent management", "智能体管理"))}</h2>
            <button class="btn wf-add-btn wf-add-btn-blue" type="button" data-agent-open-modal="create">${escapeHtml(t("Create agent", "新建智能体"))}</button>
          </div>
          <div class="meta" data-agent-source>${escapeHtml(t("Loading agents...", "正在读取智能体..."))}</div>
          <div class="model-config-table-wrap">
            <table class="model-config-grid-table" data-agent-table>
              <thead>
                <tr>
                  <th>${escapeHtml(t("Agent ID", "智能体ID"))}</th>
                  <th>${escapeHtml(t("Agent name", "智能体名称"))}</th>
                  <th>${escapeHtml(t("Model", "独立模型"))}</th>
                  <th>${escapeHtml(t("Actions", "操作"))}</th>
                </tr>
              </thead>
              <tbody>
                <tr><td colspan="4" class="meta">${escapeHtml(t("Loading agents...", "正在读取智能体..."))}</td></tr>
              </tbody>
            </table>
          </div>
        </section>
        <span class="meta" data-agent-status></span>
        <div class="model-config-modal-backdrop" data-agent-modal-backdrop hidden>
          <div class="model-config-modal" role="dialog" aria-modal="true">
            <button class="model-config-modal-close" type="button" data-agent-modal-close><i class="fa-solid fa-xmark"></i></button>
            <h3 data-agent-modal-title>${escapeHtml(t("Create/Edit agent", "新建/编辑智能体"))}</h3>
            <form class="model-config-form" data-agent-modal-form>
              <label>${escapeHtml(t("Agent ID", "智能体ID"))}
                <input type="text" name="id" required placeholder="translation_agent" />
              </label>
              <label>${escapeHtml(t("Agent name", "智能体名称"))}
                <input type="text" name="name" required placeholder="${escapeHtml(t("e.g. Translation Assistant", "例如 翻译助手"))}" />
              </label>
              <label>${escapeHtml(t("Model (optional)", "独立模型（可选）"))}
                <div class="agent-model-picker">
                  <input type="text" name="model" data-agent-model-input placeholder="${escapeHtml(t("Click to select model (optional)", "点击选择模型（留空使用默认）"))}" autocomplete="off" />
                  <div class="agent-model-menu" data-agent-model-menu hidden></div>
                </div>
              </label>
              <div class="agent-md-editor-shell">
                <div class="agent-md-tabs">
                  <button type="button" class="agent-md-tab active" data-agent-tab="soulContent">SOUL.md | ${escapeHtml(t("Persona", "人格"))}</button>
                  <button type="button" class="agent-md-tab" data-agent-tab="userContent">USER.md | ${escapeHtml(t("User", "用户"))}</button>
                  <button type="button" class="agent-md-tab" data-agent-tab="agentsContent">AGENTS.md | ${escapeHtml(t("Agents", "智能体"))}</button>
                  <button type="button" class="agent-md-tab" data-agent-tab="toolsContent">TOOLS.md | ${escapeHtml(t("Tools", "工具"))}</button>
                  <button type="button" class="agent-md-tab" data-agent-tab="heartbeatContent">HEARTBEAT.md | ${escapeHtml(t("Heartbeat", "心跳"))}</button>
                  <button type="button" class="agent-md-tab" data-agent-tab="identityContent">IDENTITY.md | ${escapeHtml(t("Identity", "身份"))}</button>
                </div>
                <textarea class="agent-md-editor-input" data-agent-md-editor rows="8"></textarea>
              </div>
              <div class="model-config-modal-actions">
                <button class="btn wf-cancel-btn" type="button" data-agent-modal-cancel>${escapeHtml(t("Cancel", "取消"))}</button>
                <button class="btn wf-confirm-btn" type="submit">${escapeHtml(t("Confirm", "确认"))}</button>
              </div>
            </form>
          </div>
  ...
  `;
  const skillsSection = `
    <section class="chat-page-shell model-config-page-shell skills-page-shell" data-skills-root data-language="${escapeHtml(options.language)}">
      <section class="model-config-workspace">
        <section class="model-config-block">
          <div class="model-config-block-head">
            <h2>${escapeHtml(t("Skills", "技能"))}</h2>
            <button class="btn wf-add-btn wf-add-btn-blue" type="button" data-skill-open-modal>${escapeHtml(t("Create skill", "新建技能"))}</button>
          </div>
          <div class="meta" data-skill-source>${escapeHtml(t("Loading skills...", "正在读取技能..."))}</div>
          <div class="skills-table-section">
            <div class="meta">${escapeHtml(t("Enabled skills", "已启用技能"))}</div>
          </div>
          <div class="model-config-table-wrap">
            <table class="model-config-grid-table" data-skill-table-enabled>
              <thead>
                <tr>
                  <th>${escapeHtml(t("Skill key", "技能标识"))}</th>
                  <th>${escapeHtml(t("Description", "说明"))}</th>
                  <th>${escapeHtml(t("Source", "来源"))}</th>
                  <th>${escapeHtml(t("Status", "状态"))}</th>
                </tr>
              </thead>
              <tbody>
                <tr><td colspan="4" class="meta">${escapeHtml(t("Loading skills...", "正在读取技能..."))}</td></tr>
              </tbody>
            </table>
          </div>
          <button class="skills-collapsed-row" type="button" data-skill-disabled-toggle>${escapeHtml(t("Disabled skills", "未启用的技能"))}</button>
          <div class="model-config-table-wrap" data-skill-disabled-panel hidden>
            <table class="model-config-grid-table" data-skill-table>
              <thead>
                <tr>
                  <th>${escapeHtml(t("Skill key", "技能标识"))}</th>
                  <th>${escapeHtml(t("Description", "说明"))}</th>
        ...
        <span class="meta" data-skill-status></span>
        <div class="model-config-modal-backdrop" data-skill-modal-backdrop hidden>
          <div class="model-config-modal" role="dialog" aria-modal="true">
            <button class="model-config-modal-close" type="button" data-skill-modal-close><i class="fa-solid fa-xmark"></i></button>
            <h3>${escapeHtml(t("Create skill", "新建技能"))}</h3>
            <form class="model-config-form" data-skill-modal-form>
              <label>${escapeHtml(t("Source", "来源"))}
                <input type="text" name="source" required placeholder="${escapeHtml(t("npm package / git url / local path", "npm包名 / git 地址 / 本地路径"))}" />
              </label>
              <div class="model-config-modal-actions">
                <button class="btn wf-cancel-btn" type="button" data-skill-modal-cancel>${escapeHtml(t("Cancel", "取消"))}</button>
                <button class="btn wf-confirm-btn" type="submit">${escapeHtml(t("Install", "安装"))}</button>
              </div>
            </form>
          </div>
        </div>
  ...
  `;
  const chatChannelConfigSection = `
    <section class="chat-page-shell model-config-page-shell chat-channel-config-page-shell" data-channel-config-root data-language="${escapeHtml(options.language)}">
      <section class="model-config-workspace">
        <section class="model-config-block channel-config-main-block">
          <h2>${escapeHtml(t("Connected channels", "已连入渠道"))}</h2>
          <div class="meta" data-channel-config-source></div>
          <div class="channel-wire-grid" data-channel-config-list>
            <div class="empty-state">${escapeHtml(t("Loading channels...", "正在加载渠道列表..."))}</div>
          </div>
        </section>
        <section class="channel-config-add-row">
          <div class="channel-config-add-action-line">
            <button class="btn wf-confirm-btn channel-config-plus-btn" type="button" data-channel-config-open-dialog><i class="fa-solid fa-plus" aria-hidden="true"></i></button>
            <div class="meta">${escapeHtml(t("Click + to add or replace channel settings.", "点击 + 通过对话框新增或覆盖渠道配置。"))}</div>
          </div>
        </section>
        <span class="meta" data-channel-config-status></span>
        <dialog class="channel-config-dialog" data-channel-config-dialog>
          <form class="model-config-form" method="dialog" data-channel-config-form>
            <label>${escapeHtml(t("Channel key", "渠道标识"))}
              <input type="text" name="channelKey" required placeholder="${escapeHtml(t("e.g. feishu", "例如 feishu"))}" />
            </label>
            <label>${escapeHtml(t("Config JSON", "配置 JSON"))}
              <textarea name="channelConfigJson" rows="10" required placeholder="${escapeHtml(t("{\"appId\":\"...\"}", "{\"appId\":\"...\"}"))}"></textarea>
            </label>
            <div class="model-config-modal-actions">
              <button class="btn wf-cancel-btn" type="button" data-channel-config-cancel>${escapeHtml(t("Cancel", "取消"))}</button>
              <button class="btn wf-confirm-btn" type="submit">${escapeHtml(t("Confirm", "确认"))}</button>
            </div>
          </form>
        </dialog>
  ...
  `;
  let chatGatewaySrc = "/chat-gateway/?ccv=20260318a";
  if (activeSection === "chat") {
    const gatewayConfig = await loadGatewayUiConfigPayload();
    const params = new URLSearchParams();
    params.set("ccv", "20260318a");
    let gatewayUrl = (GATEWAY_URL || gatewayConfig.gatewayUrl || "").trim();
    try {
      const parsedGateway = new URL(gatewayUrl);
      const requestHost = typeof options.requestHost === "string" ? options.requestHost.split(",")[0].trim() : "";
      const requestHostname = requestHost.replace(/:\d+$/, "");
      const localHosts = new Set(["127.0.0.1", "localhost", "::1"]);
      if (requestHostname && localHosts.has(parsedGateway.hostname.toLowerCase())) {
        const wsProtocol = parsedGateway.protocol === "wss:" ? "wss:" : "ws:";
        gatewayUrl = `${wsProtocol}//${requestHostname}:${parsedGateway.port || "18789"}`;
      }
    } catch {}
    const gatewayToken = (gatewayConfig.token ?? "").trim();
    if (gatewayUrl) params.set("gatewayUrl", gatewayUrl);
    if (gatewayToken) params.set("token", gatewayToken);
    chatGatewaySrc = `/chat-gateway/?${params.toString()}`;
  }
  const chatSection = `
    <section class="chat-page-shell" data-language="${escapeHtml(options.language)}">
      <header class="chat-topbar">
        <a class="chat-topbar-brand" href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "overview", options.language, options.usageView))}">
          <img src="/img/logo-v2.png" alt="TinyClaw" />
          <strong>TinyClaw</strong>
        </a>
        <nav class="chat-topbar-nav">
          <a href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "ai-model-config", options.language, options.usageView))}">${escapeHtml(t("AI model", "AI模型"))}</a>
          <a href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "settings", options.language, options.usageView))}">${escapeHtml(t("Gateway", "网关"))}</a>
          <a href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "chat-channel-config", options.language, options.usageView))}">${escapeHtml(t("Channels", "渠道"))}</a>
          <a href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "agent-management", options.language, options.usageView))}">${escapeHtml(t("Agents", "智能体管理"))}</a>
          <a href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "skills", options.language, options.usageView))}">${escapeHtml(t("Skills", "技能"))}</a>
          <a href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "usage-cost", options.language, options.usageView))}">${escapeHtml(t("Statistics", "统计"))}</a>
        </nav>
      </header>
      <section class="chat-gateway-shell">
        <iframe class="chat-gateway-frame" src="${escapeHtml(chatGatewaySrc)}" title="${escapeHtml(t("TinyClaw Chat", "TinyClaw 聊天"))}" loading="eager" referrerpolicy="same-origin" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-downloads"></iframe>
      </section>
    </section>
  `;
  let sectionBody = overviewSection;
  if (options.section === "chat") sectionBody = chatSection;
  if (options.section === "ai-model-config") sectionBody = aiModelSection;
  if (options.section === "agent-management") sectionBody = agentManagementSection;
  if (options.section === "skills") sectionBody = skillsSection;
  if (options.section === "chat-channel-config") sectionBody = chatChannelConfigSection;
  ...
  const fileWorkbenchScript = options.section === "memory" || options.section === "docs" ? renderFileWorkbenchScript() : "";
  ...
  const adminAuthBootstrapScript = renderAdminAuthBootstrapScript(options.language);
  const gatewayControlScript = options.section === "overview" ? renderGatewayControlScript(options.language) : "";
  const chatWorkspaceScript = options.section === "chat" ? renderChatWorkspaceScript(options.language) : "";
  const gatewaySettingsScript = options.section === "settings" ? renderGatewaySettingsScript(options.language) : "";
  const agentManagementScript = options.section === "agent-management" ? renderAgentManagementScript(options.language) : "";
  const skillsScript = options.section === "skills" ? renderSkillsScript(options.language) : "";
  const modelConfigScript = options.section === "ai-model-config" ? renderModelConfigScript(options.language) : "";
  const channelConfigScript = options.section === "chat-channel-config" ? renderChannelConfigScript(options.language) : "";
  const quotaResetScript = options.section === "usage-cost" ? renderQuotaResetScript() : "";
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    h1, h2, h3 { margin: 0; line-height: 1.87em; letter-spacing: -0.012em; }
    a {
      color: #0071e3;
    ...
    .sidebar li,
    .sidebar span,
    .inspector-sidebar .meta,
    .inspector-sidebar .meta-inline,
    .inspector-sidebar p,
    .inspector-sidebar li,
    .inspector-sidebar span {
      font-size: 14px;
      line-height: 1.5em;
    }
    ...
    body.overview-wireframe {
      background: #343638;
    }
    body.overview-wireframe::before {
      display: none;
    }
    body.overview-wireframe .app-shell {
      grid-template-columns: minmax(0, 1fr);
      max-width: 960px;
      padding: 34px 28px 42px;
      gap: 0;
    }
    body.overview-wireframe .sidebar,
    body.overview-wireframe .inspector-sidebar,
    body.overview-wireframe .section-hero-head {
      display: none;
    }
    body.overview-wireframe .panel {
      border: none;
    ...
    body.overview-wireframe .content-stack {
      margin-top: 0;
      gap: 0;
    }
    body.overview-wireframe #global-visibility-card,
    body.overview-wireframe #overview-decision-home,
    body.overview-wireframe #overview-primary-section,
    body.overview-wireframe #overview-secondary-shell {
      display: none;
    }
    body.chat-workspace-only {
      background: #343638;
    }
    body.chat-workspace-only::before {
      display: none;
    }
    body.chat-workspace-only .app-shell {
      grid-template-columns: minmax(0, 1fr);
      max-width: none;
      padding: 0;
      gap: 0;
    }
    body.chat-workspace-only .sidebar,
    body.chat-workspace-only .inspector-sidebar,
    body.chat-workspace-only .section-hero-head {
      display: none;
    }
    body.chat-workspace-only .panel {
      border: none;
      background: transparent;
      box-shadow: none;
      backdrop-filter: none;
    ...
    body.chat-workspace-only .content-stack {
      margin-top: 0;
      gap: 0;
    }
    body.model-config-workspace-only {
      background: #343638;
    }
    body.model-config-workspace-only::before {
      display: none;
    }
    body.model-config-workspace-only .app-shell {
      grid-template-columns: minmax(0, 1fr);
      max-width: none;
      padding: 0;
      gap: 0;
    }
    body.model-config-workspace-only .sidebar,
    body.model-config-workspace-only .inspector-sidebar,
    body.model-config-workspace-only .section-hero-head {
      display: none;
    }
    body.model-config-workspace-only .panel {
      border: none;
      background: transparent;
      box-shadow: none;
      backdrop-filter: none;
    ...
    body.model-config-workspace-only .model-config-page-shell h1,
    body.model-config-workspace-only .model-config-page-shell h2,
    body.model-config-workspace-only .model-config-page-shell h3,
    body.model-config-workspace-only .model-config-page-shell h4,
    body.model-config-workspace-only .model-config-page-shell h5,
    body.model-config-workspace-only .model-config-page-shell h6,
    body.model-config-workspace-only .model-config-page-shell strong,
    body.model-config-workspace-only .model-config-page-shell th,
    body.model-config-workspace-only .model-config-page-shell label,
    body.model-config-workspace-only .model-config-page-shell button,
    body.model-config-workspace-only .model-config-page-shell a {
      font-weight: 400 !important;
    }
    body.model-config-workspace-only .content-stack {
      margin-top: 0;
      gap: 0;
    }
    body.usage-workspace-only {
      background: #343638;
    }
    body.usage-workspace-only::before {
      display: none;
    }
    body.usage-workspace-only .app-shell {
      grid-template-columns: minmax(0, 1fr);
      max-width: none;
      padding: 10px 12px 12px;
      gap: 10px;
    }
    body.usage-workspace-only .app-shell > aside.sidebar:not(.inspector-sidebar) {
      display: none;
    }
    body.usage-workspace-only .panel {
    ...
    body.usage-workspace-only .section-title {
      color: #aaaaaa;
      font-size: 18px;
      line-height: 1.5em;
    }
    body.usage-workspace-only .section-blurb,
    body.usage-workspace-only .meta {
      color: #777777;
      font-size: 12px;
      line-height: 1.87em;
    }
    body.usage-workspace-only .inspector-sidebar {
      display: none;
    ...
    body.usage-workspace-only .card {
      border-color: rgba(255, 255, 255, 0.08);
      background: #23262a;
      box-shadow: none;
      border-radius: 12px;
    ...
    body.usage-workspace-only .card::before {
      box-shadow: none;
    }
    body.usage-workspace-only .card h2 {
      color: #aaaaaa;
    }
    body.usage-workspace-only h2,
    body.usage-workspace-only h3 {
      font-size: 16px;
      line-height: 1.5em;
      color: #aaaaaa;
    }
    body.usage-workspace-only h1 {
      font-size: 18px;
      line-height: 1.5em;
      color: #aaaaaa;
    }
    body.usage-workspace-only th {
      color: #aab2bc;
      font-size: 14px;
      line-height: 1.5em;
    }
    body.usage-workspace-only td {
      border-bottom-color: rgba(255, 255, 255, 0.15);
      font-size: 14px;
      line-height: 1.5em;
    }
    body.usage-workspace-only tr:hover td {
      background: rgba(255, 255, 255, 0.1);
    }
    body.usage-workspace-only .badge.todo,
    body.usage-workspace-only .badge.planned,
    body.usage-workspace-only .badge.message,
    body.usage-workspace-only .badge.tool_event,
    body.usage-workspace-only .badge.idle,
    body.usage-workspace-only .badge.disabled {
      color: #c8ced6;
      border-color: rgba(255, 255, 255, 0.22);
      background: rgba(255, 255, 255, 0.08);
    }
    body.usage-workspace-only a {
      color: #aaaaaa;
    }
    body.usage-workspace-only .empty-state {
      background: #23262a;
      border-color: rgba(255, 255, 255, 0.08);
      color: #d5dbe2;
      border-radius: 12px;
    ...
    body.usage-workspace-only .panel-toggle {
    ...
    body.usage-workspace-only .panel-toggle:hover {
      background: #3b4046;
      border-color: rgba(255, 255, 255, 0.18);
      color: #edf1f5;
      box-shadow: none;
    }
    body.usage-workspace-only .segment-switch {
      border-color: rgba(255, 255, 255, 0.1);
      background: #2a2d31;
      box-shadow: none;
      border-radius: 12px;
    }
    body.usage-workspace-only .segment-item {
      color: #c7ced6;
      border-radius: 12px;
    }
    body.usage-workspace-only .segment-item:hover {
      color: #edf1f5;
      background: #34383d;
    }
    body.usage-workspace-only .segment-item.active {
      background: #3d434a;
      color: #eef2f7;
      box-shadow: inset 0 0 0 1px rgba(255, 255, 255, 0.18);
    }
    body.usage-workspace-only .btn {
    ...
    body.usage-workspace-only .btn:hover {
      border-color: rgba(255, 255, 255, 0.2);
      background: #3b4046;
      color: #eef2f6;
      box-shadow: none;
    }
    body.usage-workspace-only .chat-topbar {
      margin-bottom: 12px;
  
      border: 1px solid rgba(255, 255, 255, 0.08);
      background: #23262a;
    }
    body.usage-workspace-only .chat-topbar .chat-topbar-brand strong,
    body.usage-workspace-only .chat-topbar .chat-topbar-nav,
    body.usage-workspace-only .chat-topbar .chat-topbar-nav a {
      color: #aaaaaa;
      font-size: 14px;
      line-height: 1.5em;
    }
    body.usage-workspace-only .status-chip strong,
    body.usage-workspace-only .usage-chip strong,
    body.usage-workspace-only strong {
      font-size: 14px;
      line-height: 1.5em;
    }
    body.usage-workspace-only summary {
      font-size: 18px;
      line-height: 1.5em;
      color: #00a3ff;
    }
    body.usage-workspace-only .status-chip,
    body.usage-workspace-only .badge,
    body.usage-workspace-only .compact-table-details,
    body.usage-workspace-only .queue-item {
      border-radius: 12px;
    }
    body.usage-workspace-only .usage-wf-shell {
      border: 1px solid rgba(255, 255, 255, 0.08);
      border-radius: 12px;
      background: #1f2225;
    ...
    body.usage-workspace-only .usage-wf-head {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 10px;
    }
    body.usage-workspace-only .usage-wf-head h2 {
      margin: 0;
      color: #c9ced5;
      font-size: 18px;
      line-height: 1.5em;
    }
    body.usage-workspace-only .usage-wf-controls .segment-switch {
      margin: 0;
    }
    body.usage-workspace-only .usage-wf-kpi-grid {
      margin-top: 2px;
      display: grid;
      grid-template-columns: repeat(3, minmax(0, 1fr));
      gap: 14px;
    }
    body.usage-workspace-only .usage-wf-kpi {
      border: 1px solid rgba(255, 255, 255, 0.08);
      border-radius: 12px;
      background: #35393d;
      padding: 12px 14px;
    ...
    body.usage-workspace-only .usage-wf-kpi-label {
      color: #c2c8d0;
      font-size: 14px;
      line-height: 1.5em;
    }
    body.usage-workspace-only .usage-wf-kpi-value {
      color: #00d66e;
      font-size: 18px;
      line-height: 1.5em;
    ...
    body.usage-workspace-only .usage-wf-kpi-meta {
      color: #aab2bc;
      font-size: 14px;
      line-height: 1.5em;
    }
    body.usage-workspace-only .usage-wf-legend {
      display: flex;
      align-items: center;
      flex-wrap: wrap;
      gap: 14px;
    ...
    body.usage-workspace-only .usage-wf-legend-item {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      line-height: 1.5em;
    }
    body.usage-workspace-only .usage-wf-dot {
      width: 10px;
      height: 10px;
      border-radius: 999px;
      box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.15) inset;
    }
    body.usage-workspace-only .usage-wf-chart-wrap {
      border: 1px solid rgba(255, 255, 255, 0.08);
      border-radius: 12px;
      background: #22262a;
      padding: 8px 6px 2px;
    ...
    body.usage-workspace-only .usage-wf-svg {
      width: 100%;
      height: 324px;
      display: block;
    }
    .overview-wireframe-shell {
      display: grid;
    ...
    .wf-mode-dot-btn:hover {
      transform: translateY(-1px);
      box-shadow: 0 6px 16px rgba(0, 0, 0, 0.25);
    }
    .wf-mode-dot-btn:active {
      transform: translateY(0);
    }
    .wf-mode-dot-btn:disabled {
      cursor: wait;
      opacity: 0.65;
    }
    .wf-mode-dot i,
    ...
    .wf-mode-green { background: #00df7f; }
    .wf-mode-red { background: #ff3b08; color: #fff; }
    .wf-mode-blue { background: #00a3ff; }
    .wf-mode-gray { background: #b8bdc3; }
    .wf-mode-flag {
      background: #2b2f35;
      color: #fff;
      font-size: 21px;
    ...
    .wf-mode-flag::-webkit-details-marker { display: none; }
    .wf-flag-menu {
      position: relative;
    }
    .wf-flag-menu[open] .wf-mode-flag {
      border-color: rgba(0, 163, 255, 0.65);
      box-shadow: 0 0 0 2px rgba(0, 163, 255, 0.18);
    }
    .wf-flag-list {
      position: absolute;
      top: calc(100% + 8px);
    ...
    .wf-grid-config .wf-card-link {
      height: 100%;
    }
    .wf-grid-config .wf-config-card {
      height: 100%;
      min-height: 172px;
    }
    .wf-card-icon {
    ...
    .wf-accent-red { color: #ff3b08; }
    .wf-accent-yellow { color: #ffd059; }
    .wf-accent-green { color: #00df7f; }
    .wf-accent-blue { color: #00a3ff; }
    .wf-config-lines {
      color: #acb1b7;
      display: grid;
      gap: 2px;
    ...
    .wf-config-icon.wf-accent-red { color: #ff3b08; }
    .wf-config-icon.wf-accent-yellow { color: #ffd059; }
    .wf-config-icon.wf-accent-green { color: #00df7f; }
    .wf-config-icon.wf-accent-blue { color: #00a3ff; }
    .chat-page-shell {
      min-height: 100vh;
      display: grid;
    ...
    .chat-page-shell .empty-state.timeout-tip {
      background: #23262a;
      border-color: rgba(255, 255, 255, 0.08);
      color: #d5dbe2;
    }
    ...
    .chat-topbar-nav a:hover {
      color: #d6dbe1;
      text-decoration: none;
    }
    .ai-model-page-shell .chat-topbar-nav a:first-child {
      color: #f0f2f5;
    }
    .gateway-config-page-shell .chat-topbar-nav a:nth-child(2) {
      color: #f0f2f5;
    }
    .chat-channel-config-page-shell .chat-topbar-nav a:nth-child(3) {
      color: #f0f2f5;
    }
    .agent-management-page-shell .chat-topbar-nav a:nth-child(4) {
      color: #f0f2f5;
    }
    .skills-page-shell .chat-topbar-nav a:nth-child(5) {
      color: #f0f2f5;
    }
    .usage-workspace-only .chat-topbar-nav a:nth-child(6) {
      color: #f0f2f5;
    }
    ...
    .model-config-block h2 {
      margin: 0;
      font-size: 18px;
      color: #edf0f3;
    }
    .model-config-block-head {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 10px;
    ...
    .model-config-grid-table {
      width: 100%;
      border-collapse: collapse;
      table-layout: fixed;
      min-width: 980px;
    }
    .model-config-grid-table th,
    .model-config-grid-table td {
    ...
    .model-config-grid-table td:last-child,
    .model-config-grid-table th:last-child {
      text-align: center;
      width: 130px;
    }
    .skills-page-shell .model-config-grid-table td:last-child,
    .skills-page-shell .model-config-grid-table th:last-child {
      width: 260px;
      text-align: center;
    }
    .skills-status-cell {
    ...
    .skills-collapsed-row:hover {
      border-color: rgba(127, 213, 255, 0.7);
      color: #7fd5ff;
      background: rgba(0, 163, 255, 0.08);
    }
    .skills-status-badges {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      flex-wrap: wrap;
    ...
    .model-config-row-action:hover {
      border-color: rgba(127, 213, 255, 0.72);
      color: #7fd5ff;
    }
    .model-config-row-action-danger {
      color: #ff9e95;
      border-color: rgba(255, 59, 8, 0.32);
    }
    .model-config-row-action-danger:hover {
      color: #ff5a4a;
      border-color: rgba(255, 90, 74, 0.68);
    }
    .skills-check-icon {
      display: inline-flex;
      align-items: center;
    ...
    .skill-enable-action {
      min-width: 44px;
      color: #0f8b44;
      border-color: rgba(16, 173, 88, 0.45);
      background: rgba(20, 218, 112, 0.14);
      font-size: 12px;
      padding: 0 8px;
    }
    .skill-enable-action:hover {
      color: #0da04d;
      border-color: rgba(20, 218, 112, 0.68);
      background: rgba(20, 218, 112, 0.22);
    }
    .badge.neutral {
      color: #333333;
    ...
    .model-config-form input:focus {
      border-color: rgba(0, 163, 255, 0.6);
    }
    .model-config-form textarea:focus {
      border-color: rgba(0, 163, 255, 0.6);
    }
    ...
    .model-config-inline-actions {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      gap: 12px;
    }
    .model-config-inline-icon {
      border: none;
      background: transparent;
      color: #d9dfe6;
      cursor: pointer;
    ...
    .model-config-inline-icon.danger {
      color: #ff3b08;
    }
    ...
    .model-config-modal-backdrop[hidden] {
    ...
    .model-config-modal h3 {
      margin: 0;
      font-size: 16px;
      color: #dce2e9;
      text-align: center;
    }
    .model-config-modal-close {
      position: absolute;
      top: 10px;
      right: 10px;
      width: 28px;
    ...
    .model-config-form-row {
      display: grid;
      grid-template-columns: repeat(2, minmax(0, 1fr));
      gap: 12px;
    }
    .model-config-modal-actions {
      margin-top: 8px;
      display: grid;
      grid-template-columns: repeat(2, minmax(0, 1fr));
      gap: 18px;
    ...
    .model-config-modal-actions .wf-cancel-btn {
      min-height: 38px;
      border: none;
      border-radius: 8px;
      background: #6b7076;
    ...
    .model-config-modal-actions .wf-confirm-btn {
      min-height: 38px;
      border: none;
      border-radius: 8px;
      background: #10db67;
    ...
    .channel-wire-badge.ok {
      color: #10db67;
    }
    .channel-wire-badge.warn {
      color: #ffd059;
    }
    .channel-wire-form {
      display: grid;
    ...
    .channel-wire-field input,
    .channel-wire-field select {
      border-radius: 8px;
    ...
    .channel-wire-actions {
      display: flex;
      justify-content: flex-end;
      gap: 10px;
    }
    .channel-config-add-row {
      display: grid;
      gap: 8px;
    }
    .channel-config-add-action-line {
      display: flex;
      align-items: center;
      gap: 12px;
    }
    .channel-config-plus-btn {
      min-width: 92px;
    }
    .channel-config-page-shell .section-title {
      font-size: 18px;
      color: #d7dce3;
    }
    ...
    .channel-config-dialog::backdrop {
    ...
    .chat-message-item.role-user .chat-message-body {
      background: #3f4348;
      border: 1px solid rgba(255, 255, 255, 0.06);
    }
    .chat-message-item.role-assistant,
    .chat-message-item.role-system {
      justify-self: start;
    }
    .chat-message-item.role-assistant .chat-message-body,
    .chat-message-item.role-system .chat-message-body {
      background: #23262a;
      border: 1px solid rgba(255, 255, 255, 0.06);
    }
    .chat-message-meta {
    ...
    .chat-message-item.role-user .chat-message-meta {
      justify-content: flex-end;
    }
    .chat-message-body {
      color: #e8ebef;
    ...
    .badge.ok, .badge.done { color: #1d7435; border-color: rgba(36, 138, 61, 0.3); background:#00df7f; }
    ...
    tr:hover td { background: rgba(245, 247, 250, 0.1); }
    ...
    .story-list { margin-top: 8px; line-height: 1.87; }
    ...
    .task-wireframe-shell {
      background: #2d2f31;
      border: 1px solid #3a3d40;
      border-radius: 10px;
      padding: 14px;
    }
    .task-wireframe-shell::before {
      display: none;
    }
    .task-wireframe-summary {
      display: grid;
      grid-template-columns: repeat(3, minmax(0, 1fr));
      gap: 12px;
      margin-bottom: 14px;
    ...
    .task-wireframe-table-head {
      margin: 10px 0 8px;
      color: #c8ced4;
      font-size: 13px;
      font-weight: 600;
    }
    .task-wireframe-table {
      width: 100%;
      border-collapse: collapse;
      border: 1px solid #3a3d40;
      border-top: 0;
    ...
    .task-wireframe-table th,
    .task-wireframe-table td {
      border: 1px solid #3a3d40;
      padding: 10px 12px;
      font-size: 13px;
      text-align: left;
    ...
    .wf-status-toggle.is-on {
      background: #00c16a;
      border-color: #00c16a;
      justify-content: flex-end;
    }
    .wf-status-toggle-dot {
      width: 14px;
      height: 14px;
      border-radius: 50%;
      background: #ffffff;
...
<body class="${options.section === "overview" ? "overview-wireframe " : ""}${options.section === "chat" ? "chat-workspace-only " : ""}${options.section === "ai-model-config" || options.section === "chat-channel-config" || options.section === "settings" || options.section === "agent-management" || options.section === "skills" || options.section === "projects-tasks" ? "model-config-workspace-only " : ""}${options.section === "usage-cost" ? "usage-workspace-only " : ""}ui-preload" data-ui-polish="apple-native-v3" data-apple-window-controls="true" data-ui-language="${escapeHtml(options.language)}" style="--fold-open-label:${options.language === "en" ? "'Expand'" : "'展开'"}; --fold-close-label:${options.language === "en" ? "'Collapse'" : "'收起'"};">
  ${
    options.section === "usage-cost" || options.section === "ai-model-config" || options.section === "settings" || options.section === "chat-channel-config" || options.section === "agent-management" || options.section === "skills" || options.section === "projects-tasks"
      ? `<header class="chat-topbar">
      <a class="chat-topbar-brand" href="${escapeHtml(buildHomeHref(filters, options.compactStatusStrip, "overview", options.language, options.usageView))}">
        <img src="/img/logo-v2.png" alt="TinyClaw" />
        <strong>TinyClaw</strong>
  ...
  ${adminAuthBootstrapScript}
  ${fileWorkbenchScript}
  ${nativeMotionScript}
  ${gatewayControlScript}
  ${chatWorkspaceScript}
  ${gatewaySettingsScript}
  ${agentManagementScript}
  ${skillsScript}
  ${modelConfigScript}
  ${channelConfigScript}
  ${quotaResetScript}
...
function parseSessionQuery(searchParams: URLSearchParams, strict: boolean): SessionQuery {
  ...
  return {
    ...
    historyLimit: readNonNegativeIntQuery(searchParams.get("historyLimit"), "historyLimit", 8, strict, 200),
  };
}
...
async function loadTeamSnapshot(officeRoster: AgentRosterSnapshot): Promise<TeamSnapshot> {
  ...
  try {
    ...
    const configuredModels = extractConfiguredModelsFromOpenClawJson(parsed);
...
function extractConfiguredModelsFromOpenClawJson(parsed: Record<string, unknown>): string[] {
  const result = new Set<string>();
  const agentsRoot = (parsed.agents ?? {}) as Record<string, unknown>;
  const defaults = (agentsRoot.defaults ?? {}) as Record<string, unknown>;
  const defaultModel = ((defaults.model ?? {}) as Record<string, unknown>).primary;
  if (typeof defaultModel === "string" && defaultModel.trim()) {
    result.add(defaultModel.trim());
  }

  const agentList = Array.isArray(agentsRoot.list) ? agentsRoot.list : [];
  ...
  const modelsRoot = (parsed.models ?? {}) as Record<string, unknown>;
  ...
  for (const providerValue of Object.values(providers)) {
    ...
    for (const modelItem of models) {
      ...
      const modelObj = modelItem as Record<string, unknown>;
      const idValue = modelObj.id;
      const nameValue = modelObj.name;
      ...
      else if (normalizedName) result.add(normalizedName);
    }
  }
...
interface AppendModelProviderInput {
  baseUrl: string;
  apiKey: string;
  modelName: string;
...
async function loadModelProviderSummary(): Promise<ModelProviderSummary> {
  const raw = await safeReadTextFile(OPENCLAW_CONFIG_PATH);
  if (!raw) {
    return {
      sourcePath: OPENCLAW_CONFIG_PATH,
  ...
  try {
    ...
    return {
      sourcePath: OPENCLAW_CONFIG_PATH,
      defaultPrimaryModel: extractDefaultPrimaryModel(parsed),
      providers: parseModelProviderEntries(parsed),
    };
  } catch {
    return {
      sourcePath: OPENCLAW_CONFIG_PATH,
...
async function appendModelProviderConfig(input: AppendModelProviderInput): Promise<{
  providerKey: string;
  modelKey: string;
  ...
  const parsed = raw
    ...
    : {};
  const root = parsed;
  const modelsRoot = ensureObjectRecord(root, "models");
  const providersRoot = ensureObjectRecord(modelsRoot, "providers");
  const normalizedBaseUrl = input.baseUrl.trim();
  const normalizedModelName = input.modelName.trim();
  const providerKey = resolveModelProviderKey(providersRoot, normalizedBaseUrl, input.providerAlias);
  const providerRecord = ensureObjectRecord(providersRoot, providerKey);
  if (typeof providerRecord.api !== "string" || !providerRecord.api.trim()) {
    providerRecord.api = "openai-completions";
  } else if (providerRecord.api === "openai") {
    providerRecord.api = "openai-completions";
  }
  providerRecord.baseUrl = normalizedBaseUrl;
  providerRecord.apiKey = input.apiKey.trim();
  const models = Array.isArray(providerRecord.models) ? providerRecord.models : [];
  const normalizedNameLower = normalizedModelName.toLowerCase();
  const exists = models.some((item) => {
    if (!item || typeof item !== "object") return false;
    const obj = item as Record<string, unknown>;
    const id = typeof obj.id === "string" ? obj.id.trim().toLowerCase() : "";
    const name = typeof obj.name === "string" ? obj.name.trim().toLowerCase() : "";
    return id === normalizedNameLower || name === normalizedNameLower;
  });
  if (!exists) {
    models.push({
  ...
  providerRecord.models = models;

  const agentsRoot = ensureObjectRecord(root, "agents");
  const defaultsRoot = ensureObjectRecord(agentsRoot, "defaults");
  const modelRoot = ensureObjectRecord(defaultsRoot, "model");
  const modelsMap = ensureObjectRecord(defaultsRoot, "models");
  const modelKey = normalizedModelName.includes("/") ? normalizedModelName : `${providerKey}/${normalizedModelName}`;
  if (!(modelKey in modelsMap)) {
    modelsMap[modelKey] = {};
  }
  if (
    typeof modelRoot.primary !== "string" ||
    !modelRoot.primary.trim() ||
    !String(modelRoot.primary).includes("/")
  ) {
    modelRoot.primary = modelKey;
  }

  const metaRoot = ensureObjectRecord(root, "meta");
  metaRoot.lastTouchedAt = new Date().toISOString();

  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
  const syncedDefaultAgentId = await syncModelProviderToDefaultAgent({
    providerKey,
  ...
  return {
    providerKey,
    modelKey,
    syncedDefaultAgentId,
    defaultPrimaryModel: extractDefaultPrimaryModel(root),
    providers: parseModelProviderEntries(root),
  };
}
...
interface SyncModelProviderToDefaultAgentInput {
  providerKey: string;
  baseUrl: string;
  apiKey: string;
  modelName: string;
...
async function listOpenClawAgents(): Promise<AgentListEntry[]> {
  const result = await runCommandCapture("openclaw", ["agents", "list", "--json"], 10000);
  ...
  try {
    ...
    for (const item of parsed) {
      ...
      const id = firstNonEmptyString(row.id, row.agentId);
      if (!id) continue;
      rows.push({
        id,
...
async function detectDefaultAgentId(root: Record<string, unknown>): Promise<string | undefined> {
  const cliRows = await listOpenClawAgents();
  const fromCli = cliRows.find((item) => item.isDefault)?.id ?? cliRows[0]?.id;
  ...
  for (const item of list) {
    if (!item || typeof item !== "object") continue;
    const row = item as Record<string, unknown>;
    const id = firstNonEmptyString(row.id, row.agentId, row.name);
    if (id) return id;
  }
  return undefined;
}

async function syncModelProviderToDefaultAgent(input: SyncModelProviderToDefaultAgentInput): Promise<string | undefined> {
  const root = await readOpenClawConfigRoot();
  const defaultAgentId = await detectDefaultAgentId(root);
  if (!defaultAgentId) return undefined;

  const agentModelConfigPath = join(OPENCLAW_HOME_DIR, "agents", defaultAgentId, "agent", "models.json");
  const raw = await safeReadTextFile(agentModelConfigPath);
  ...
  const providersRoot = ensureObjectRecord(agentConfig, "providers");
  const providerRecord = ensureObjectRecord(providersRoot, input.providerKey);
  providerRecord.api = "openai-completions";
  providerRecord.baseUrl = input.baseUrl;
  providerRecord.apiKey = input.apiKey;

  const models = Array.isArray(providerRecord.models) ? providerRecord.models : [];
  const normalizedModelId = input.modelName.toLowerCase();
  const exists = models.some((item) => {
    if (!item || typeof item !== "object") return false;
    const row = item as Record<string, unknown>;
  ...
  await mkdir(dirname(agentModelConfigPath), { recursive: true });
  await writeFile(agentModelConfigPath, `${JSON.stringify(agentConfig, null, 2)}\n`, "utf8");
  await runCommandCapture("openclaw", ["models", "set", "--agent", defaultAgentId, input.modelKey], 12000);
  return defaultAgentId;
}

interface CompatEndpointRecord {
  id: string;
  baseUrl: string;
  api: string;
  apiKey: string;
...
interface CompatModelRecord {
  id: string;
  alias: string;
  primary: boolean;
}

interface CompatGatewayPayload {
  endpoints: CompatEndpointRecord[];
  models: CompatModelRecord[];
  defaultModel: string;
}

async function readCompatGatewayConfigPayload(): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  return buildCompatGatewayPayload(root);
}

async function upsertCompatEndpoint(input: {
  id: string;
  baseUrl: string;
  api: string;
  apiKey: string;
}): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  const providersRoot = ensureObjectRecord(ensureObjectRecord(root, "models"), "providers");
  const provider = ensureObjectRecord(providersRoot, input.id);
  const existingApiKey = firstNonEmptyString(provider.apiKey, provider.apikey, provider.token, provider.key);
  const normalizedApi = (input.api || "").trim().toLowerCase();
  provider.api = normalizedApi && normalizedApi !== "openai" ? normalizedApi : "openai-responses";
  provider.baseUrl = input.baseUrl;
  provider.apiKey = input.apiKey || existingApiKey;
  if (!Array.isArray(provider.models)) {
    provider.models = [];
  }
  touchOpenClawConfigMeta(root);
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
  return buildCompatGatewayPayload(root);
}

async function deleteCompatEndpoint(endpointId: string): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  const providersRoot = ensureObjectRecord(ensureObjectRecord(root, "models"), "providers");
  if (endpointId in providersRoot) {
    delete providersRoot[endpointId];
  }
  const removedPrefix = `${endpointId}/`;
  const defaultModel = extractDefaultPrimaryModel(root);
  if (defaultModel.startsWith(removedPrefix)) {
    const modelRoot = ensureObjectRecord(ensureObjectRecord(ensureObjectRecord(root, "agents"), "defaults"), "model");
    modelRoot.primary = "";
  }
  touchOpenClawConfigMeta(root);
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
...
async function createCompatModel(input: {
  endpoint: string;
  modelName: string;
  alias?: string;
}): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  const providersRoot = ensureObjectRecord(ensureObjectRecord(root, "models"), "providers");
  const provider = ensureObjectRecord(providersRoot, input.endpoint);
  if (!Array.isArray(provider.models)) {
    provider.models = [];
  }
  const fullId = `${input.endpoint}/${input.modelName}`;
  ...
  if (!exists) {
    models.push({
      id: fullId,
      name: input.alias && input.alias.trim().length > 0 ? input.alias.trim() : input.modelName,
    });
  }
  const modelRoot = ensureObjectRecord(ensureObjectRecord(ensureObjectRecord(root, "agents"), "defaults"), "model");
  if (typeof modelRoot.primary !== "string" || !modelRoot.primary.trim()) {
    modelRoot.primary = fullId;
  }
  touchOpenClawConfigMeta(root);
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
  return buildCompatGatewayPayload(root);
...
async function updateCompatModel(input: {
  id: string;
  alias?: string;
}): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  const providersRoot = ensureObjectRecord(ensureObjectRecord(root, "models"), "providers");
  for (const providerValue of Object.values(providersRoot)) {
    if (!providerValue || typeof providerValue !== "object") continue;
    const providerObj = providerValue as Record<string, unknown>;
    const models = Array.isArray(providerObj.models) ? providerObj.models : [];
    for (const item of models) {
      ...
      const id = typeof modelObj.id === "string" ? modelObj.id.trim() : "";
      if (id !== input.id) continue;
      if (input.alias && input.alias.trim().length > 0) {
        modelObj.name = input.alias.trim();
      }
      touchOpenClawConfigMeta(root);
      await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
      return buildCompatGatewayPayload(root);
...
async function deleteCompatModel(id: string): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  const providersRoot = ensureObjectRecord(ensureObjectRecord(root, "models"), "providers");
  for (const providerValue of Object.values(providersRoot)) {
    if (!providerValue || typeof providerValue !== "object") continue;
    const providerObj = providerValue as Record<string, unknown>;
    const models = Array.isArray(providerObj.models) ? providerObj.models : [];
    providerObj.models = models.filter((item) => {
      if (!item || typeof item !== "object") return false;
      const modelObj = item as Record<string, unknown>;
      const currentId = typeof modelObj.id === "string" ? modelObj.id.trim() : "";
      return currentId !== id;
    });
  }
  const modelRoot = ensureObjectRecord(ensureObjectRecord(ensureObjectRecord(root, "agents"), "defaults"), "model");
  if (typeof modelRoot.primary === "string" && modelRoot.primary.trim() === id) {
    modelRoot.primary = "";
  }
  touchOpenClawConfigMeta(root);
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
...
async function setCompatModelDefault(id: string): Promise<CompatGatewayPayload> {
  const root = await readOpenClawConfigRoot();
  const modelRoot = ensureObjectRecord(ensureObjectRecord(ensureObjectRecord(root, "agents"), "defaults"), "model");
  modelRoot.primary = id;
  touchOpenClawConfigMeta(root);
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
  return buildCompatGatewayPayload(root);
}
...
function buildCompatGatewayPayload(root: Record<string, unknown>): CompatGatewayPayload {
  const providersRoot = ensureObjectRecord(ensureObjectRecord(root, "models"), "providers");
  const endpoints: CompatEndpointRecord[] = [];
  const models: CompatModelRecord[] = [];
  const defaultModel = extractDefaultPrimaryModel(root);
  for (const [providerKey, providerValue] of Object.entries(providersRoot)) {
    if (!providerValue || typeof providerValue !== "object") continue;
    const providerObj = providerValue as Record<string, unknown>;
    endpoints.push({
      id: providerKey,
      baseUrl: firstNonEmptyString(providerObj.baseUrl, providerObj.baseURL, providerObj.endpoint, providerObj.url),
      api: firstNonEmptyString(providerObj.api, "openai-responses"),
      apiKey: firstNonEmptyString(providerObj.apiKey, providerObj.apikey, providerObj.token, providerObj.key),
    });
    const modelRows = Array.isArray(providerObj.models) ? providerObj.models : [];
    for (const modelItem of modelRows) {
      if (!modelItem || typeof modelItem !== "object") continue;
      const modelObj = modelItem as Record<string, unknown>;
      const id = firstNonEmptyString(modelObj.id, modelObj.name);
      if (!id) continue;
      models.push({
        id,
        alias: firstNonEmptyString(modelObj.name, modelObj.alias),
        primary: id === defaultModel,
      });
    }
  }
  endpoints.sort((a, b) => a.id.localeCompare(b.id, "zh-Hans-CN"));
  models.sort((a, b) => a.id.localeCompare(b.id, "zh-Hans-CN"));
  return {
    endpoints,
    models,
    defaultModel,
...
function touchOpenClawConfigMeta(root: Record<string, unknown>): void {
  const metaRoot = ensureObjectRecord(root, "meta");
  metaRoot.lastTouchedAt = new Date().toISOString();
}

interface GatewayUiSettings {
  allowedHosts: string[];
  maxPermissions: boolean;
  password: string;
}

interface GatewayUiConfigPayload {
  gatewayUrl: string;
  token: string;
  password: string;
...
async function loadGatewayUiConfigPayload(): Promise<GatewayUiConfigPayload> {
  const root = await readOpenClawConfigRoot();
  const settings = await readGatewayUiSettings();
  const gatewayRoot = ensureObjectRecord(root, "gateway");
  const authRoot = ensureObjectRecord(gatewayRoot, "auth");
  const agentsRoot = ensureObjectRecord(root, "agents");
  const defaultsRoot = ensureObjectRecord(agentsRoot, "defaults");
  const gatewayUrl = resolveGatewayUrlFromConfig(root);
  const token = firstNonEmptyString(authRoot.token, gatewayRoot.token);
  const openclawWorkspace = firstNonEmptyString(defaultsRoot.workspace);
  return {
    gatewayUrl,
    token,
...
async function detectGatewayUiConfigPayload(): Promise<Record<string, unknown>> {
  const loaded = await loadGatewayUiConfigPayload();
  let gatewayPort = "";
  try {
    const status = await runOpenClawGatewayCommand(["status", "--json"], 8000);
    const parsed = JSON.parse(status.stdout) as Record<string, unknown>;
    const portRoot = (parsed.port ?? {}) as Record<string, unknown>;
    const listeners = Array.isArray(portRoot.listeners) ? portRoot.listeners : [];
    for (const listener of listeners) {
      if (!listener || typeof listener !== "object") continue;
      const row = listener as Record<string, unknown>;
      const value = firstNonEmptyString(row.port, row.listenPort, row.boundPort);
      if (value) {
        gatewayPort = value;
        break;
      }
    ...
    if (!gatewayPort) {
      gatewayPort = firstNonEmptyString(portRoot.port, portRoot.boundPort);
    }
  } catch {}
  return {
    ...
    gatewayPort,
    token: loaded.token,
    workspacePath: loaded.openclawWorkspace,
    allowedHosts: loaded.allowedHosts,
    password: loaded.password,
  };
}

async function saveGatewayUiConfigPayload(payload: Record<string, unknown>): Promise<GatewayUiConfigPayload> {
  const root = await readOpenClawConfigRoot();
  const gatewayRoot = ensureObjectRecord(root, "gateway");
  const authRoot = ensureObjectRecord(gatewayRoot, "auth");
  const agentsRoot = ensureObjectRecord(root, "agents");
  const defaultsRoot = ensureObjectRecord(agentsRoot, "defaults");
  const gatewayUrl = optionalBoundedString(payload.gatewayUrl, "gatewayUrl", 1024)?.trim();
  const token = optionalBoundedString(payload.token, "token", 1024)?.trim();
  const openclawWorkspace = optionalBoundedString(payload.openclawWorkspace, "openclawWorkspace", 2048)?.trim();
  const password = optionalBoundedString(payload.password, "password", 1024)?.trim();
  const allowedHostsRaw = payload.allowedHosts;
  if (gatewayUrl && gatewayUrl.length > 0) {
    applyGatewayUrlToConfig(gatewayRoot, gatewayUrl);
  }
  if (token !== undefined) {
    authRoot.mode = token ? "token" : "none";
    authRoot.token = token;
  }
  if (openclawWorkspace !== undefined) {
    defaultsRoot.workspace = openclawWorkspace;
  }
  touchOpenClawConfigMeta(root);
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
  const settings = await writeGatewayUiSettings({
    allowedHosts: normalizeGatewayAllowedHosts(allowedHostsRaw),
    password: password ?? undefined,
  });
  return {
    gatewayUrl: resolveGatewayUrlFromConfig(root),
    token: firstNonEmptyString(authRoot.token, gatewayRoot.token),
    password: settings.password,
    openclawWorkspace: firstNonEmptyString(defaultsRoot.workspace),
    allowedHosts: settings.allowedHosts,
  };
}

function applyGatewayUrlToConfig(gatewayRoot: Record<string, unknown>, gatewayUrl: string): void {
  if ("endpoint" in gatewayRoot) {
    delete gatewayRoot.endpoint;
  }
  if ("url" in gatewayRoot) {
    delete gatewayRoot.url;
  }
  try {
    const parsed = new URL(gatewayUrl.includes("://") ? gatewayUrl : `ws://${gatewayUrl}`);
    const port = Number.parseInt(parsed.port || "", 10);
    if (Number.isFinite(port) && port > 0) {
      gatewayRoot.port = port;
    }
...
function resolveGatewayUrlFromConfig(root: Record<string, unknown>): string {
  const gatewayRoot = ensureObjectRecord(root, "gateway");
  const fromConfig = firstNonEmptyString(gatewayRoot.endpoint, gatewayRoot.url);
  if (fromConfig) return fromConfig;
  const portRaw = gatewayRoot.port;
  const parsedPort =
    typeof portRaw === "number" && Number.isFinite(portRaw)
      ...
      : typeof portRaw === "string"
        ...
        : Number.NaN;
  const port = Number.isFinite(parsedPort) && parsedPort > 0 ? parsedPort : 18789;
  return `ws://127.0.0.1:${port}`;
}

function normalizeGatewayAllowedHosts(value: unknown): string[] | undefined {
  ...
  for (const item of value) {
    ...
    if (!host) continue;
    if (host.length > 260) continue;
    unique.add(host);
  }
  return Array.from(unique);
}

async function readGatewayUiSettings(): Promise<GatewayUiSettings> {
  const raw = await safeReadTextFile(GATEWAY_UI_SETTINGS_PATH);
  if (!raw) {
    return {
      allowedHosts: [],
      maxPermissions: false,
      password: "",
    };
  }
  try {
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    return {
      allowedHosts: normalizeGatewayAllowedHosts(parsed.allowedHosts) ?? [],
      maxPermissions: parsed.maxPermissions === true,
      password: firstNonEmptyString(parsed.password),
    };
  } catch {
    return {
      allowedHosts: [],
...
async function writeGatewayUiSettings(input: {
  allowedHosts?: string[] | undefined;
  maxPermissions?: boolean | undefined;
  password?: string | undefined;
}): Promise<GatewayUiSettings> {
  const current = await readGatewayUiSettings();
  const next: GatewayUiSettings = {
    allowedHosts: input.allowedHosts ?? current.allowedHosts,
    maxPermissions: input.maxPermissions ?? current.maxPermissions,
    password: input.password ?? current.password,
  };
  const payload = {
    allowedHosts: next.allowedHosts,
    maxPermissions: next.maxPermissions,
    password: next.password,
    updatedAt: new Date().toISOString(),
  };
  await writeFile(GATEWAY_UI_SETTINGS_PATH, `${JSON.stringify(payload, null, 2)}\n`, "utf8");
  return next;
}

async function testGatewayConnection(payload: Record<string, unknown>): Promise<{
  success: boolean;
  message: string;
  output?: string;
}> {
  // This probe intentionally avoids spawning openclaw CLI health checks.
  // It validates endpoint reachability at transport level and, for wss,
  // includes a lightweight certificate probe.
  const loaded = await loadGatewayUiConfigPayload();
  const gatewayUrl = optionalBoundedString(payload.gatewayUrl, "gatewayUrl", 1024)?.trim() || loaded.gatewayUrl;
  if (!gatewayUrl) {
    return { success: false, message: "Gateway URL is required." };
  }
  let parsed: URL;
  try {
    parsed = new URL(gatewayUrl);
  } catch {
    return { success: false, message: "Gateway URL is invalid." };
  }
  const protocol = parsed.protocol.toLowerCase();
  const host = parsed.hostname;
  const port =
    Number.parseInt(parsed.port, 10) ||
    (protocol === "wss:" ? 443 : protocol === "ws:" ? 80 : Number.NaN);
  if (!host || !Number.isFinite(port) || port <= 0) {
    return { success: false, message: "Gateway URL is invalid." };
  }
  const tcpProbe = await probeTcpReachability(host, port, 1800);
  if (!tcpProbe.reachable) {
    return {
      success: false,
      message: `Gateway not reachable: ${host}:${port}`,
      output: `protocol=${protocol.replace(":", "")}, host=${host}, port=${port}`,
    };
  }
  if (protocol === "wss:") {
    const certProbe = await probeTlsCertificate(host, port, 2200);
    const certLine = `cert=${certProbe.status}${certProbe.expiryLabel ? ` (${certProbe.expiryLabel})` : ""}`;
    return {
      success: true,
      message: "Connection successful.",
      output: `protocol=wss, host=${host}, port=${port}, latency=${tcpProbe.latencyMs ?? "n/a"}ms, ${certLine}`,
    };
  }
  return {
    success: true,
    message: "Connection successful.",
    output: `protocol=${protocol.replace(":", "")}, host=${host}, port=${port}, latency=${tcpProbe.latencyMs ?? "n/a"}ms`,
  };
}

async function restartGatewayService(): Promise<{
  success: boolean;
  message: string;
  output?: string;
}> {
  const attempts = [
    { command: "sudo", args: ["-n", "systemctl", "restart", "openclaw-gateway.service"] },
    { command: "systemctl", args: ["restart", "openclaw-gateway.service"] },
    { command: "openclaw", args: ["gateway", "restart"] },
  ];
  const outputs: string[] = [];
  for (const attempt of attempts) {
    const result = await runCommandCapture(attempt.command, attempt.args, 20000);
    const output = `${result.stdout}\n${result.stderr}`.trim();
    if (output) outputs.push(`${attempt.command} ${attempt.args.join(" ")}\n${output}`);
    if (result.ok) {
      return {
        success: true,
...
function parseModelProviderEntries(parsed: Record<string, unknown>): ModelProviderEntry[] {
  const modelsRoot = (parsed.models ?? {}) as Record<string, unknown>;
  const providersRoot = (modelsRoot.providers ?? {}) as Record<string, unknown>;
  const rows: ModelProviderEntry[] = [];
  for (const [providerKey, providerValue] of Object.entries(providersRoot)) {
    if (!providerValue || typeof providerValue !== "object") continue;
    const providerObj = providerValue as Record<string, unknown>;
    const api = typeof providerObj.api === "string" && providerObj.api.trim() ? providerObj.api.trim() : "openai";
    const baseUrl = firstNonEmptyString(
      providerObj.baseUrl,
      providerObj.baseURL,
      providerObj.endpoint,
      providerObj.url,
    );
    const apiKeyRaw = firstNonEmptyString(
      providerObj.apiKey,
      providerObj.apikey,
      providerObj.token,
      providerObj.key,
  ...
  rows.sort((a, b) => a.providerKey.localeCompare(b.providerKey, "zh-Hans-CN"));
  return rows;
}

function extractDefaultPrimaryModel(parsed: Record<string, unknown>): string {
  const agentsRoot = (parsed.agents ?? {}) as Record<string, unknown>;
  const defaultsRoot = (agentsRoot.defaults ?? {}) as Record<string, unknown>;
  const modelRoot = (defaultsRoot.model ?? {}) as Record<string, unknown>;
  return typeof modelRoot.primary === "string" ? modelRoot.primary.trim() : "";
}
...
function normalizeModelProviderSlug(value: string): string {
  return value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
function resolveModelProviderKey(
  providersRoot: Record<string, unknown>,
  baseUrl: string,
  alias?: string,
): string {
  const aliasKey = alias ? normalizeModelProviderSlug(alias) : "";
  if (aliasKey) return aliasKey;
  const normalizedBase = baseUrl.trim().toLowerCase().replace(/\/+$/, "");
  for (const [providerKey, providerValue] of Object.entries(providersRoot)) {
    if (!providerValue || typeof providerValue !== "object") continue;
    const providerObj = providerValue as Record<string, unknown>;
    const existingBase = firstNonEmptyString(providerObj.baseUrl, providerObj.baseURL, providerObj.endpoint, providerObj.url)
      .toLowerCase()
      .replace(/\/+$/, "");
    if (existingBase && existingBase === normalizedBase) {
      return providerKey;
    }
  }
  const fallbackKey = normalizeModelProviderSlug(baseUrl);
  if (!fallbackKey) return `provider-${Date.now().toString(36)}`;
  if (!(fallbackKey in providersRoot)) return fallbackKey;
  let suffix = 2;
  while (`${fallbackKey}-${suffix}` in providersRoot) {
    suffix += 1;
  }
  return `${fallbackKey}-${suffix}`;
}

function ensureObjectRecord(root: Record<string, unknown>, key: string): Record<string, unknown> {
...
interface AppendChannelConfigInput {
  channelKey: string;
  config: Record<string, unknown>;
}

async function loadChannelConfigSummary(): Promise<ChannelConfigSummary> {
  const raw = await safeReadTextFile(OPENCLAW_CONFIG_PATH);
  if (!raw) {
    return {
      sourcePath: OPENCLAW_CONFIG_PATH,
  ...
  try {
    ...
    return {
      sourcePath: OPENCLAW_CONFIG_PATH,
      channels: parseChannelConfigEntries(parsed),
    };
  } catch {
    return {
      sourcePath: OPENCLAW_CONFIG_PATH,
...
async function appendChannelConfig(input: AppendChannelConfigInput): Promise<{
  channelKey: string;
  channels: ChannelConfigEntry[];
}> {
  const raw = await safeReadTextFile(OPENCLAW_CONFIG_PATH);
  const root = raw ? (JSON.parse(raw) as Record<string, unknown>) : {};
  const channelsRoot = ensureObjectRecord(root, "channels");
  channelsRoot[input.channelKey] = sanitizeChannelConfigInput(input.config);
  const metaRoot = ensureObjectRecord(root, "meta");
  metaRoot.lastTouchedAt = new Date().toISOString();
  await writeFile(OPENCLAW_CONFIG_PATH, `${JSON.stringify(root, null, 2)}\n`, "utf8");
  return {
    channelKey: input.channelKey,
    channels: parseChannelConfigEntries(root),
  };
}

function parseChannelConfigEntries(parsed: Record<string, unknown>): ChannelConfigEntry[] {
  const channelsRoot = (parsed.channels ?? {}) as Record<string, unknown>;
  const rows: ChannelConfigEntry[] = [];
  for (const [channelKey, channelValue] of Object.entries(channelsRoot)) {
    if (!channelValue || typeof channelValue !== "object" || Array.isArray(channelValue)) continue;
    const channelObj = channelValue as Record<string, unknown>;
    const configKeys = Object.keys(channelObj).sort();
    const secretFields = configKeys.filter((key) => /secret|token|key|password/i.test(key));
    const enabledValue = channelObj.enabled;
      ...
      const valueText = typeof value === "string"
        ...
        : typeof value === "number" || typeof value === "boolean"
          ...
          : Array.isArray(value)
            ? `[${value.length}]`
            : "{...}";
      preview[key] = /secret|token|key|password/i.test(key) ? maskSecret(valueText) : valueText.slice(0, 120);
    }
    rows.push({
      channelKey,
      enabled,
      ...
      secretFields,
      preview,
      config: sanitizeChannelConfigInput(channelObj),
    });
  }
  rows.sort((a, b) => a.channelKey.localeCompare(b.channelKey, "zh-Hans-CN"));
  return rows;
}

function sanitizeChannelConfigInput(input: Record<string, unknown>): Record<string, unknown> {
...
const SKILLS_STATUS_CACHE_TTL_MS = 30_000;
let skillsStatusCache:
  | {
      snapshot: SkillsStatusSummary;
      fetchedAtMs: number;
    }
  | undefined;
let skillsStatusInflight: Promise<SkillsStatusSummary> | undefined;

function parseGatewayJsonPayload(raw: string): Record<string, unknown> {
  const text = `${raw ?? ""}`.trim();
  if (!text) throw new Error("Gateway returned empty payload.");
  const objectIndex = text.indexOf("{");
  if (objectIndex >= 0) {
    const objectText = text.slice(objectIndex);
    try {
...
function normalizeSkillStatusEntries(payload: Record<string, unknown>): SkillStatusEntry[] {
  const rows = Array.isArray(payload.skills) ? payload.skills : [];
  const output: SkillStatusEntry[] = [];
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const entry = row as Record<string, unknown>;
    const skillKey = `${entry.skillKey ?? entry.name ?? ""}`.trim();
    if (!skillKey) continue;
    const missingRaw = entry.missing && typeof entry.missing === "object" && !Array.isArray(entry.missing)
      ? (entry.missing as Record<string, unknown>)
      : {};
    output.push({
      skillKey,
      name: `${entry.name ?? skillKey}`.trim() || skillKey,
      description: `${entry.description ?? ""}`.trim(),
      source: `${entry.source ?? "unknown"}`.trim() || "unknown",
      ...
      missing: {
        bins: Array.isArray(missingRaw.bins) ? missingRaw.bins.map((item) => `${item ?? ""}`.trim()).filter((item) => item.length > 0) : [],
        env: Array.isArray(missingRaw.env) ? missingRaw.env.map((item) => `${item ?? ""}`.trim()).filter((item) => item.length > 0) : [],
        config: Array.isArray(missingRaw.config) ? missingRaw.config.map((item) => `${item ?? ""}`.trim()).filter((item) => item.length > 0) : [],
        os: Array.isArray(missingRaw.os) ? missingRaw.os.map((item) => `${item ?? ""}`.trim()).filter((item) => item.length > 0) : [],
      },
    });
  }
  output.sort((a, b) => a.skillKey.localeCompare(b.skillKey, "en"));
  return output;
}

function buildSkillsStatusSummary(payload: Record<string, unknown>): SkillsStatusSummary {
  const skills = normalizeSkillStatusEntries(payload);
  const bundledSkills = skills.filter((item) => item.bundled).length;
  const installedSkills = skills.length - bundledSkills;
  return {
...
function getSkillsStatusSummaryFromCache(maxAgeMs = SKILLS_STATUS_CACHE_TTL_MS): SkillsStatusSummary | undefined {
  if (!skillsStatusCache) return undefined;
  const ageMs = Date.now() - skillsStatusCache.fetchedAtMs;
  if (!Number.isFinite(ageMs) || ageMs < 0 || ageMs > maxAgeMs) return undefined;
  return skillsStatusCache.snapshot;
}

async function loadSkillsStatusSummary(options?: {
  forceRefresh?: boolean;
  timeoutMs?: number;
}): Promise<SkillsStatusSummary> {
  const forceRefresh = options?.forceRefresh === true;
  const timeoutMs = Math.max(3_000, options?.timeoutMs ?? 26_000);
  if (!forceRefresh) {
    const cached = getSkillsStatusSummaryFromCache();
    if (cached) return cached;
    if (skillsStatusInflight) return await skillsStatusInflight;
  }
  const gatewayTimeoutMs = Math.max(2_000, Math.min(20_000, timeoutMs - 1_000));
  const inflight = (async () => {
    const { stdout } = await runOpenClawGatewayCommand(
      ["call", "skills.status", "--json", "--timeout", String(gatewayTimeoutMs)],
      timeoutMs,
    );
    const payload = parseGatewayJsonPayload(stdout);
    const summary = buildSkillsStatusSummary(payload);
    skillsStatusCache = {
      snapshot: summary,
  ...
  skillsStatusInflight = inflight;
  try {
    return await inflight;
  } catch (error) {
    const fallback = getSkillsStatusSummaryFromCache(5 * 60_000);
    if (fallback) return fallback;
    throw error;
  } finally {
    if (skillsStatusInflight === inflight) skillsStatusInflight = undefined;
  }
}

async function installSkillBySource(source: string): Promise<SkillsStatusSummary> {
  await runOpenClawGatewayCommand(
    ["call", "skills.install", "--json", "--params", JSON.stringify({ source }), "--timeout", "45000"],
    52000,
  );
  return await loadSkillsStatusSummary({ forceRefresh: true });
}

async function updateSkillDisabledState(skillKey: string, disabled: boolean): Promise<SkillsStatusSummary> {
  await runOpenClawGatewayCommand(
    ["call", "skills.update", "--json", "--params", JSON.stringify({ skillKey, disabled }), "--timeout", "30000"],
    36000,
  );
  return await loadSkillsStatusSummary({ forceRefresh: true });
}

async function uninstallSkill(skillKey: string): Promise<SkillsStatusSummary> {
  await runOpenClawGatewayCommand(
    ["call", "skills.uninstall", "--json", "--params", JSON.stringify({ skillKey }), "--timeout", "30000"],
    36000,
  );
  return await loadSkillsStatusSummary({ forceRefresh: true });
}

async function loadChatChannelsSnapshot(
  snapshot: ReadModelSnapshot,
  usageCost: UsageCostSnapshot,
): Promise<ChatChannelsSnapshot> {
  const runtimeChannels = extractRuntimeConnectedChannels(usageCost);
  const runtimeSessions = runtimeChannels.reduce((sum, item) => sum + item.sessions, 0);
  const connectedFromRuntime = runtimeChannels.map((item) => item.label);
  const connectedFromSessionKeys = extractConnectedChannelsFromSessionKeys(snapshot);
  const connectedFromConfig = await extractConnectedChannelsFromOpenClawConfig();
  const merged = new Set<string>();
  for (const name of connectedFromRuntime) merged.add(name);
  for (const name of connectedFromSessionKeys) merged.add(name);
  for (const name of connectedFromConfig) merged.add(name);
  return {
    connectedChannels: [...merged].slice(0, 6),
    connectedSessions: runtimeSessions,
    sourcePath: OPENCLAW_CONFIG_PATH,
  };
}

function extractRuntimeConnectedChannels(usageCost: UsageCostSnapshot): Array<{ label: string; sessions: number }> {
  const rows = usageCost.breakdownToday.bySessionType.length > 0
    ? usageCost.breakdownToday.bySessionType
    : usageCost.breakdown.bySessionType;
  return rows
    .filter((row) => normalizeChannelLabel(row.label) !== "cron")
    .filter((row) => row.sessions > 0 || row.requests > 0)
    .map((row) => ({ label: row.label.trim(), sessions: row.sessions }));
}

function extractConnectedChannelsFromSessionKeys(snapshot: ReadModelSnapshot): string[] {
  ...
  for (const session of snapshot.sessions) {
    ...
    if (!channel) continue;
    if (normalizeChannelLabel(channel) === "cron") continue;
    result.add(channel);
  }
  return [...result];
}

async function extractConnectedChannelsFromOpenClawConfig(): Promise<string[]> {
  const raw = await safeReadTextFile(OPENCLAW_CONFIG_PATH);
  if (!raw) return [];
  ...
  const result = new Set<string>();
  if (Array.isArray(channelsRoot)) {
    for (const item of channelsRoot) {
      if (!item || typeof item !== "object") continue;
      const obj = item as Record<string, unknown>;
      const enabled = typeof obj.enabled === "boolean" ? obj.enabled : true;
      if (!enabled) continue;
      const name = pickChannelName(obj);
      if (!name) continue;
      if (normalizeChannelLabel(name) === "cron") continue;
      result.add(name);
    }
  } else if (channelsRoot && typeof channelsRoot === "object") {
    const obj = channelsRoot as Record<string, unknown>;
    for (const [key, value] of Object.entries(obj)) {
      if (key === "list" || key === "items") continue;
      if (value && typeof value === "object") {
        const item = value as Record<string, unknown>;
        const enabled = typeof item.enabled === "boolean" ? item.enabled : true;
        if (!enabled) continue;
        const name = pickChannelName(item) || key.trim();
        if (!name) continue;
        if (normalizeChannelLabel(name) === "cron") continue;
        result.add(name);
      }
    }
    const list = Array.isArray(obj.list) ? obj.list : Array.isArray(obj.items) ? obj.items : [];
    for (const item of list) {
      if (!item || typeof item !== "object") continue;
      const record = item as Record<string, unknown>;
      const enabled = typeof record.enabled === "boolean" ? record.enabled : true;
      if (!enabled) continue;
      const name = pickChannelName(record);
      if (!name) continue;
      if (normalizeChannelLabel(name) === "cron") continue;
      result.add(name);
    }
...
function pickChannelName(obj: Record<string, unknown>): string | undefined {
  const candidates = [obj.id, obj.name, obj.type, obj.channel, obj.provider];
...
function inferChannelFromSessionKey(sessionKey: string): string | undefined {
  ...
  if (key.includes(":slack:") || key.startsWith("slack:")) return "Slack";
  if (key.includes(":lark:") || key.startsWith("lark:")) return "Lark";
  if (key.includes(":feishu:") || key.startsWith("feishu:")) return "Feishu";
  if (key.includes(":wechat:") || key.startsWith("wechat:")) return "WeChat";
  if (key.includes(":dingtalk:") || key.startsWith("dingtalk:")) return "DingTalk";
...
async function sendMessageToSession(
  sessionKey: string,
  message: string,
): Promise<{ ok: boolean; output: string; summary: string; message?: string }> {
  const idempotencyKey = `cc-${Date.now().toString(36)}-${randomUUID().slice(0, 8)}`;
  try {
    ...
    const gatewayResult = await runCommandCapture(
      "openclaw",
      ["gateway", "call", "chat.send", "--json", "--timeout", "15000", "--params", gatewayPayload],
      20000,
      {
        OPENCLAW_CONFIG_PATH: OPENCLAW_CONFIG_PATH,
      },
    );
    const gatewayOutput = `${gatewayResult.stdout}\n${gatewayResult.stderr}`.trim();
    if (gatewayResult.ok) {
      return {
        ok: true,
  ...
  const routing = parseSessionRouting(sessionKey);
  if (!routing) {
    return {
      ok: false,
      output: "",
      summary: "",
      message: "Unsupported session key format for sending.",
    };
  }
  const result = await runCommandCapture(
    "openclaw",
    ["agent", "--agent", routing.agentId, "--session-id", routing.sessionId, "--message", message, "--timeout", "45", "--json"],
    50000,
    {
      OPENCLAW_CONFIG_PATH: OPENCLAW_CONFIG_PATH,
    },
  ...
  if (!result.ok) {
    return {
      ...
      output,
      summary: "",
      message: "OpenClaw agent command failed.",
    };
  }
  let summary = "";
  try {
    const parsed = JSON.parse(result.stdout) as Record<string, unknown>;
    const resultNode = (parsed.result ?? {}) as Record<string, unknown>;
    const payloads = Array.isArray(resultNode.payloads) ? resultNode.payloads : [];
    const first = payloads.length > 0 && payloads[0] && typeof payloads[0] === "object"
      ? (payloads[0] as Record<string, unknown>)
      : undefined;
    summary = typeof first?.text === "string" ? first.text.trim().slice(0, 400) : "";
  } catch {
    summary = "";
  }
  return {
...
function parseSessionRouting(sessionKey: string): { agentId: string; sessionId: string } | undefined {
...
interface OpenClawGatewayControlStatus {
  running: boolean;
  statusText: string;
  gateway: GatewayRuntimeSnapshot;
}

async function readOpenClawGatewayRuntimeStatus(): Promise<OpenClawGatewayControlStatus> {
  const gateway = await loadGatewayRuntimeSnapshot();
  const systemService = await readSystemGatewayServiceStatus();
  const fallback: OpenClawGatewayControlStatus = {
    running: systemService.known ? systemService.running : gateway.reachability === "connected",
    statusText:
      systemService.statusText || (gateway.reachability === "connected" ? "running" : "stopped"),
    gateway,
  };
  try {
    const { stdout } = await runOpenClawGatewayCommand(["status", "--json"], 8000);
    const parsed = JSON.parse(stdout) as Record<string, unknown>;
    const service = (parsed.service ?? {}) as Record<string, unknown>;
    const runtime = (service.runtime ?? {}) as Record<string, unknown>;
    const rawStatus = `${runtime.status ?? service.status ?? ""}`.trim().toLowerCase();
    const rpc = (parsed.rpc ?? {}) as Record<string, unknown>;
    const port = (parsed.port ?? {}) as Record<string, unknown>;
    const portStatus = `${port.status ?? ""}`.trim().toLowerCase();
    const listeners = Array.isArray(port.listeners) ? port.listeners : [];
    const rpcOk = rpc.ok === true;
    const portBusy = portStatus === "busy" || listeners.length > 0;
    const runningByStatus = rawStatus.includes("running") || rawStatus === "active";
    const running =
      runningByStatus || rpcOk || portBusy || gateway.reachability === "connected" || systemService.running;
    return {
      running,
      statusText: rawStatus || systemService.statusText || (running ? "running" : "stopped"),
      gateway,
    };
  } catch {
    return fallback;
...
async function executeOpenClawGatewayAction(
  action: "start" | "stop",
): Promise<{
  ok: boolean;
  action: "start" | "stop";
  status: OpenClawGatewayControlStatus;
  message: string;
  output: string;
}> {
  const commandResult = await runGatewayControlAction(action);
  const status = await readOpenClawGatewayRuntimeStatus();
  const expectedRunning = action === "start";
  const stateMatched = status.running === expectedRunning;
  const ok = commandResult.ok && stateMatched;
  const fallbackMessage = action === "start" ? "OpenClaw gateway started." : "OpenClaw gateway stopped.";
  const mismatchMessage = action === "start" ? "Gateway did not enter running state." : "Gateway is still running.";
  return {
    ok,
    action,
    status,
    message: ok ? fallbackMessage : commandResult.message || mismatchMessage,
    output: commandResult.output,
  };
}

async function runOpenClawGatewayCommand(args: string[], timeoutMs: number): Promise<{ stdout: string; stderr: string }> {
  const result = await runCommandCapture("openclaw", ["gateway", ...args], timeoutMs);
  if (!result.ok) {
    throw new Error(result.stderr || result.stdout || "openclaw gateway command failed.");
  }
  return {
...
async function readSystemGatewayServiceStatus(): Promise<{ known: boolean; running: boolean; statusText: string }> {
  const result = await runCommandCapture("systemctl", ["is-active", "openclaw-gateway.service"], 5000);
  const text = `${result.stdout}\n${result.stderr}`
    .split(/\r?\n/)
    .map((line) => line.trim().toLowerCase())
    .find((line) => line.length > 0);
  if (!text) return { known: false, running: false, statusText: "" };
  const running = text === "active" || text === "activating" || text === "reloading";
  return { known: true, running, statusText: text };
}

async function runGatewayControlAction(action: "start" | "stop"): Promise<{ ok: boolean; output: string; message: string }> {
  const attempts = [
    { command: "sudo", args: ["-n", "systemctl", action, "openclaw-gateway.service"] },
    { command: "systemctl", args: [action, "openclaw-gateway.service"] },
    { command: "openclaw", args: ["gateway", action] },
  ];
  const outputs: string[] = [];
  for (const attempt of attempts) {
    const result = await runCommandCapture(attempt.command, attempt.args, 15000);
    const output = `${result.stdout}\n${result.stderr}`.trim();
    if (output) outputs.push(`${attempt.command} ${attempt.args.join(" ")}\n${output}`);
    if (result.ok) {
      return {
  ...
  return {
    ok: false,
    output: outputs.join("\n\n").trim(),
    message: "Gateway control command failed.",
  };
}
...
async function runCommandCapture(
  ...
  // Some environments start services without a stable cwd; choose the first
  // valid directory to prevent uv_cwd failures in child processes.
  const commandCwdCandidates = [
    OPENCLAW_WORKSPACE_ROOT,
    join(OPENCLAW_HOME_DIR, "workspace-main"),
    homedir(),
    "/tmp",
  ];
  let commandCwd: string | undefined;
  for (const candidate of commandCwdCandidates) {
    try {
...
async function loadGatewayRuntimeSnapshot(): Promise<GatewayRuntimeSnapshot> {
  const endpoint = GATEWAY_URL.trim() || "ws://127.0.0.1:18789";
  let parsedUrl: URL;
  try {
  ...
  const protocolLabel = protocol.replace(":", "") || "unknown";
  ...
  const tcpProbe = await probeTcpReachability(host, port, 1200);
  const reachability: GatewayRuntimeSnapshot["reachability"] = tcpProbe.reachable ? "connected" : "not_connected";
  if (protocol !== "wss:") {
    return {
      endpoint,
      protocol: protocolLabel,
  ...
  const certProbe = await probeTlsCertificate(host, port, 1600);
  return {
    endpoint,
    protocol: protocolLabel,
    reachability,
    latencyMs: tcpProbe.latencyMs,
    certStatus: certProbe.status,
    certExpiryLabel: certProbe.expiryLabel,
  };
}
...
async function probeTcpReachability(
  ...
  return await new Promise<{ reachable: boolean; latencyMs?: number }>((resolveProbe) => {
    const startedAt = Date.now();
    const socket = connectTcp({ host, port });
    let settled = false;
    const resolveOnce = (value: { reachable: boolean; latencyMs?: number }): void => {
      if (settled) return;
      settled = true;
      socket.destroy();
      resolveProbe(value);
    ...
    socket.once("connect", () => resolveOnce({ reachable: true, latencyMs: Math.max(0, Date.now() - startedAt) }));
    socket.once("timeout", () => resolveOnce({ reachable: false }));
    socket.once("error", () => resolveOnce({ reachable: false }));
  });
}

async function probeTlsCertificate(
  host: string,
  port: number,
  timeoutMs: number,
): Promise<{ status: GatewayRuntimeSnapshot["certStatus"]; expiryLabel?: string }> {
  return await new Promise((resolveProbe) => {
    ...
    let settled = false;
    const resolveOnce = (value: { status: GatewayRuntimeSnapshot["certStatus"]; expiryLabel?: string }): void => {
      if (settled) return;
      settled = true;
      socket.destroy();
      resolveProbe(value);
    ...
    socket.once("secureConnect", () => {
      ...
      if (!certificate || !certificate.valid_to) {
        resolveOnce({ status: "unknown" });
        return;
      }
      const expiryAt = Date.parse(certificate.valid_to);
      if (!Number.isFinite(expiryAt)) {
        resolveOnce({ status: "unknown" });
        return;
      }
      const expiryLabel = `exp ${formatDateYYYYMMDD(expiryAt)}`;
      resolveOnce({ status: expiryAt > Date.now() ? "ok" : "invalid", expiryLabel });
    });
    socket.once("timeout", () => resolveOnce({ status: "unknown" }));
    socket.once("error", () => resolveOnce({ status: "unknown" }));
  });
}
...
function renderAdminAuthBootstrapScript(language: UiLanguage = "zh"): string {
  const passwordPrompt = escapeHtml(
    pickUiText(language, "Enter local admin token", "请输入本地管理令牌"),
  );
  const authFailed = escapeHtml(
    pickUiText(language, "Local admin token verification failed", "本地管理令牌验证失败"),
  );
  const submitLabel = escapeHtml(pickUiText(language, "Confirm", "确认"));
  const cancelLabel = escapeHtml(pickUiText(language, "Cancel", "取消"));
  return `<script>
(() => {
  const authRequired = ${LOCAL_TOKEN_AUTH_REQUIRED ? "true" : "false"};
  if (!authRequired) return;
  const passwordPrompt = ${JSON.stringify(passwordPrompt)};
  const authFailed = ${JSON.stringify(authFailed)};
  const submitLabel = ${JSON.stringify(submitLabel)};
  const cancelLabel = ${JSON.stringify(cancelLabel)};
  ...
  const askPassword = async () => {
    ...
    title.textContent = passwordPrompt;
    title.style.fontSize = '16px';
    title.style.color = '#eef1f3';
    ...
    input.style.background = '#0f1114';
    input.style.color = '#eef1f3';
    ...
    cancel.style.background = '#23262b';
    cancel.style.color = '#d9dde1';
    ...
    submit.style.background = '#00df7f';
    submit.style.color = '#0c1a10';
  ...
  const ensureAuth = async () => {
    const session = await request('/api/admin/session', 'GET');
    if (!session.response.ok) return false;
    if (session.data && session.data.authenticated) return true;
    const password = await askPassword();
    if (typeof password !== 'string' || password.trim() === '') return false;
    const login = await request('/api/admin/login', 'POST', { password: password.trim() });
    if (!login.response.ok || !(login.data && login.data.ok)) {
      const message = typeof login.data?.message === 'string' ? login.data.message : authFailed;
  ...
  window.__openclawEnsureAdminAuth = ensureAuth;
  void ensureAuth();
})();
</script>`;
}

function renderGatewayControlScript(language: UiLanguage = "zh"): string {
  const startLabel = escapeHtml(pickUiText(language, "Start OpenClaw gateway", "启动 OpenClaw 网关"));
  const stopLabel = escapeHtml(pickUiText(language, "Stop OpenClaw gateway", "停止 OpenClaw 网关"));
  const failMessage = escapeHtml(pickUiText(language, "Gateway control failed", "网关控制失败"));
  return `<script>
(() => {
  const button = document.getElementById('wf-gateway-toggle');
  if (!(button instanceof HTMLButtonElement)) return;
  const icon = button.querySelector('i');
  if (!(icon instanceof HTMLElement)) return;
  const startLabel = ${JSON.stringify(startLabel)};
  const stopLabel = ${JSON.stringify(stopLabel)};
  const failMessage = ${JSON.stringify(failMessage)};
  let running = button.dataset.running === 'true';
  const setVisualState = (nextRunning, busy) => {
    running = nextRunning;
    button.dataset.running = nextRunning ? 'true' : 'false';
    button.disabled = Boolean(busy);
    button.classList.toggle('wf-mode-red', nextRunning);
    button.classList.toggle('wf-mode-green', !nextRunning);
    icon.classList.toggle('fa-stop', nextRunning);
    icon.classList.toggle('fa-play', !nextRunning);
    button.setAttribute('aria-label', nextRunning ? stopLabel : startLabel);
    button.setAttribute('title', nextRunning ? stopLabel : startLabel);
  };
  ...
  const refreshStatus = async () => {
    try {
      const { response, payload } = await request('/api/openclaw/gateway/status', 'GET');
      if (!response.ok || !payload || typeof payload !== 'object') return;
      const status = payload.status;
      if (!status || typeof status !== 'object') return;
      setVisualState(Boolean(status.running), false);
    } catch {
      setVisualState(running, false);
    }
  };
  const runToggle = async (action) => {
    setVisualState(running, true);
    try {
      const result = await request('/api/openclaw/gateway/toggle', 'POST', { action });
      if (!result.response.ok) {
        const message = typeof result.payload?.message === 'string' ? result.payload.message : failMessage;
        window.alert(message);
        await refreshStatus();
        return;
      }
      const nextRunning = Boolean(result.payload?.running);
      setVisualState(nextRunning, false);
      await refreshStatus();
    } catch {
...
function renderChatWorkspaceScript(language: UiLanguage = "zh"): string {
  const titleFallback = escapeHtml(pickUiText(language, "Untitled session", "未命名会话"));
  const loadingSessions = escapeHtml(pickUiText(language, "Loading sessions...", "正在加载会话..."));
  const loadFailed = escapeHtml(pickUiText(language, "Failed to load sessions", "加载会话失败"));
  const loadTimeout = escapeHtml(pickUiText(language, "Request timed out", "请求超时"));
  const noSessions = escapeHtml(pickUiText(language, "No session yet.", "暂无会话。"));
  const selectTip = escapeHtml(pickUiText(language, "Select a session", "请选择会话"));
  const noActiveMeta = escapeHtml(pickUiText(language, "No active session", "暂无激活会话"));
  const chooseLeftTip = escapeHtml(
    pickUiText(language, "Choose a session on the left to view messages.", "从左侧选择会话以查看对话内容。"),
  );
  const sendingNeedSession = escapeHtml(
    pickUiText(language, "Select a real session before sending.", "请先选择一个真实会话后再发送。"),
  );
  const sendingFailed = escapeHtml(
    pickUiText(language, "Failed to send message", "发送消息失败"),
  );
  const sendingInProgress = escapeHtml(
    pickUiText(language, "Sending...", "发送中..."),
  );
  const newSessionText = escapeHtml(pickUiText(language, "New session", "新建会话"));
  const draftTitle = escapeHtml(pickUiText(language, "New draft session", "新建草稿会话"));
  const draftMsg = escapeHtml(
    pickUiText(
      language,
      "This draft is local only for now. You can browse existing OpenClaw sessions on the left.",
      "该草稿当前仅本地展示。你可以在左侧浏览已接入的 OpenClaw 会话。",
    ),
  );
  const roleLabels = {
    user: escapeHtml(pickUiText(language, "User", "用户")),
    assistant: escapeHtml(pickUiText(language, "Assistant", "助手")),
    system: escapeHtml(pickUiText(language, "System", "系统")),
  };
  return `<script>
(() => {
  ...
  const listNode = root.querySelector('[data-chat-session-list]');
  const titleNode = root.querySelector('[data-chat-current-title]');
  const metaNode = root.querySelector('[data-chat-current-meta]');
  const messageNode = root.querySelector('[data-chat-message-list]');
  const newButton = root.querySelector('[data-chat-new-session]');
  const inputForm = root.querySelector('[data-chat-input-form]');
  const inputNode = root.querySelector('[data-chat-input]');
  const submitNode = inputForm instanceof HTMLFormElement ? inputForm.querySelector('button[type="submit"]') : null;
  const language = (root.getAttribute('data-language') || 'zh').trim().toLowerCase() === 'en' ? 'en' : 'zh';
  if (!(listNode instanceof HTMLElement) || !(titleNode instanceof HTMLElement) || !(metaNode instanceof HTMLElement) || !(messageNode instanceof HTMLElement)) return;

  const l = {
    titleFallback: ${JSON.stringify(titleFallback)},
    loadingSessions: ${JSON.stringify(loadingSessions)},
    loadFailed: ${JSON.stringify(loadFailed)},
    loadTimeout: ${JSON.stringify(loadTimeout)},
    noSessions: ${JSON.stringify(noSessions)},
    selectTip: ${JSON.stringify(selectTip)},
    noActiveMeta: ${JSON.stringify(noActiveMeta)},
    chooseLeftTip: ${JSON.stringify(chooseLeftTip)},
    sendingNeedSession: ${JSON.stringify(sendingNeedSession)},
    sendingFailed: ${JSON.stringify(sendingFailed)},
    sendingInProgress: ${JSON.stringify(sendingInProgress)},
    newSessionText: ${JSON.stringify(newSessionText)},
    draftTitle: ${JSON.stringify(draftTitle)},
    draftMsg: ${JSON.stringify(draftMsg)},
    roleUser: ${JSON.stringify(roleLabels.user)},
    roleAssistant: ${JSON.stringify(roleLabels.assistant)},
    roleSystem: ${JSON.stringify(roleLabels.system)},
  };
  ...
  const roleLabel = (value) => {
    ...
    if (key === 'assistant') return l.roleAssistant;
    if (key === 'system') return l.roleSystem;
    return l.roleUser;
  };

  const fetchJson = async (url, init, timeoutMs = 12000) => {
  ...
  const summarize = (entry) => {
    const text = String(entry?.latestSnippet || entry?.summary || '').trim();
    if (!text) return language === 'en' ? 'No message yet' : '暂无消息';
  ...
  const sanitizeMessageContent = (input) => {
    ...
    const senderIndex = text.indexOf(senderPrefix);
    if (senderIndex >= 0) {
      const firstFence = text.indexOf(fence, senderIndex);
      const secondFence = firstFence >= 0 ? text.indexOf(fence, firstFence + fence.length) : -1;
      if (firstFence >= 0 && secondFence >= 0) {
        text = text.slice(0, senderIndex) + text.slice(secondFence + fence.length);
      }
    }
    ...
    text = text.split(nl).map((line) => {
      ...
      const inside = trimmed.slice(1, right);
      if (inside.includes('GMT') || inside.includes('-') || inside.includes(':')) {
        return trimmed.slice(right + 1).trimStart();
      }
      return line;
    }).join(nl);
    text = text.split(nl).map((line) => String(line || '').replace(String.fromCharCode(13), '').trimEnd()).join(nl).trim();
    return text;
  };
  ...
  const historyDigest = (history) => {
    ...
    const stamp = String(last.timestamp || last.updatedAt || last.createdAt || '');
    const content = sanitizeMessageContent(String(last.content || last.text || '')).slice(0, 64);
    return String(history.length) + ':' + stamp + ':' + content;
  };

  const renderEmptyMessages = (text) => {
    messageNode.innerHTML = '';
    const empty = document.createElement('div');
    const isTimeout = String(text || '').includes(l.loadTimeout);
    empty.className = 'empty-state' + (isTimeout ? ' timeout-tip' : '');
    empty.textContent = text;
    messageNode.append(empty);
  };

  const renderMessages = (history) => {
    messageNode.innerHTML = '';
    if (!Array.isArray(history) || history.length === 0) {
      renderEmptyMessages(language === 'en' ? 'No message yet' : '暂无消息');
      return;
    }
    history.forEach((item) => {
      const roleKey = String(item?.role || item?.kind || 'user').trim().toLowerCase();
      const wrap = document.createElement('article');
      wrap.className = 'chat-message-item';
      if (roleKey === 'assistant') wrap.classList.add('role-assistant');
      else if (roleKey === 'system') wrap.classList.add('role-system');
      else wrap.classList.add('role-user');
      const body = document.createElement('div');
      body.className = 'chat-message-body';
      body.textContent = sanitizeMessageContent(item?.content || item?.text || '');
      wrap.append(body);
      messageNode.append(wrap);
    });
  };

  const draftByKey = (key) => state.drafts.find((item) => item.key === key);
  const sessionByKey = (key) => state.sessions.find((item) => item.sessionKey === key);
  const selectedSession = () => sessionByKey(state.selectedKey);
  const selectedIsDraft = () => Boolean(draftByKey(state.selectedKey));
  let liveSyncTimer = 0;

  const loadSessionHistory = async (key, timeoutMs = 20000, historyLimit = 60) => {
    const { response, payload } = await fetchJson(
      '/api/sessions/' + encodeURIComponent(key) + '?historyLimit=' + encodeURIComponent(String(historyLimit)),
      undefined,
      timeoutMs,
    );
    if (!response.ok || !payload || payload.ok === false) {
      throw new Error(String(payload?.error?.message || l.loadFailed));
    }
    return Array.isArray(payload?.history)
      ? payload.history
      : Array.isArray(payload?.conversation?.history)
        ? payload.conversation.history
        : [];
  };

  const refreshSelectedSessionLive = async () => {
    const session = selectedSession();
    if (!session || selectedIsDraft() || state.loadingDetail) return;
    try {
      const history = await loadSessionHistory(session.sessionKey, 16000, 60);
      const digest = historyDigest(history);
      if (digest !== state.currentHistoryDigest) {
        state.currentHistoryDigest = digest;
        metaNode.textContent = language === 'en'
          ? 'Messages: ' + String(history.length)
          : '消息数：' + String(history.length);
  ...
  const ensureLiveSync = () => {
    ...
    liveSyncTimer = window.setInterval(() => {
      void refreshSelectedSessionLive();
    }, 2200);
  };

  const updateInputState = () => {
    if (!(inputNode instanceof HTMLTextAreaElement) || !(submitNode instanceof HTMLButtonElement)) return;
    const canSend = Boolean(selectedSession()) && !state.loadingDetail && !state.sending;
    inputNode.disabled = !canSend;
    submitNode.disabled = !canSend;
    if (!canSend && selectedIsDraft()) {
      inputNode.placeholder = language === 'en'
        ? 'Draft session cannot send to OpenClaw yet'
        : '草稿会话暂不支持发送到 OpenClaw';
    } else if (!canSend) {
      inputNode.placeholder = language === 'en' ? 'Select a session...' : '请先选择会话...';
    } else {
      inputNode.placeholder = language === 'en' ? 'Write message...' : '输入消息...';
    }
  };
  ...
  const renderSessionList = () => {
    listNode.innerHTML = '';
    const allItems = [
      ...state.drafts.map((item) => ({ ...item, _isDraft: true })),
      ...state.sessions.map((item) => ({ ...item, _isDraft: false })),
    ];
    if (allItems.length === 0) {
      const empty = document.createElement('div');
      empty.className = 'empty-state';
      empty.textContent = l.noSessions;
    ...
    allItems.forEach((entry) => {
      ...
      const key = entry._isDraft ? entry.key : entry.sessionKey;
      if (key === state.selectedKey) button.classList.add('active');
      button.dataset.sessionKey = key;
      const head = document.createElement('div');
      head.className = 'chat-session-item-head';
      const title = document.createElement('strong');
      title.textContent = String(entry.title || entry.displayTitle || l.titleFallback);
      const stamp = document.createElement('span');
      stamp.className = 'meta';
      stamp.textContent = formatDate(entry.updatedAt || entry.lastUpdatedAt);
      head.append(title, stamp);
      const snippet = document.createElement('div');
      snippet.className = 'chat-session-item-snippet';
      snippet.textContent = entry._isDraft ? l.draftMsg : summarize(entry);
  ...
  const selectSession = async (key) => {
    ...
    if (draft) {
      titleNode.textContent = draft.title;
      metaNode.textContent = language === 'en' ? 'Draft session' : '草稿会话';
      renderMessages(draft.history);
      return;
    }
    const session = sessionByKey(key);
    if (!session) {
      titleNode.textContent = l.selectTip;
      metaNode.textContent = l.noActiveMeta;
      renderEmptyMessages(l.chooseLeftTip);
      return;
    }
    titleNode.textContent = String(session.displayTitle || session.title || l.titleFallback);
    metaNode.textContent = language === 'en'
      ? 'Loading conversation...'
      : '正在加载对话...';
    state.loadingDetail = true;
    try {
      const history = await loadSessionHistory(key, 45000, 60);
      state.currentHistoryDigest = historyDigest(history);
      metaNode.textContent = language === 'en'
        ? 'Messages: ' + String(history.length)
        : '消息数：' + String(history.length);
      renderMessages(history);
      ...
      metaNode.textContent = language === 'en' ? 'Load failed' : '加载失败';
      renderEmptyMessages(error instanceof Error ? error.message : l.loadFailed);
    } finally {
  ...
  const loadSessions = async () => {
    listNode.innerHTML = '<div class="empty-state">' + l.loadingSessions + '</div>';
    try {
      const { response, payload } = await fetchJson('/api/sessions?page=1&pageSize=20&historyLimit=0', undefined, 12000);
      if (!response.ok || !payload || payload.ok === false) {
        throw new Error(String(payload?.error?.message || l.loadFailed));
      }
      state.sessions = Array.isArray(payload.items) ? payload.items : [];
      renderSessionList();
      if (!state.selectedKey && state.sessions.length > 0) {
        await selectSession(String(state.sessions[0].sessionKey || ''));
      ...
      titleNode.textContent = l.selectTip;
      metaNode.textContent = language === 'en' ? 'Load failed' : '加载失败';
      const errorText = error instanceof Error ? error.message : l.loadFailed;
      const timeoutClass = String(errorText || '').includes(l.loadTimeout) ? ' timeout-tip' : '';
      listNode.innerHTML = '<div class="empty-state' + timeoutClass + '">' + errorText + '</div>';
      renderEmptyMessages(errorText);
      updateInputState();
    }
  };
  ...
  if (newButton instanceof HTMLButtonElement) {
    ...
    if (newButton.classList.contains('chat-add-session-btn')) {
      newButton.textContent = '+';
      newButton.setAttribute('aria-label', l.newSessionText);
      newButton.title = l.newSessionText;
    } else {
      newButton.textContent = l.newSessionText;
    }
  }
  ...
  if (inputForm instanceof HTMLFormElement) {
    inputForm.addEventListener('submit', async (event) => {
      ...
      const session = selectedSession();
      if (!session || selectedIsDraft()) {
        window.alert(l.sendingNeedSession);
        return;
      }
      ...
      state.sending = true;
      updateInputState();
      const oldMeta = metaNode.textContent || '';
      metaNode.textContent = l.sendingInProgress;
      try {
        if (typeof window.__openclawEnsureAdminAuth === 'function') {
          const authed = await window.__openclawEnsureAdminAuth();
          if (!authed) {
            throw new Error(l.sendingFailed);
          }
        }
        const { response, payload } = await fetchJson('/api/sessions/' + encodeURIComponent(session.sessionKey) + '/messages', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
        ...
        if (!response.ok || !payload?.ok) {
          const messageText = typeof payload?.message === 'string' ? payload.message : l.sendingFailed;
          throw new Error(messageText);
        }
        inputNode.value = '';
        await selectSession(session.sessionKey);
        window.setTimeout(() => {
          void refreshSelectedSessionLive();
        }, 1200);
        window.setTimeout(() => {
        ...
        metaNode.textContent = oldMeta;
        window.alert(error instanceof Error ? error.message : l.sendingFailed);
      } finally {
        state.sending = false;
        updateInputState();
      }
...
function renderGatewaySettingsScript(language: UiLanguage = "zh"): string {
  const loadFailed = escapeHtml(pickUiText(language, "Failed to load gateway settings", "读取网关设置失败"));
  const saveFailed = escapeHtml(pickUiText(language, "Failed to save gateway settings", "保存网关设置失败"));
  const savedOk = escapeHtml(pickUiText(language, "Gateway settings saved", "网关设置已保存"));
  const testing = escapeHtml(pickUiText(language, "Testing connection...", "正在测试连接..."));
  const testOk = escapeHtml(pickUiText(language, "Connection successful", "连接成功"));
  const testFail = escapeHtml(pickUiText(language, "Connection failed", "连接失败"));
  const detecting = escapeHtml(pickUiText(language, "Detecting...", "正在自动检测..."));
  const detectFail = escapeHtml(pickUiText(language, "Auto detect failed", "自动检测失败"));
  const restartOk = escapeHtml(pickUiText(language, "Gateway restarted", "网关已重启"));
  const restartFail = escapeHtml(pickUiText(language, "Gateway restart failed", "网关重启失败"));
  const requiredGatewayUrl = escapeHtml(pickUiText(language, "Gateway URL is required", "请填写网关地址"));
  return `<script>
(() => {
  const root = document.querySelector('[data-gateway-config-root]');
  if (!(root instanceof HTMLElement)) return;
  const language = (root.getAttribute('data-language') || 'zh').trim().toLowerCase() === 'en' ? 'en' : 'zh';
  const sourceNode = root.querySelector('[data-gateway-source]');
  const statusNode = root.querySelector('[data-gateway-status]');
  const formNode = root.querySelector('[data-gateway-form]');
  const detectBtn = root.querySelector('[data-gateway-detect]');
  const maxPermissionBtn = root.querySelector('[data-gateway-max-permission]');
  const maxPermissionLabel = root.querySelector('[data-gateway-max-permission-label]');
  const hostInput = root.querySelector('[data-gateway-host-input]');
  const hostAddBtn = root.querySelector('[data-gateway-host-add]');
  const hostList = root.querySelector('[data-gateway-host-list]');
  const testBtn = root.querySelector('[data-gateway-test]');
  const restartBtn = root.querySelector('[data-gateway-restart]');
  const saveBtn = root.querySelector('[data-gateway-save]');
  if (!(sourceNode instanceof HTMLElement) || !(statusNode instanceof HTMLElement) || !(formNode instanceof HTMLFormElement) || !(detectBtn instanceof HTMLElement) || !(maxPermissionBtn instanceof HTMLElement) || !(maxPermissionLabel instanceof HTMLElement) || !(hostInput instanceof HTMLInputElement) || !(hostAddBtn instanceof HTMLElement) || !(hostList instanceof HTMLElement) || !(testBtn instanceof HTMLElement) || !(restartBtn instanceof HTMLElement) || !(saveBtn instanceof HTMLElement)) return;

  const l = {
    loadFailed: ${JSON.stringify(loadFailed)},
    saveFailed: ${JSON.stringify(saveFailed)},
    savedOk: ${JSON.stringify(savedOk)},
    testing: ${JSON.stringify(testing)},
    testOk: ${JSON.stringify(testOk)},
    testFail: ${JSON.stringify(testFail)},
    detecting: ${JSON.stringify(detecting)},
    detectFail: ${JSON.stringify(detectFail)},
    restartOk: ${JSON.stringify(restartOk)},
    restartFail: ${JSON.stringify(restartFail)},
    requiredGatewayUrl: ${JSON.stringify(requiredGatewayUrl)},
    enabled: language === 'en' ? 'Enabled' : '开启',
    disabled: language === 'en' ? 'Disabled' : '关闭',
    add: language === 'en' ? 'Add' : '添加',
    update: language === 'en' ? 'Update' : '更新',
    edit: language === 'en' ? 'Edit' : '编辑',
    remove: language === 'en' ? 'Delete' : '删除',
    emptyHosts: language === 'en' ? 'No allowed domain yet.' : '暂无已配置域名。',
    source: language === 'en' ? 'Config file' : '配置文件',
    saving: language === 'en' ? 'Saving...' : '保存中...',
    restarting: language === 'en' ? 'Restarting...' : '重启中...',
  };

  const apiPrefix = '/api';
  const directApiPrefix = '/api';
  let allowedHosts = [];
  let editingHost = '';
  let maxPermissions = false;

  const setStatus = (text) => {
    statusNode.textContent = text || '';
  };

  const setMaxPermission = (enabled) => {
    maxPermissions = Boolean(enabled);
    maxPermissionBtn.classList.toggle('active', maxPermissions);
    maxPermissionLabel.textContent = maxPermissions ? l.enabled : l.disabled;
  };

  const fetchJson = async (url, init, timeoutMs = 15000) => {
    const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
    const timer = controller ? window.setTimeout(() => controller.abort(), timeoutMs) : 0;
    try {
      const response = await fetch(url, { ...init, signal: controller ? controller.signal : undefined, credentials: 'same-origin' });
      let payload = {};
      try {
        payload = await response.json();
      } catch {
  ...
  const fetchWithFallback = async (endpoint, init, timeoutMs = 15000) => {
    const urls = [apiPrefix + endpoint, directApiPrefix + endpoint];
    let lastResult = null;
    let lastError = null;
    for (const url of urls) {
      try {
        const result = await fetchJson(url, init, timeoutMs);
  ...
  const updatePublicGatewayUrl = () => {
    const gatewayUrl = getFormValue('gatewayUrl');
    const publicInput = formNode.querySelector('[name="publicGatewayUrl"]');
    if (!(publicInput instanceof HTMLInputElement)) return;
    const host = window.location.hostname || '';
    if (!gatewayUrl || !host) {
      publicInput.value = window.location.origin;
      return;
    }
    let parsedPort = '';
    try {
    ...
    publicInput.value = parsedPort ? protocol + '://' + host + ':' + parsedPort : window.location.origin;
  };

  const mergeDisplayHosts = (hosts) => {
    const base = Array.isArray(hosts) ? hosts : [];
    const currentHost = (window.location.hostname || '').trim().toLowerCase();
    const currentHostWithPort = (window.location.host || '').trim().toLowerCase();
    const unique = new Set(
      base
        .map((item) => String(item || '').trim().toLowerCase())
        .filter((item) => item.length > 0)
    );
    if (currentHost) unique.add(currentHost);
    if (currentHostWithPort) unique.add(currentHostWithPort);
    return Array.from(unique);
  };

  const renderHosts = () => {
    hostList.innerHTML = '';
    const displayHosts = mergeDisplayHosts(allowedHosts);
    if (!Array.isArray(displayHosts) || displayHosts.length === 0) {
      hostList.innerHTML = '<div class="empty-state">' + l.emptyHosts + '</div>';
      return;
    }
    displayHosts.forEach((host) => {
      const editable = allowedHosts.includes(host);
      const row = document.createElement('div');
      row.className = 'gateway-host-item';
      row.innerHTML = '<span>' + String(host) + '</span><div class="gateway-host-actions">' + (editable ? ('<button class="btn wf-cancel-btn" type="button" data-host-edit="' + String(host) + '">' + l.edit + '</button><button class="btn wf-confirm-btn" type="button" data-host-delete="' + String(host) + '">' + l.remove + '</button>') : '') + '</div>';
      hostList.append(row);
    });
  };

  const setHostEditorState = (host) => {
    editingHost = host || '';
    hostAddBtn.textContent = editingHost ? l.update : l.add;
    if (!editingHost) {
      hostInput.value = '';
      return;
    }
    hostInput.value = editingHost;
    hostInput.focus();
  };

  const saveConfig = async (partial, successMessage) => {
    try {
      const { response, payload } = await fetchJson(apiPrefix + '/config', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(partial),
      }, 25000);
      if (!response.ok || (payload && payload.success === false)) {
        throw new Error(String(payload?.message || l.saveFailed));
      }
      setStatus(successMessage || l.savedOk);
      return true;
    } catch (error) {
  ...
  const reload = async () => {
    setStatus('');
    sourceNode.textContent = language === 'en' ? 'Loading...' : '加载中...';
    try {
      const [configResult, maxPermissionResult] = await Promise.all([
        fetchJson(apiPrefix + '/config', undefined, 20000),
        fetchJson(apiPrefix + '/config/max-permissions', undefined, 20000),
      ]);
      if (!configResult.response.ok) {
        throw new Error(String(configResult.payload?.message || l.loadFailed));
      }
      const config = configResult.payload || {};
      setFormValue('gatewayUrl', config.gatewayUrl || '');
      setFormValue('token', config.token || '');
      setFormValue('password', config.password || '');
      setFormValue('openclawWorkspace', config.openclawWorkspace || '');
      allowedHosts = Array.isArray(config.allowedHosts) ? config.allowedHosts.filter((item) => typeof item === 'string' && item.trim().length > 0) : [];
      renderHosts();
      setHostEditorState('');
      setMaxPermission(Boolean(maxPermissionResult.payload?.enabled));
      sourceNode.textContent = l.source + '：app_config';
      updatePublicGatewayUrl();
    } catch (error) {
      sourceNode.textContent = error instanceof Error ? error.message : l.loadFailed;
      setStatus(l.loadFailed);
    }
  };
  ...
  detectBtn.addEventListener('click', async () => {
    ...
    try {
      const { response, payload } = await fetchJson(apiPrefix + '/config/detect-all', undefined, 25000);
      if (!response.ok || !payload?.success || !payload?.data) {
        throw new Error(String(payload?.message || l.detectFail));
      }
      const detectedGatewayUrl = String(payload.data.gatewayUrl || '').trim();
      const detectedPort = String(payload.data.port || payload.data.gatewayPort || '').trim();
      const detectedToken = String(payload.data.token || payload.data.gateToken || payload.data.gatewayToken || '').trim();
      if (detectedGatewayUrl) {
        setFormValue('gatewayUrl', detectedGatewayUrl);
      } else if (detectedPort) {
        setFormValue('gatewayUrl', 'ws://127.0.0.1:' + detectedPort);
      }
      if (detectedToken) setFormValue('token', detectedToken);
      if (payload.data.password) setFormValue('password', payload.data.password);
      if (payload.data.workspacePath) setFormValue('openclawWorkspace', payload.data.workspacePath);
      {
        const fallback = await fetchJson(apiPrefix + '/config', undefined, 12000);
        if (fallback.response.ok && fallback.payload) {
          if (!getFormValue('gatewayUrl') && fallback.payload.gatewayUrl) setFormValue('gatewayUrl', String(fallback.payload.gatewayUrl));
          if (!getFormValue('token') && fallback.payload.token) setFormValue('token', String(fallback.payload.token));
          allowedHosts = Array.isArray(fallback.payload.allowedHosts) ? fallback.payload.allowedHosts.filter((item) => typeof item === 'string' && item.trim().length > 0) : allowedHosts;
          renderHosts();
        }
      }
      updatePublicGatewayUrl();
      ...
      setStatus(error instanceof Error ? error.message : l.detectFail);
    }
  });

  maxPermissionBtn.addEventListener('click', async () => {
    setStatus(l.saving);
    try {
      const { response, payload } = await fetchJson(apiPrefix + '/config/max-permissions', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      setMaxPermission(Boolean(payload.enabled));
      setStatus(l.savedOk);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : l.saveFailed);
    }
  ...
  hostAddBtn.addEventListener('click', async () => {
    const value = hostInput.value.trim();
    if (!value) return;
    const nextHosts = editingHost
      ? allowedHosts.map((item) => (item === editingHost ? value : item))
      : Array.from(new Set([...allowedHosts, value]));
    const saved = await saveConfig({ allowedHosts: nextHosts });
    if (!saved) return;
  ...
  hostList.addEventListener('click', async (event) => {
    ...
    const remove = target.closest('[data-host-delete]');
    if (edit instanceof HTMLElement) {
      const host = String(edit.getAttribute('data-host-edit') || '');
      if (!host) return;
      setHostEditorState(host);
      return;
    }
    if (remove instanceof HTMLElement) {
      const host = String(remove.getAttribute('data-host-delete') || '');
      if (!host) return;
      const nextHosts = allowedHosts.filter((item) => item !== host);
      const saved = await saveConfig({ allowedHosts: nextHosts });
      if (!saved) return;
      allowedHosts = nextHosts;
      renderHosts();
      if (editingHost === host) setHostEditorState('');
    }
  });
  ...
  testBtn.addEventListener('click', async () => {
    const gatewayUrl = getFormValue('gatewayUrl');
    if (!gatewayUrl) {
      setStatus(l.requiredGatewayUrl);
      return;
    }
    setStatus(l.testing);
    try {
      const { response, payload } = await fetchJson(apiPrefix + '/config/test', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          gatewayUrl,
          token: getFormValue('token'),
          password: getFormValue('password'),
        }),
      }, 30000);
      if (!response.ok || !payload?.success) {
        throw new Error(String(payload?.message || l.testFail));
      }
      setStatus(payload?.message ? String(payload.message) : l.testOk);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : l.testFail);
    }
  });
  ...
  restartBtn.addEventListener('click', async () => {
    ...
    try {
      const { response, payload } = await fetchJson(apiPrefix + '/config/restart', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      }, 35000);
      if (!response.ok || !payload?.success) {
        throw new Error(String(payload?.error || payload?.message || l.restartFail));
      }
      setStatus(l.restartOk);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : l.restartFail);
    }
  });
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    const saved = await saveConfig({
      ...
      token: getFormValue('token'),
      password: getFormValue('password'),
      openclawWorkspace: getFormValue('openclawWorkspace'),
      allowedHosts,
    });
    if (!saved) return;
    updatePublicGatewayUrl();
  ...
  const gatewayUrlInput = formNode.querySelector('[name="gatewayUrl"]');
  if (gatewayUrlInput instanceof HTMLInputElement) {
    gatewayUrlInput.addEventListener('input', updatePublicGatewayUrl);
  }
...
function renderModelConfigScript(language: UiLanguage = "zh"): string {
  const loadingProviders = escapeHtml(pickUiText(language, "Loading model settings...", "正在加载模型管理..."));
  const loadFailed = escapeHtml(pickUiText(language, "Failed to load model settings", "读取模型管理失败"));
  const saveFailed = escapeHtml(pickUiText(language, "Failed to save", "保存失败"));
  const savedOk = escapeHtml(pickUiText(language, "Saved and synced to openclaw.json", "已保存并同步到 openclaw.json"));
  const submitHint = escapeHtml(pickUiText(language, "Changes are synced to openclaw.json", "所有改动都会同步到 openclaw.json"));
  const requireFields = escapeHtml(pickUiText(language, "Please complete required fields.", "请填写必填字段。"));
  return `<script>
(() => {
  const root = document.querySelector('[data-model-config-root]');
  if (!(root instanceof HTMLElement)) return;
  const language = (root.getAttribute('data-language') || 'zh').trim().toLowerCase() === 'en' ? 'en' : 'zh';
  const modelTable = root.querySelector('[data-model-table]');
  const endpointTable = root.querySelector('[data-endpoint-table]');
  const sourceNode = root.querySelector('[data-model-config-source]');
  const statusNode = root.querySelector('[data-model-config-status]');
  const openModelButtons = Array.from(root.querySelectorAll('[data-open-model-modal]'));
  const openEndpointButtons = Array.from(root.querySelectorAll('[data-open-endpoint-modal]'));
  const modelBackdrop = root.querySelector('[data-model-modal-backdrop]');
  const endpointBackdrop = root.querySelector('[data-endpoint-modal-backdrop]');
  const modelModalForm = root.querySelector('[data-model-modal-form]');
  const endpointModalForm = root.querySelector('[data-endpoint-modal-form]');
  const modelModalTitle = root.querySelector('[data-model-modal-title]');
  const endpointModalTitle = root.querySelector('[data-endpoint-modal-title]');
  const modelEndpointSelect = root.querySelector('[data-model-endpoint-select]');
  const modelCloseButton = root.querySelector('[data-model-modal-close]');
  const endpointCloseButton = root.querySelector('[data-endpoint-modal-close]');
  const modelCancelButton = root.querySelector('[data-model-modal-cancel]');
  const endpointCancelButton = root.querySelector('[data-endpoint-modal-cancel]');
  if (!(modelTable instanceof HTMLTableElement) || !(endpointTable instanceof HTMLTableElement) || !(sourceNode instanceof HTMLElement) || !(statusNode instanceof HTMLElement) || !(modelBackdrop instanceof HTMLElement) || !(endpointBackdrop instanceof HTMLElement) || !(modelModalForm instanceof HTMLFormElement) || !(endpointModalForm instanceof HTMLFormElement) || !(modelModalTitle instanceof HTMLElement) || !(endpointModalTitle instanceof HTMLElement) || !(modelEndpointSelect instanceof HTMLSelectElement)) return;

  const modelTbody = modelTable.querySelector('tbody');
  const endpointTbody = endpointTable.querySelector('tbody');
  if (!(modelTbody instanceof HTMLElement) || !(endpointTbody instanceof HTMLElement)) return;

  const modelPriceStoreKey = 'openclaw_cc_model_prices';
  const readPriceMap = () => {
    try {
      const raw = localStorage.getItem(modelPriceStoreKey);
  ...
  const writePriceMap = (next) => {
    try {
      localStorage.setItem(modelPriceStoreKey, JSON.stringify(next));
    } catch {}
  };

  const l = {
    loadingProviders: ${JSON.stringify(loadingProviders)},
    loadFailed: ${JSON.stringify(loadFailed)},
    saveFailed: ${JSON.stringify(saveFailed)},
    savedOk: ${JSON.stringify(savedOk)},
    submitHint: ${JSON.stringify(submitHint)},
    requireFields: ${JSON.stringify(requireFields)},
    source: language === 'en' ? 'Config file' : '配置文件',
    modelsEmpty: language === 'en' ? 'No model configured yet.' : '当前没有模型配置。',
    endpointsEmpty: language === 'en' ? 'No endpoint configured yet.' : '当前没有节点配置。',
    modelsLabel: language === 'en' ? 'Models' : '模型',
    endpointsLabel: language === 'en' ? 'Endpoints' : '节点',
    addModel: language === 'en' ? 'Add model' : '添加模型',
    editModel: language === 'en' ? 'Edit model' : '修改模型',
    addEndpoint: language === 'en' ? 'Add endpoint' : '添加节点',
    editEndpoint: language === 'en' ? 'Edit endpoint' : '修改节点',
    defaultChecked: language === 'en' ? 'Current default' : '当前默认',
    setDefaultFailed: language === 'en' ? 'Failed to set default model' : '设置默认模型失败',
    confirmDeleteModel: language === 'en' ? 'Delete this model?' : '确认删除该模型吗？',
    confirmDeleteEndpoint: language === 'en' ? 'Delete this endpoint and all models under it?' : '确认删除该节点和其下所有模型吗？',
    placeholderInput: language === 'en' ? 'Input' : '单行输入',
    priceUnknown: language === 'en' ? 'Input: -  Output: -' : '输入：- 输出：-',
    saving: language === 'en' ? 'Saving...' : '保存中...',
  };

  const apiPrefix = '/chat-gateway/api';
  let endpoints = [];
  let models = [];
  let modelPriceMap = readPriceMap();
  let modelModalMode = 'add';
  let endpointModalMode = 'add';
  let editingModelId = '';
  let editingEndpointId = '';
  ...
  const ensureAdmin = async () => {
    if (typeof window.__openclawEnsureAdminAuth !== 'function') return true;
    return await window.__openclawEnsureAdminAuth();
  };
  ...
  const openBackdrop = (node) => {
    node.hidden = false;
  };
  const closeBackdrop = (node) => {
    node.hidden = true;
  };

  const fillModelEndpointSelect = () => {
    modelEndpointSelect.innerHTML = '';
    if (!Array.isArray(endpoints) || endpoints.length === 0) {
      const option = document.createElement('option');
      option.value = '';
      option.textContent = l.endpointsEmpty;
      modelEndpointSelect.append(option);
      modelEndpointSelect.disabled = true;
      return;
    }
    modelEndpointSelect.disabled = false;
  ...
  const formatPriceLine = (modelId) => {
    const info = modelPriceMap[modelId] || {};
    const priceIn = String(info.input ?? '').trim();
    const priceOut = String(info.output ?? '').trim();
    if (!priceIn && !priceOut) return l.priceUnknown;
    return (language === 'en' ? 'Input: ' : '输入：') + (priceIn || '-') + (language === 'en' ? ' Output: ' : ' 输出：') + (priceOut || '-');
  };

  const buildModelActions = (modelId) => '<div class="model-config-inline-actions"><button class="model-config-inline-icon" type="button" data-model-edit="' + modelId + '"><i class="fa-solid fa-pen"></i></button><button class="model-config-inline-icon danger" type="button" data-model-delete="' + modelId + '"><i class="fa-solid fa-trash"></i></button></div>';
  const buildEndpointActions = (endpointId) => '<div class="model-config-inline-actions"><button class="model-config-inline-icon" type="button" data-endpoint-edit="' + endpointId + '"><i class="fa-solid fa-pen"></i></button><button class="model-config-inline-icon danger" type="button" data-endpoint-delete="' + endpointId + '"><i class="fa-solid fa-trash"></i></button></div>';

  const renderModels = () => {
    modelTbody.innerHTML = '';
    if (!Array.isArray(models) || models.length === 0) {
      modelTbody.innerHTML = '<tr><td colspan="5" class="meta">' + l.modelsEmpty + '</td></tr>';
      return;
    }
    models.forEach((model) => {
      ...
      row.innerHTML =
        '<td>' + (alias || '-') + '</td>' +
        '<td>' + modelId + '</td>' +
        '<td><input class="model-default-toggle" type="checkbox" data-model-default="' + modelId + '"' + (model?.primary ? ' checked' : '') + ' /></td>' +
        '<td>' + formatPriceLine(modelId) + '</td>' +
        '<td>' + buildModelActions(modelId) + '</td>';
      modelTbody.append(row);
    });
  };
  ...
  const renderEndpoints = () => {
    ...
    if (!Array.isArray(endpoints) || endpoints.length === 0) {
      endpointTbody.innerHTML = '<tr><td colspan="5" class="meta">' + l.endpointsEmpty + '</td></tr>';
      return;
    }
    endpoints.forEach((endpoint) => {
      const endpointId = String(endpoint?.id || '');
      const modelCount = models.filter((item) => String(item?.id || '').startsWith(endpointId + '/')).length;
      const row = document.createElement('tr');
      row.innerHTML =
        '<td>' + endpointId + '</td>' +
        '<td>' + String(endpoint?.baseUrl || '-') + '</td>' +
        '<td>' + String(endpoint?.api || 'openai-completions') + '</td>' +
        '<td>' + String(modelCount) + '</td>' +
        '<td>' + buildEndpointActions(endpointId) + '</td>';
      endpointTbody.append(row);
    });
  };

  const setSourceSummary = () => {
    sourceNode.textContent = l.source + '：openclaw.json · ' + l.modelsLabel + ' ' + models.length + ' · ' + l.endpointsLabel + ' ' + endpoints.length;
  };

  const openModelModal = (mode, modelId = '') => {
    modelModalMode = mode;
    editingModelId = modelId;
    modelModalTitle.textContent = mode === 'edit' ? l.editModel : l.addModel;
    modelModalForm.reset();
    fillModelEndpointSelect();
    const endpointInput = modelModalForm.querySelector('select[name="endpoint"]');
    const modelInput = modelModalForm.querySelector('input[name="modelName"]');
    const aliasInput = modelModalForm.querySelector('input[name="alias"]');
    const priceInInput = modelModalForm.querySelector('input[name="priceIn"]');
    const priceOutInput = modelModalForm.querySelector('input[name="priceOut"]');
    if (mode === 'edit' && modelId) {
      const model = models.find((item) => String(item?.id || '') === modelId);
      const endpointId = modelId.includes('/') ? modelId.split('/')[0] : '';
      const modelName = modelId.includes('/') ? modelId.slice(endpointId.length + 1) : modelId;
      const priceInfo = modelPriceMap[modelId] || {};
      if (endpointInput instanceof HTMLSelectElement) endpointInput.value = endpointId || String(endpointInput.value || '');
      if (modelInput instanceof HTMLInputElement) {
        modelInput.value = modelName;
        modelInput.readOnly = true;
      }
      if (aliasInput instanceof HTMLInputElement) aliasInput.value = String(model?.alias || '');
      if (priceInInput instanceof HTMLInputElement) priceInInput.value = String(priceInfo.input || '');
      if (priceOutInput instanceof HTMLInputElement) priceOutInput.value = String(priceInfo.output || '');
    } else if (modelInput instanceof HTMLInputElement) {
      modelInput.readOnly = false;
    }
    openBackdrop(modelBackdrop);
  };

  const openEndpointModal = (mode, endpointId = '') => {
    endpointModalMode = mode;
    editingEndpointId = endpointId;
    endpointModalTitle.textContent = mode === 'edit' ? l.editEndpoint : l.addEndpoint;
    endpointModalForm.reset();
    if (mode === 'edit' && endpointId) {
      const endpoint = endpoints.find((item) => String(item?.id || '') === endpointId);
      if (endpoint) {
        const idInput = endpointModalForm.querySelector('input[name="endpointId"]');
        const baseUrlInput = endpointModalForm.querySelector('input[name="baseUrl"]');
        const apiKeyInput = endpointModalForm.querySelector('input[name="apiKey"]');
        const apiTypeInput = endpointModalForm.querySelector('select[name="apiType"]');
        if (idInput instanceof HTMLInputElement) idInput.value = String(endpoint.id || '');
        if (baseUrlInput instanceof HTMLInputElement) baseUrlInput.value = String(endpoint.baseUrl || '');
        if (apiKeyInput instanceof HTMLInputElement) apiKeyInput.value = String(endpoint.apiKey || '');
        if (apiTypeInput instanceof HTMLSelectElement) apiTypeInput.value = String(endpoint.api || 'openai-completions');
      }
    }
    openBackdrop(endpointBackdrop);
  };

  const reload = async () => {
    modelTbody.innerHTML = '<tr><td colspan="5" class="meta">' + l.loadingProviders + '</td></tr>';
    endpointTbody.innerHTML = '<tr><td colspan="5" class="meta">' + l.loadingProviders + '</td></tr>';
    try {
      const [modelsResult, endpointsResult] = await Promise.all([
        fetchJson(apiPrefix + '/models', undefined, 20000),
        fetchJson(apiPrefix + '/endpoints', undefined, 20000),
      ]);
      if (!modelsResult.response.ok || !modelsResult.payload?.success) {
        throw new Error(String(modelsResult.payload?.error || l.loadFailed));
      }
      if (!endpointsResult.response.ok || !endpointsResult.payload?.success) {
        throw new Error(String(endpointsResult.payload?.error || l.loadFailed));
      }
      models = Array.isArray(modelsResult.payload.models) ? modelsResult.payload.models : [];
      endpoints = Array.isArray(endpointsResult.payload.endpoints) ? endpointsResult.payload.endpoints : [];
      fillModelEndpointSelect();
      setSourceSummary();
      renderModels();
      renderEndpoints();
      ...
      const errText = error instanceof Error ? error.message : l.loadFailed;
      modelTbody.innerHTML = '<tr><td colspan="5" class="meta">' + errText + '</td></tr>';
      endpointTbody.innerHTML = '<tr><td colspan="5" class="meta">' + errText + '</td></tr>';
      setStatus(errText);
    }
  };
  ...
  openModelButtons.forEach((btn) => {
    ...
    btn.addEventListener('click', () => openModelModal('add'));
  });
  openEndpointButtons.forEach((btn) => {
    if (!(btn instanceof HTMLElement)) return;
    btn.addEventListener('click', () => openEndpointModal('add'));
  });

  if (modelCloseButton instanceof HTMLElement) modelCloseButton.addEventListener('click', () => closeBackdrop(modelBackdrop));
  if (endpointCloseButton instanceof HTMLElement) endpointCloseButton.addEventListener('click', () => closeBackdrop(endpointBackdrop));
  if (modelCancelButton instanceof HTMLElement) modelCancelButton.addEventListener('click', () => closeBackdrop(modelBackdrop));
  if (endpointCancelButton instanceof HTMLElement) endpointCancelButton.addEventListener('click', () => closeBackdrop(endpointBackdrop));
  closeBackdrop(modelBackdrop);
  closeBackdrop(endpointBackdrop);

  modelBackdrop.addEventListener('click', (event) => {
    if (event.target === modelBackdrop) closeBackdrop(modelBackdrop);
  });
  endpointBackdrop.addEventListener('click', (event) => {
    if (event.target === endpointBackdrop) closeBackdrop(endpointBackdrop);
  });

  endpointModalForm.addEventListener('submit', async (event) => {
    event.preventDefault();
    const formData = new FormData(endpointModalForm);
    const id = String(formData.get('endpointId') || '').trim();
    const baseUrl = String(formData.get('baseUrl') || '').trim();
    const apiKey = String(formData.get('apiKey') || '').trim();
    const api = String(formData.get('apiType') || '').trim();
    if (!id || !baseUrl || !api) {
      setStatus(l.requireFields);
      return;
    }
    if (!(await ensureAdmin())) {
      setStatus(l.saveFailed);
      return;
    }
    setStatus(l.saving);
    try {
      const { response, payload } = await fetchJson(apiPrefix + '/endpoints', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ id, baseUrl, apiKey, api }),
      }, 25000);
      if (!response.ok || !payload?.success) {
        throw new Error(String(payload?.error || l.saveFailed));
      }
      closeBackdrop(endpointBackdrop);
      setStatus(l.savedOk);
      await reload();
  ...
  modelModalForm.addEventListener('submit', async (event) => {
    event.preventDefault();
    const formData = new FormData(modelModalForm);
    ...
    try {
      if (modelModalMode === 'add') {
        const { response, payload } = await fetchJson(apiPrefix + '/models/manage', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ endpoint, modelName, alias: alias || undefined }),
        }, 25000);
        if (!response.ok || !payload?.success) {
          throw new Error(String(payload?.error || l.saveFailed));
        }
        const fullId = endpoint + '/' + modelName;
        modelPriceMap[fullId] = { input: priceIn, output: priceOut };
      } else {
        const fullId = editingModelId;
        const { response, payload } = await fetchJson(apiPrefix + '/models/manage', {
          method: 'PUT',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ id: fullId, alias }),
        }, 25000);
        if (!response.ok || !payload?.success) {
          throw new Error(String(payload?.error || l.saveFailed));
        }
  ...
  modelTbody.addEventListener('click', async (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    const editBtn = target.closest('[data-model-edit]');
    const deleteBtn = target.closest('[data-model-delete]');
    const defaultInput = target.closest('[data-model-default]');
    if (editBtn instanceof HTMLElement) {
      const modelId = String(editBtn.getAttribute('data-model-edit') || '');
      if (!modelId) return;
      openModelModal('edit', modelId);
      return;
    }
    if (deleteBtn instanceof HTMLElement) {
      const modelId = String(deleteBtn.getAttribute('data-model-delete') || '');
      if (!modelId) return;
      if (!window.confirm(l.confirmDeleteModel)) return;
      if (!(await ensureAdmin())) {
        setStatus(l.saveFailed);
        return;
      }
    ...
    if (defaultInput instanceof HTMLInputElement) {
      const modelId = String(defaultInput.getAttribute('data-model-default') || '');
      if (!modelId) return;
      let nextDefaultId = modelId;
      if (!defaultInput.checked) {
        const fallback = models.find((item) => String(item?.id || '') !== modelId);
        if (!fallback) {
          defaultInput.checked = true;
          return;
        }
        nextDefaultId = String(fallback.id || '');
      }
      if (!(await ensureAdmin())) {
        setStatus(l.saveFailed);
        return;
      ...
      setStatus(l.saving);
      try {
        const { response, payload } = await fetchJson(apiPrefix + '/models/manage/default', {
          method: 'PUT',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ id: nextDefaultId }),
        }, 25000);
        if (!response.ok || !payload?.success) {
          throw new Error(String(payload?.error || l.setDefaultFailed));
        }
        setStatus(l.savedOk);
        await reload();
      } catch (error) {
        setStatus(error instanceof Error ? error.message : l.setDefaultFailed);
        await reload();
      }
    }
  });

  endpointTbody.addEventListener('click', async (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    const editBtn = target.closest('[data-endpoint-edit]');
    const deleteBtn = target.closest('[data-endpoint-delete]');
    if (editBtn instanceof HTMLElement) {
      const endpointId = String(editBtn.getAttribute('data-endpoint-edit') || '');
      if (!endpointId) return;
      openEndpointModal('edit', endpointId);
      return;
    }
    if (deleteBtn instanceof HTMLElement) {
      const endpointId = String(deleteBtn.getAttribute('data-endpoint-delete') || '');
      if (!endpointId) return;
      if (!window.confirm(l.confirmDeleteEndpoint)) return;
      if (!(await ensureAdmin())) {
        setStatus(l.saveFailed);
        return;
      }
      setStatus(l.saving);
      try {
        const { response, payload } = await fetchJson(apiPrefix + '/endpoints/manage', {
          method: 'DELETE',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
        ...
        Object.keys(modelPriceMap).forEach((key) => {
          if (key.startsWith(endpointId + '/')) delete modelPriceMap[key];
        });
        writePriceMap(modelPriceMap);
        setStatus(l.savedOk);
        await reload();
...
function renderAgentManagementScript(language: UiLanguage = "zh"): string {
  const loadingAgents = escapeHtml(pickUiText(language, "Loading agents...", "正在读取智能体..."));
  const loadFailed = escapeHtml(pickUiText(language, "Failed to load agents", "读取智能体失败"));
  const saveFailed = escapeHtml(pickUiText(language, "Failed to save agent", "保存智能体失败"));
  const savedOk = escapeHtml(pickUiText(language, "Agent saved", "智能体已保存"));
  const deletedOk = escapeHtml(pickUiText(language, "Agent deleted", "智能体已删除"));
  const deleting = escapeHtml(pickUiText(language, "Deleting...", "正在删除..."));
  const saving = escapeHtml(pickUiText(language, "Saving...", "正在保存..."));
  const requireFields = escapeHtml(pickUiText(language, "Please fill Agent ID and Agent name", "请填写智能体ID和名称"));
  const confirmDelete = escapeHtml(pickUiText(language, "Delete this agent?", "确认删除该智能体吗？"));
  return `<script>
(() => {
  const root = document.querySelector('[data-agent-management-root]');
  if (!(root instanceof HTMLElement)) return;
  const language = (root.getAttribute('data-language') || ${JSON.stringify(language)}).trim().toLowerCase() === 'en' ? 'en' : 'zh';
  const sourceNode = root.querySelector('[data-agent-source]');
  const statusNode = root.querySelector('[data-agent-status]');
  const tableNode = root.querySelector('[data-agent-table] tbody');
  const openButtons = Array.from(root.querySelectorAll('[data-agent-open-modal]'));
  const backdrop = root.querySelector('[data-agent-modal-backdrop]');
  const closeBtn = root.querySelector('[data-agent-modal-close]');
  const cancelBtn = root.querySelector('[data-agent-modal-cancel]');
  const titleNode = root.querySelector('[data-agent-modal-title]');
  const formNode = root.querySelector('[data-agent-modal-form]');
  const modelInput = root.querySelector('[data-agent-model-input]');
  const modelMenu = root.querySelector('[data-agent-model-menu]');
  const mdEditor = root.querySelector('[data-agent-md-editor]');
  const tabButtons = Array.from(root.querySelectorAll('[data-agent-tab]'));
  if (!(sourceNode instanceof HTMLElement) || !(statusNode instanceof HTMLElement) || !(tableNode instanceof HTMLElement) || !(backdrop instanceof HTMLElement) || !(formNode instanceof HTMLFormElement) || !(titleNode instanceof HTMLElement) || !(modelInput instanceof HTMLInputElement) || !(modelMenu instanceof HTMLElement) || !(mdEditor instanceof HTMLTextAreaElement)) return;

  const l = {
    loadingAgents: ${JSON.stringify(loadingAgents)},
    loadFailed: ${JSON.stringify(loadFailed)},
    saveFailed: ${JSON.stringify(saveFailed)},
    savedOk: ${JSON.stringify(savedOk)},
    deletedOk: ${JSON.stringify(deletedOk)},
    deleting: ${JSON.stringify(deleting)},
    saving: ${JSON.stringify(saving)},
    requireFields: ${JSON.stringify(requireFields)},
    confirmDelete: ${JSON.stringify(confirmDelete)},
    source: language === 'en' ? 'Agents' : '智能体',
    edit: language === 'en' ? 'Edit' : '编辑',
    remove: language === 'en' ? 'Delete' : '删除',
    createTitle: language === 'en' ? 'Create agent' : '新建智能体',
    editTitle: language === 'en' ? 'Edit agent' : '编辑智能体',
    empty: language === 'en' ? 'No agents yet.' : '暂无智能体。',
    cannotDeleteMain: language === 'en' ? 'Main agent cannot be deleted' : '主智能体不可删除',
    modelDefault: language === 'en' ? 'Click to select model (optional)' : '点击选择模型（留空使用默认）',
    noModels: language === 'en' ? 'No models found' : '未发现可用模型',
  };

  const templates = {
    soulContent: language === 'en' ? 'Describe persona, style, and goals' : '请描述智能体的人格、表达风格与目标',
    userContent: language === 'en' ? 'Describe preferred user interaction style' : '请描述与用户交互的方式与边界',
    agentsContent: language === 'en' ? 'Describe multi-agent collaboration principles' : '请描述与其他智能体协作原则',
    toolsContent: language === 'en' ? 'Describe tool usage constraints and safety rules' : '请描述工具调用限制与安全规则',
    heartbeatContent: language === 'en' ? 'Describe heartbeat checks and periodic actions' : '请描述心跳检查与周期性动作',
    identityContent: language === 'en' ? 'Describe identity profile and context' : '请描述身份定位与上下文',
  };
  ...
  const inputOf = (name) => formNode.querySelector('[name="' + name + '"]');
  ...
  const setIdReadOnly = (readonly) => {
    const input = inputOf('id');
    if (input instanceof HTMLInputElement) input.readOnly = readonly;
  };
  ...
  const syncTabButtons = () => {
    tabButtons.forEach((btn) => {
      if (!(btn instanceof HTMLElement)) return;
      const tab = String(btn.getAttribute('data-agent-tab') || '');
      btn.classList.toggle('active', tab === activeTab);
    });
  };

  const syncEditor = () => {
    mdEditor.value = String(draft[activeTab] || '');
    mdEditor.placeholder = String(templates[activeTab] || '');
    syncTabButtons();
  };

  const setDraft = (payload) => {
    draft = {
      soulContent: String(payload?.soulContent || ''),
      userContent: String(payload?.userContent || ''),
      agentsContent: String(payload?.agentsContent || ''),
      toolsContent: String(payload?.toolsContent || ''),
      heartbeatContent: String(payload?.heartbeatContent || ''),
      identityContent: String(payload?.identityContent || ''),
    };
    syncEditor();
  };
  ...
  const renderModelMenu = () => {
    ...
    const rows = availableModels.filter((item) => {
      const id = String(item?.id || '').toLowerCase();
      const alias = String(item?.alias || '').toLowerCase();
      return !q || id.includes(q) || alias.includes(q);
    });
    if (rows.length === 0) {
      const empty = document.createElement('button');
      empty.type = 'button';
      empty.className = 'agent-model-option';
      empty.textContent = l.noModels;
      empty.disabled = true;
    ...
    rows
      ...
      .forEach((item) => {
        ...
        btn.type = 'button';
        btn.className = 'agent-model-option' + (getValue('model') === id ? ' active' : '');
        btn.textContent = id;
  ...
  const loadModels = async () => {
    try {
      const { response, payload } = await fetchWithFallback('/models', undefined, 20000);
      if (!response.ok) return;
      const rows = Array.isArray(payload) ? payload : (Array.isArray(payload?.models) ? payload.models : []);
      availableModels = rows.filter((item) => item && typeof item === 'object');
    } catch {}
  };
  ...
  const closeModal = () => {
    ...
    editingId = '';
    activeTab = 'soulContent';
    setDraft({});
  };

  const openModal = async (mode, rowId = '') => {
    formNode.reset();
    editingId = mode === 'edit' ? rowId : '';
    setIdReadOnly(mode === 'edit');
    titleNode.textContent = mode === 'edit' ? l.editTitle : l.createTitle;
    setDraft({});
    setValue('model', '');
    modelInput.value = '';
    backdrop.hidden = false;
    if (mode === 'edit') {
      const row = sessions.find((item) => String(item?.sessionId || item?.id || '') === rowId || String(item?.id || '') === rowId);
      setValue('id', row?.id || '');
      setValue('name', row?.name || '');
      setValue('model', row?.model || '');
      modelInput.value = String(row?.model || '');
      try {
        const { response, payload } = await fetchWithFallback('/sessions/' + encodeURIComponent(rowId) + '/configs', undefined, 25000);
  ...
  const normalizeRows = (payload) => {
    ...
    const rows = list
      ...
      .map((item) => ({
        id: String(item.agentId || item.id || '').trim(),
  ...
  const renderRows = () => {
    ...
    if (!Array.isArray(sessions) || sessions.length === 0) {
      tableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.empty + '</td></tr>';
      return;
    }
    sessions.forEach((item) => {
      const opId = item.sessionId || item.id;
      const isMain = item.id === 'main' || opId === 'main';
      const tr = document.createElement('tr');
      tr.innerHTML = '<td>' + item.id + '</td><td>' + (item.name || '-') + '</td><td>' + (item.model || '-') + '</td><td><button class="model-config-row-action" type="button" data-agent-edit="' + opId + '" title="' + l.edit + '"><i class="fa-solid fa-pen"></i></button><button class="model-config-row-action model-config-row-action-danger" type="button" data-agent-delete="' + opId + '" title="' + (isMain ? l.cannotDeleteMain : l.remove) + '"' + (isMain ? ' disabled' : '') + '><i class="fa-solid fa-trash"></i></button></td>';
      tableNode.append(tr);
    });
  };

  const reload = async () => {
    sourceNode.textContent = l.loadingAgents;
    try {
      const { response, payload } = await fetchWithFallback('/sessions', undefined, 25000);
      if (!response.ok) throw new Error(l.loadFailed);
      sessions = normalizeRows(payload);
      sourceNode.textContent = l.source + '：' + String(sessions.length);
      renderRows();
      setStatus('');
    } catch (error) {
      sourceNode.textContent = error instanceof Error ? error.message : l.loadFailed;
      tableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.loadFailed + '</td></tr>';
      setStatus(l.loadFailed);
    }
  };
  ...
  if (cancelBtn instanceof HTMLElement) cancelBtn.addEventListener('click', closeModal);
  backdrop.addEventListener('click', (event) => {
    if (event.target === backdrop) closeModal();
  });

  const idInput = inputOf('id');
  if (idInput instanceof HTMLInputElement) {
    idInput.addEventListener('input', () => {
      const value = sanitizeId(idInput.value);
      if (value !== idInput.value) idInput.value = value;
    });
  }
  ...
  tabButtons.forEach((btn) => {
    if (!(btn instanceof HTMLElement)) return;
    btn.addEventListener('click', () => {
      draft[activeTab] = mdEditor.value;
      activeTab = String(btn.getAttribute('data-agent-tab') || 'soulContent');
      syncEditor();
      mdEditor.focus();
    });
  });
  ...
  modelInput.addEventListener('input', () => {
    setValue('model', modelInput.value.trim());
    modelSearch = modelInput.value.trim();
    renderModelMenu();
  });
  root.addEventListener('click', (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    if (target.closest('[data-agent-model-input]') || target.closest('[data-agent-model-menu]')) return;
    closeModelMenu();
  });
  ...
  formNode.addEventListener('submit', async (event) => {
    ...
    const body = {
      ...
      soulContent: draft.soulContent,
      userContent: draft.userContent,
      agentsContent: draft.agentsContent,
      toolsContent: draft.toolsContent,
      heartbeatContent: draft.heartbeatContent,
      identityContent: draft.identityContent,
    };
    setStatus(l.saving);
    try {
      const targetId = editingId || id;
      const { response, payload } = await fetchWithFallback('/sessions' + (editingId ? '/' + encodeURIComponent(targetId) : ''), {
  ...
  tableNode.addEventListener('click', async (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    const editBtn = target.closest('[data-agent-edit]');
    if (editBtn instanceof HTMLElement) {
      const rowId = String(editBtn.getAttribute('data-agent-edit') || '');
      if (!rowId) return;
      void openModal('edit', rowId);
      return;
    }
    const deleteBtn = target.closest('[data-agent-delete]');
    if (deleteBtn instanceof HTMLElement) {
      const rowId = String(deleteBtn.getAttribute('data-agent-delete') || '');
      if (!rowId || rowId === 'main') return;
      if (!window.confirm(l.confirmDelete)) return;
      if (!(await ensureAdmin())) {
        setStatus(l.saveFailed);
        return;
      }
      setStatus(l.deleting);
      try {
        const { response, payload } = await fetchWithFallback('/sessions/' + encodeURIComponent(rowId), {
          method: 'DELETE',
          headers: { 'content-type': 'application/json' },
        }, 25000);
        if (!response.ok || payload?.success === false) {
...
function renderSkillsScript(language: UiLanguage = "zh"): string {
  const loading = escapeHtml(pickUiText(language, "Loading skills...", "正在读取技能..."));
  const loadFailed = escapeHtml(pickUiText(language, "Failed to load skills", "读取技能失败"));
  const saveFailed = escapeHtml(pickUiText(language, "Operation failed", "操作失败"));
  const savedOk = escapeHtml(pickUiText(language, "Skill installed", "技能已安装"));
  const updatedOk = escapeHtml(pickUiText(language, "Skill updated", "技能状态已更新"));
  const removedOk = escapeHtml(pickUiText(language, "Skill removed", "技能已移除"));
  const requireSource = escapeHtml(pickUiText(language, "Please input skill source", "请填写技能来源"));
  const confirmUninstall = escapeHtml(pickUiText(language, "Uninstall this skill?", "确认卸载该技能吗？"));
  return `<script>
(() => {
  const root = document.querySelector('[data-skills-root]');
  if (!(root instanceof HTMLElement)) return;
  const language = (root.getAttribute('data-language') || ${JSON.stringify(language)}).trim().toLowerCase() === 'en' ? 'en' : 'zh';
  const sourceNode = root.querySelector('[data-skill-source]');
  const statusNode = root.querySelector('[data-skill-status]');
  const enabledTableNode = root.querySelector('[data-skill-table-enabled] tbody');
  const disabledTableNode = root.querySelector('[data-skill-table] tbody');
  const disabledToggle = root.querySelector('[data-skill-disabled-toggle]');
  const disabledPanel = root.querySelector('[data-skill-disabled-panel]');
  const openBtn = root.querySelector('[data-skill-open-modal]');
  const backdrop = root.querySelector('[data-skill-modal-backdrop]');
  const closeBtn = root.querySelector('[data-skill-modal-close]');
  const cancelBtn = root.querySelector('[data-skill-modal-cancel]');
  const formNode = root.querySelector('[data-skill-modal-form]');
  if (!(sourceNode instanceof HTMLElement) || !(statusNode instanceof HTMLElement) || !(enabledTableNode instanceof HTMLElement) || !(disabledTableNode instanceof HTMLElement) || !(disabledToggle instanceof HTMLElement) || !(disabledPanel instanceof HTMLElement) || !(backdrop instanceof HTMLElement) || !(formNode instanceof HTMLFormElement)) return;
  ...
  const l = {
    ...
    saveFailed: ${JSON.stringify(saveFailed)},
    savedOk: ${JSON.stringify(savedOk)},
    updatedOk: ${JSON.stringify(updatedOk)},
    removedOk: ${JSON.stringify(removedOk)},
    requireSource: ${JSON.stringify(requireSource)},
    confirmUninstall: ${JSON.stringify(confirmUninstall)},
    empty: language === 'en' ? 'No skills found.' : '未发现技能。',
    installed: language === 'en' ? 'Installed' : '已安装',
    bundled: language === 'en' ? 'Built-in' : '内置',
    eligible: language === 'en' ? 'eligible' : '可用',
    blocked: language === 'en' ? 'blocked' : '受限',
    enable: language === 'en' ? 'Enable' : '启用',
    uninstall: language === 'en' ? 'Uninstall' : '卸载',
    source: language === 'en' ? 'Skills summary' : '技能概览',
    operating: language === 'en' ? 'Processing...' : '处理中...',
    enabledList: language === 'en' ? 'Enabled skills' : '已启用技能',
    disabledList: language === 'en' ? 'Disabled skills' : '未启用的技能',
    hideDisabledList: language === 'en' ? 'Hide disabled skills' : '收起未启用的技能',
  };
  ...
  const setDisabledPanelExpanded = (next) => {
    disabledExpanded = next;
    disabledPanel.hidden = !disabledExpanded;
  };

  const renderRows = () => {
    enabledTableNode.innerHTML = '';
    disabledTableNode.innerHTML = '';
    if (!Array.isArray(skillRows) || skillRows.length === 0) {
      enabledTableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.empty + '</td></tr>';
      disabledTableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.empty + '</td></tr>';
      disabledToggle.textContent = l.disabledList + ' (0)';
      setDisabledPanelExpanded(false);
      return;
    }
    const enabledRows = skillRows.filter((item) => item?.eligible === true && item?.bundled !== true && item?.disabled !== true);
    const disabledRows = skillRows.filter((item) => !(item?.eligible === true && item?.bundled !== true && item?.disabled !== true));
    const drawRow = (item, targetTable, isEnabledList) => {
      const missingList = []
        .concat(Array.isArray(item?.missing?.bins) ? item.missing.bins : [])
        .concat(Array.isArray(item?.missing?.env) ? item.missing.env : [])
        .concat(Array.isArray(item?.missing?.config) ? item.missing.config : [])
        .concat(Array.isArray(item?.missing?.os) ? item.missing.os : []);
      const desc = String(item?.description || '').trim();
      const detail = missingList.length > 0 ? '<div class="meta">missing: ' + missingList.join(', ') + '</div>' : '';
      const statusBadge = item?.eligible
        ? '<span class="skills-check-icon" title="' + l.eligible + '"><i class="fa-solid fa-circle-check"></i></span>'
        : '<span class="badge warn">' + l.blocked + '</span>';
      const opKey = String(item?.skillKey || '');
      const enableBtn = !isEnabledList
        ? '<button class="model-config-row-action skill-enable-action" type="button" data-skill-enable="' + opKey + '" title="' + l.enable + '">' + l.enable + '</button>'
        : '';
      const uninstallBtn = item?.bundled
        ? ''
        : '<button class="model-config-row-action model-config-row-action-danger" type="button" data-skill-uninstall="' + opKey + '" title="' + l.uninstall + '"><i class="fa-solid fa-trash"></i></button>';
      const tr = document.createElement('tr');
      tr.innerHTML = '<td>' + (item?.emoji ? item.emoji + ' ' : '') + opKey + '</td><td>' + (desc || '-') + detail + '</td><td>' + String(item?.source || '-') + '</td><td class="skills-status-cell"><div class="skills-status-badges">' + statusBadge + '<span class="skills-status-actions">' + enableBtn + uninstallBtn + '</span></div></td>';
      targetTable.append(tr);
    };
    if (enabledRows.length === 0) {
      enabledTableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.empty + '</td></tr>';
    } else {
      enabledRows.forEach((item) => drawRow(item, enabledTableNode, true));
    }
    if (disabledRows.length === 0) {
      disabledTableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.empty + '</td></tr>';
    } else {
      disabledRows.forEach((item) => drawRow(item, disabledTableNode, false));
    }
    disabledToggle.textContent = (disabledExpanded ? l.hideDisabledList : l.disabledList) + ' (' + String(disabledRows.length) + ')';
    if (disabledRows.length === 0) setDisabledPanelExpanded(false);
  };

  const reload = async () => {
    sourceNode.textContent = l.loading;
    try {
      const { response, payload } = await fetchJson('/api/skills', undefined, 30000);
      if (!response.ok || !payload?.ok) throw new Error(String(payload?.message || l.loadFailed));
      skillRows = Array.isArray(payload.skills) ? payload.skills : [];
      const total = Number(payload.totalSkills || skillRows.length || 0);
      const installed = Number(payload.installedSkills || 0);
      sourceNode.textContent = l.source + '：' + l.installed + ' ' + installed + ' / total ' + total;
      renderRows();
      setStatus('');
    } catch (error) {
      sourceNode.textContent = error instanceof Error ? error.message : l.loadFailed;
      enabledTableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.loadFailed + '</td></tr>';
      disabledTableNode.innerHTML = '<tr><td colspan="4" class="meta">' + l.loadFailed + '</td></tr>';
      setStatus(l.loadFailed);
    }
  };

  disabledToggle.addEventListener('click', () => {
    setDisabledPanelExpanded(!disabledExpanded);
    const disabledRowsCount = skillRows.filter((item) => !(item?.eligible === true && item?.bundled !== true && item?.disabled !== true)).length;
    disabledToggle.textContent = (disabledExpanded ? l.hideDisabledList : l.disabledList) + ' (' + String(disabledRowsCount) + ')';
  });
  ...
  formNode.addEventListener('submit', async (event) => {
    event.preventDefault();
    const sourceInput = formNode.querySelector('[name="source"]');
    const source = sourceInput instanceof HTMLInputElement ? sourceInput.value.trim() : '';
    if (!source) {
      setStatus(l.requireSource);
      return;
    }
    if (!(await ensureAdmin())) {
      setStatus(l.saveFailed);
    ...
    setStatus(l.operating);
    try {
      const { response, payload } = await fetchJson('/api/skills/install', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ source }),
      }, 60000);
      if (!response.ok || !payload?.ok) throw new Error(String(payload?.message || l.saveFailed));
      closeModal();
      setStatus(l.savedOk);
      await reload();
    } catch (error) {
  ...
  root.addEventListener('click', async (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    const enableBtn = target.closest('[data-skill-enable]');
    if (enableBtn instanceof HTMLElement) {
      const skillKey = String(enableBtn.getAttribute('data-skill-enable') || '');
      if (!skillKey) return;
      if (!(await ensureAdmin())) {
        setStatus(l.saveFailed);
        return;
      ...
      setStatus(l.operating);
      try {
        const { response, payload } = await fetchJson('/api/skills/update', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ skillKey, disabled: false }),
        }, 45000);
        if (!response.ok || !payload?.ok) throw new Error(String(payload?.message || l.saveFailed));
        setStatus(l.updatedOk);
        await reload();
      } catch (error) {
        setStatus(error instanceof Error ? error.message : l.saveFailed);
      }
    ...
    const uninstallBtn = target.closest('[data-skill-uninstall]');
    if (uninstallBtn instanceof HTMLElement) {
      const skillKey = String(uninstallBtn.getAttribute('data-skill-uninstall') || '');
      if (!skillKey) return;
      if (!window.confirm(l.confirmUninstall)) return;
      if (!(await ensureAdmin())) {
        setStatus(l.saveFailed);
        return;
      }
      setStatus(l.operating);
      try {
        const { response, payload } = await fetchJson('/api/skills/uninstall', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ skillKey }),
        }, 45000);
        if (!response.ok || !payload?.ok) throw new Error(String(payload?.message || l.saveFailed));
        setStatus(l.removedOk);
        await reload();
      } catch (error) {
        setStatus(error instanceof Error ? error.message : l.saveFailed);
      }
...
function renderChannelConfigScript(language: UiLanguage = "zh"): string {
  const loadingChannels = escapeHtml(pickUiText(language, "Loading channels...", "正在加载渠道列表..."));
  const loadFailed = escapeHtml(pickUiText(language, "Failed to load channels", "读取渠道配置失败"));
  const saveFailed = escapeHtml(pickUiText(language, "Failed to save channel config", "写入渠道配置失败"));
  const savedOk = escapeHtml(pickUiText(language, "Channel config saved to openclaw.json", "渠道配置已写入 openclaw.json"));
  const parseJsonFailed = escapeHtml(pickUiText(language, "Config JSON is invalid.", "配置 JSON 格式不正确。"));
  const requireFields = escapeHtml(pickUiText(language, "Please fill channel key and config JSON.", "请填写渠道标识与配置 JSON。"));
  return `<script>
(() => {
  const root = document.querySelector('[data-channel-config-root]');
  if (!(root instanceof HTMLElement)) return;
  const language = (root.getAttribute('data-language') || 'zh').trim().toLowerCase() === 'en' ? 'en' : 'zh';
  const listNode = root.querySelector('[data-channel-config-list]');
  const sourceNode = root.querySelector('[data-channel-config-source]');
  const statusNode = root.querySelector('[data-channel-config-status]');
  const openDialogButton = root.querySelector('[data-channel-config-open-dialog]');
  const dialogNode = root.querySelector('[data-channel-config-dialog]');
  const formNode = root.querySelector('[data-channel-config-form]');
  const cancelNode = root.querySelector('[data-channel-config-cancel]');
  if (!(listNode instanceof HTMLElement) || !(sourceNode instanceof HTMLElement)) return;

  const l = {
    loadingChannels: ${JSON.stringify(loadingChannels)},
    loadFailed: ${JSON.stringify(loadFailed)},
    saveFailed: ${JSON.stringify(saveFailed)},
    savedOk: ${JSON.stringify(savedOk)},
    parseJsonFailed: ${JSON.stringify(parseJsonFailed)},
    requireFields: ${JSON.stringify(requireFields)},
    empty: language === 'en' ? 'No channel configured yet.' : '当前尚未配置任何渠道。',
    connected: language === 'en' ? 'Connected' : '已连接',
    disconnected: language === 'en' ? 'Disconnected' : '未连接',
    installed: language === 'en' ? 'Configured' : '插件已安装',
    appId: language === 'en' ? 'APP ID' : 'APP ID',
    appSecret: language === 'en' ? 'APP Secret' : 'APP Secret',
    accessUrl: language === 'en' ? 'Domain' : '接入域名',
    accessMode: language === 'en' ? 'Connection mode' : '连接模式',
    enabledField: language === 'en' ? 'Enabled' : '是否激活',
    allowGroup: language === 'en' ? 'Group feature' : '允许群功能',
    test: language === 'en' ? 'Test connection' : '测试连接',
    save: language === 'en' ? 'Save' : '保存',
    testPass: language === 'en' ? 'Validation passed.' : '校验通过。',
    testFail: language === 'en' ? 'Please complete APP ID / APP Secret / Domain.' : '请先补全 APP ID / APP Secret / 接入域名。',
    yes: language === 'en' ? 'Enabled' : '已开启',
    no: language === 'en' ? 'Disabled' : '关闭',
    saving: language === 'en' ? 'Saving...' : '保存中...',
  };
  ...
  const setStatus = (text) => {
    if (statusNode instanceof HTMLElement) statusNode.textContent = text || '';
  };
  ...
  const pickBool = (obj, keys, fallback = false) => {
    for (const key of keys) {
      ...
      if (typeof value === 'string') {
        ...
        if (normalized === 'true' || normalized === '1' || normalized === 'yes' || normalized === 'enabled' || normalized === 'on' || normalized === 'open' || normalized === 'allow') return true;
        if (normalized === 'false' || normalized === '0' || normalized === 'no' || normalized === 'disabled' || normalized === 'off' || normalized === 'closed' || normalized === 'close' || normalized === 'deny' || normalized === 'block') return false;
      }
    }
    return fallback;
  };

  const setByCandidate = (obj, keys, value) => {
    const chosen = keys.find((key) => Object.prototype.hasOwnProperty.call(obj, key)) || keys[0];
    obj[chosen] = value;
  };

  const buildChannelRows = (channels) => {
    const rows = Array.isArray(channels) ? channels.filter((item) => item && typeof item === 'object') : [];
    const byKey = new Map();
    rows.forEach((item) => {
      const key = String(item?.channelKey || '').trim().toLowerCase();
      if (!key) return;
      byKey.set(key, item);
    });
    const defaults = [
      { channelKey: 'feishu', enabled: true, config: { connectionMode: 'websocket', domain: 'feishu', enabled: true, groupPolicy: 'open' } },
      { channelKey: 'telegram', enabled: true, config: { connectionMode: 'websocket', domain: 'telegram', enabled: true, groupPolicy: 'open' } },
    ];
    defaults.forEach((item) => {
      if (!byKey.has(item.channelKey)) byKey.set(item.channelKey, item);
    });
    return Array.from(byKey.values()).sort((a, b) => {
      const order = ['feishu', 'telegram'];
      const ak = String(a?.channelKey || '').toLowerCase();
      const bk = String(b?.channelKey || '').toLowerCase();
      const ai = order.includes(ak) ? order.indexOf(ak) : order.length + 1;
      const bi = order.includes(bk) ? order.indexOf(bk) : order.length + 1;
  ...
  const renderChannels = (channels) => {
    ...
    channelRows.forEach((channel) => {
      const channelKey = String(channel?.channelKey || '').trim();
      if (!channelKey) return;
      const configObj = channel?.config && typeof channel.config === 'object' && !Array.isArray(channel.config) ? { ...channel.config } : {};
      channelState.set(channelKey, { channel, config: configObj });
      const appId = pickText(configObj, ['appId', 'app_id', 'clientId', 'client_id', 'botId', 'bot_id', 'chatId', 'chat_id']);
      const appSecret = pickText(configObj, ['appSecret', 'app_secret', 'clientSecret', 'client_secret', 'token', 'botToken', 'bot_token']);
      const accessUrl = pickText(configObj, ['domain', 'url', 'accessUrl', 'webhookUrl', 'callbackUrl']);
      const accessMode = pickText(configObj, ['connectionMode', 'mode', 'accessMode', 'type']) || 'websocket';
      const enabled = pickBool(configObj, ['enabled', 'active'], Boolean(channel?.enabled));
      const connected = enabled && Boolean(appId && appSecret && accessUrl);
      const allowGroup = pickBool(configObj, ['groupPolicy', 'allowGroup', 'allowGroupMessage', 'groupEnabled'], true);
      const keyLower = channelKey.toLowerCase();
      const displayName = keyLower === 'feishu' ? '飞书' : keyLower === 'telegram' ? 'Telegram' : channelKey;
      const card = document.createElement('article');
      card.className = 'channel-wire-card';
      card.dataset.channelKey = channelKey;
      card.innerHTML =
        '<div class="channel-wire-head"><span>' + displayName + '</span><div class="channel-wire-badges"><span class="channel-wire-badge ' + (connected ? 'ok' : 'warn') + '"><i class="fa-solid ' + (connected ? 'fa-circle-check' : 'fa-circle-exclamation') + '"></i>' + (connected ? l.connected : l.disconnected) + '</span><span class="channel-wire-badge ok"><i class="fa-solid fa-plug-circle-check"></i>' + l.installed + '</span></div></div>' +
        '<div class="channel-wire-form">' +
          '<div class="channel-wire-form-row">' +
            '<label class="channel-wire-field">' + l.appId + '<input type="text" data-field="appId" value="' + appId.replace(/"/g, '&quot;') + '" /></label>' +
            '<label class="channel-wire-field">' + l.appSecret + '<input type="text" data-field="appSecret" value="' + appSecret.replace(/"/g, '&quot;') + '" /></label>' +
          '</div>' +
          '<label class="channel-wire-field">' + l.accessUrl + '<input type="text" data-field="accessUrl" value="' + accessUrl.replace(/"/g, '&quot;') + '" /></label>' +
          '<div class="channel-wire-form-row">' +
            '<label class="channel-wire-field">' + l.accessMode + '<select data-field="accessMode"><option value="websocket">WebSocket</option><option value="webhook">Webhook</option><option value="polling">Polling</option></select></label>' +
            '<label class="channel-wire-field">' + l.enabledField + '<select data-field="enabled"><option value="true">' + l.yes + '</option><option value="false">' + l.no + '</option></select></label>' +
          '</div>' +
          '<label class="channel-wire-field">' + l.allowGroup + '<select data-field="allowGroup"><option value="false">' + l.no + '</option><option value="true">' + l.yes + '</option></select></label>' +
          '<div class="channel-wire-actions"><button class="btn wf-cancel-btn" type="button" data-channel-test="' + channelKey + '">' + l.test + '</button><button class="btn wf-confirm-btn" type="button" data-channel-save="' + channelKey + '">' + l.save + '</button></div>' +
        '</div>';
      const accessModeSelect = card.querySelector('select[data-field="accessMode"]');
      const enabledSelect = card.querySelector('select[data-field="enabled"]');
      const allowGroupSelect = card.querySelector('select[data-field="allowGroup"]');
      if (accessModeSelect instanceof HTMLSelectElement) accessModeSelect.value = accessMode;
      if (enabledSelect instanceof HTMLSelectElement) enabledSelect.value = enabled ? 'true' : 'false';
      if (allowGroupSelect instanceof HTMLSelectElement) allowGroupSelect.value = allowGroup ? 'true' : 'false';
      listNode.append(card);
    });
  };

  const collectCardConfig = (card, channelKey) => {
    const state = channelState.get(channelKey);
    const baseConfig = state && state.config && typeof state.config === 'object' ? { ...state.config } : {};
    const appIdInput = card.querySelector('input[data-field="appId"]');
    const appSecretInput = card.querySelector('input[data-field="appSecret"]');
    const accessUrlInput = card.querySelector('input[data-field="accessUrl"]');
    const accessModeSelect = card.querySelector('select[data-field="accessMode"]');
    const enabledSelect = card.querySelector('select[data-field="enabled"]');
    const allowGroupSelect = card.querySelector('select[data-field="allowGroup"]');
    const appId = appIdInput instanceof HTMLInputElement ? appIdInput.value.trim() : '';
    const appSecret = appSecretInput instanceof HTMLInputElement ? appSecretInput.value.trim() : '';
    const accessUrl = accessUrlInput instanceof HTMLInputElement ? accessUrlInput.value.trim() : '';
    const accessMode = accessModeSelect instanceof HTMLSelectElement ? accessModeSelect.value.trim() : 'default';
    const enabled = enabledSelect instanceof HTMLSelectElement ? enabledSelect.value === 'true' : true;
    const allowGroup = allowGroupSelect instanceof HTMLSelectElement ? allowGroupSelect.value === 'true' : false;
    setByCandidate(baseConfig, ['appId', 'app_id', 'clientId', 'client_id', 'botId', 'bot_id', 'chatId', 'chat_id'], appId);
    setByCandidate(baseConfig, ['appSecret', 'app_secret', 'clientSecret', 'client_secret', 'token', 'botToken', 'bot_token'], appSecret);
    setByCandidate(baseConfig, ['domain', 'url', 'accessUrl', 'webhookUrl', 'callbackUrl'], accessUrl);
    setByCandidate(baseConfig, ['connectionMode', 'mode', 'accessMode', 'type'], accessMode);
    setByCandidate(baseConfig, ['enabled', 'active'], enabled);
    const groupPolicyKey = ['groupPolicy', 'allowGroup', 'allowGroupMessage', 'groupEnabled'].find((key) => Object.prototype.hasOwnProperty.call(baseConfig, key)) || 'groupPolicy';
    if (groupPolicyKey === 'groupPolicy') {
      baseConfig[groupPolicyKey] = allowGroup ? 'open' : 'closed';
      if (Object.prototype.hasOwnProperty.call(baseConfig, 'dmPolicy')) {
        baseConfig.dmPolicy = allowGroup ? 'open' : 'closed';
      }
    } else {
      baseConfig[groupPolicyKey] = allowGroup;
    }
    return {
      appId,
      appSecret,
  ...
  const reload = async () => {
    listNode.innerHTML = '<div class="empty-state">' + l.loadingChannels + '</div>';
    try {
      const { response, payload } = await fetchJson('/api/channel-config', undefined, 20000);
      if (!response.ok || !payload?.ok) {
        throw new Error(String(payload?.error?.message || l.loadFailed));
      }
      ...
      listNode.innerHTML = '<div class="empty-state">' + (error instanceof Error ? error.message : l.loadFailed) + '</div>';
      setStatus(l.loadFailed);
    }
  };

  if (openDialogButton instanceof HTMLButtonElement && dialogNode instanceof HTMLDialogElement) {
    openDialogButton.addEventListener('click', () => {
      dialogNode.showModal();
    });
  }
  if (cancelNode instanceof HTMLElement && dialogNode instanceof HTMLDialogElement) {
  ...
  if (formNode instanceof HTMLFormElement) {
    formNode.addEventListener('submit', async (event) => {
      ...
      const channelKey = String(formData.get('channelKey') || '').trim().toLowerCase();
      const configText = String(formData.get('channelConfigJson') || '').trim();
      if (!channelKey || !configText) {
        setStatus(l.requireFields);
        return;
      }
      let configObj = null;
      ...
      setStatus(l.saving);
      try {
        const { response, payload } = await fetchJson('/api/channel-config/channels', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({
        ...
        if (!response.ok || !payload?.ok) {
          throw new Error(String(payload?.error?.message || l.saveFailed));
        }
  ...
  listNode.addEventListener('click', async (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
    const saveBtn = target.closest('[data-channel-save]');
    const testBtn = target.closest('[data-channel-test]');
    if (saveBtn instanceof HTMLElement) {
      const channelKey = String(saveBtn.getAttribute('data-channel-save') || '').trim().toLowerCase();
      if (!channelKey) return;
      const card = saveBtn.closest('.channel-wire-card');
      if (!(card instanceof HTMLElement)) return;
      const payload = collectCardConfig(card, channelKey);
      if (!payload.appId || !payload.appSecret || !payload.accessUrl) {
        setStatus(l.requireFields);
        return;
      }
      if (!(await ensureAdmin())) {
      ...
      setStatus(l.saving);
      try {
        const { response, payload: savePayload } = await fetchJson('/api/channel-config/channels', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
            channelKey,
        ...
        if (!response.ok || !savePayload?.ok) {
          throw new Error(String(savePayload?.error?.message || l.saveFailed));
        }
        setStatus(l.savedOk);
        await reload();
      } catch (error) {
    ...
    if (testBtn instanceof HTMLElement) {
      const channelKey = String(testBtn.getAttribute('data-channel-test') || '').trim().toLowerCase();
      if (!channelKey) return;
      const card = testBtn.closest('.channel-wire-card');
      if (!(card instanceof HTMLElement)) return;
      const payload = collectCardConfig(card, channelKey);
      if (!payload.appId || !payload.appSecret || !payload.accessUrl) {
        setStatus(l.testFail);
        return;
      }
      setStatus(l.testPass);
    }
  });
...
(() => {
  ...
  const hexToRgb = (hex) => {
    const safe = (hex || '#00a3ff').replace('#', '').trim();
  ...
  avatars.forEach((avatar, index) => {
    ...
    if (!canvas) return;
    const accent = getComputedStyle(avatar).getPropertyValue('--agent-accent').trim() || '#00a3ff';
...
function renderUsageWireframeSummaryCards(periods: UsageCostSnapshot["periods"], language: UiLanguage = "zh"): string {
  const orderedKeys: Array<"today" | "7d" | "30d"> = ["today", "7d", "30d"];
  const rows = orderedKeys
    .map((key) => periods.find((item) => item.key === key))
    .filter((item): item is UsageCostSnapshot["periods"][number] => Boolean(item));
  if (rows.length === 0) {
    return `<div class="empty-state">${escapeHtml(pickUiText(language, "No usage data yet.", "暂无用量数据。"))}</div>`;
  }
  return rows
    .map((period) => {
      const tokenLabel =
      ...
      const requestLabel =
        period.requestCountStatus !== "not_connected" && typeof period.requestCount === "number"
          ? String(period.requestCount)
          : pickUiText(language, "Not connected", "数据源未连接");
      return `<article class="usage-wf-kpi">
        <div class="usage-wf-kpi-label">${escapeHtml(usagePeriodLabel(period.key, period.label, language))}</div>
        <div class="usage-wf-kpi-value">${escapeHtml(tokenLabel)}</div>
        <div class="usage-wf-kpi-meta">${escapeHtml(pickUiText(language, "Estimated cost", "预估费用"))}：${escapeHtml(costLabel)}</div>
        <div class="usage-wf-kpi-meta">${escapeHtml(pickUiText(language, "Requests", "请求数"))}：${escapeHtml(requestLabel)}</div>
      </article>`;
    })
    .join("");
}

function renderUsageWireframeLegend(
  rows: UsageCostSnapshot["breakdown"]["bySessionType"],
  language: UiLanguage = "zh",
): string {
  const palette = ["#f6ae2d", "#56cfe1", "#00a3ff", "#ffd166", "#f77f00", "#72efdd"];
  const dataRows = rows.slice(0, 6);
  if (dataRows.length === 0) {
    return `<span class="usage-wf-legend-item"><span class="usage-wf-dot" style="background:#6b7280;"></span>${escapeHtml(
      pickUiText(language, "No connected channel yet", "暂无已连接渠道"),
    )}</span>`;
  }
  return dataRows
    .map((row, index) => {
      const color = palette[index % palette.length];
      return `<span class="usage-wf-legend-item"><span class="usage-wf-dot" style="background:${color};"></span>${escapeHtml(
        simplifyUsageLabel(row.label),
      )}</span>`;
    })
    .join("");
}

function renderUsageWireframeTrendChart(
  rows: UsageCostSnapshot["breakdown"]["bySessionType"],
  language: UiLanguage = "zh",
): string {
  const width = 1160;
  ...
  const palette = ["#f6ae2d", "#56cfe1", "#00a3ff", "#ffd166", "#f77f00", "#72efdd"];
  const usableRows = rows.filter((item) => item.tokens > 0).slice(0, 6);
  const fallbackRows =
    usableRows.length > 0
      ? usableRows
      : [{ key: "default", label: pickUiText(language, "Usage", "用量"), tokens: 1, sessions: 1, requests: 1, estimatedCost: 0, sourceStatus: "not_connected" as const }];
  const xAt = (index: number): number => left + (chartW * index) / Math.max(1, labels.length - 1);
  const yAt = (value: number): number => top + ((yMax - value) / yMax) * chartH;
  const grid = ticks
    .map((tick) => {
      const y = yAt(tick);
      return `<line x1="${left}" y1="${y.toFixed(2)}" x2="${(width - right).toFixed(2)}" y2="${y.toFixed(
        2,
  ...
  const yLabels = ticks
    .map((tick) => {
      const y = yAt(tick);
      return `<text x="${left - 10}" y="${(y + 4).toFixed(2)}" text-anchor="end" fill="#6b7280" font-size="11">${tick}</text>`;
    })
    .join("");
  const xLabels = labels
    .map((tick, index) => {
      const x = xAt(index);
      return `<text x="${x.toFixed(2)}" y="${height - 8}" text-anchor="middle" fill="#6b7280" font-size="10">${tick}</text>`;
    })
    .join("");
  const paths = fallbackRows
    .map((row, index) => {
      const color = palette[index % palette.length];
      const seed = stableHashIndex(`${row.label}-${row.tokens}-${row.requests}-${row.sessions}`);
      const base = 120 + (seed % 110);
      const ampA = 35 + ((seed >> 3) % 45);
      const ampB = 22 + ((seed >> 5) % 28);
      const phase = ((seed % 360) * Math.PI) / 180;
      const drift = ((row.tokens % 31) - 15) * 0.5;
      const coords = labels
        .map((_, tickIndex) => {
          const v = base + ampA * Math.sin(tickIndex / 2.6 + phase) + ampB * Math.cos(tickIndex / 1.9 + phase / 2) + drift;
          const value = Math.max(20, Math.min(yMax - 8, v));
          return `${xAt(tickIndex).toFixed(2)},${yAt(value).toFixed(2)}`;
        })
        .join(" ");
      return `<polyline fill="none" stroke="${color}" stroke-width="2.1" points="${coords}" />`;
    })
    .join("");
  return `<svg class="usage-wf-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${escapeHtml(
    pickUiText(language, "Token trend chart", "Token 趋势图"),
  )}">
    ${grid}
    ${yLabels}
    ${xLabels}
...
<head>
  <meta charset="utf-8" />
  <title>${escapeHtml(t("TinyClaw Session Drilldown", "TinyClaw 会话详情"))}</title>
  <style>
    ...
    th { color: #93aac2; }
    .badge { display:inline-block; border-radius:999px; padding:2px 6px; font-size:11px; border:1px solid transparent; text-transform:uppercase; letter-spacing:0.06em; }
...
<head>
  <meta charset="utf-8" />
  <title>TinyClaw Audit Timeline</title>
  <style>
    body { font-family: "SF Mono", Menlo, monospace; background: #0b1016; color: #d6e7f9; padding: 16px; margin: 0; }
    a { color: #7dd3fc; }
    .card { border: 1px solid #27405a; background: #111923; padding: 12px; border-radius: 8px; margin-top: 10px; }
...
<head>
  <meta charset="utf-8" />
  <title>${escapeHtml(t("TinyClaw Task detail", "TinyClaw 任务详情"))} · ${escapeHtml(task.taskId)}</title>
  <style>
...
<head>
  <meta charset="utf-8" />
  <title>${escapeHtml(pickUiText(language, "TinyClaw Cron detail", "TinyClaw Cron 详情"))} · ${escapeHtml(job.jobId)}</title>
  <style>
    body { font-family: "SF Pro Text", -apple-system, BlinkMacSystemFont, "PingFang SC", sans-serif; margin:0; padding:20px; background:#f5f5f7; color:#1d1d1f; }
    .page { max-width: 860px; margin: 0 auto; display:grid; gap:12px; }
    .card { border:1px solid rgba(17,24,39,0.1); border-radius:16px; background:#fff; padding:14px; box-shadow:0 8px 20px rgba(17,24,39,0.06); }
...
function assertMutationAuthorized(
  ...
  const decision = evaluateRequestTokenGate(req, routeLabel, explicitToken);
  if (!decision.ok) {
    throw new RequestValidationError(decision.message, decision.statusCode);
  }
}

function evaluateRequestTokenGate(
  req: IncomingMessage,
  routeLabel: string,
  explicitToken?: string | null,
): ReturnType<typeof evaluateLocalTokenGate> {
  const token = resolveProvidedAuthToken(req, explicitToken);
  return evaluateLocalTokenGate({
    gateRequired: LOCAL_TOKEN_AUTH_REQUIRED,
    configuredToken: LOCAL_API_TOKEN,
    providedToken: token,
    routeLabel,
...
function resolveProvidedAuthToken(req: IncomingMessage, explicitToken?: string | null): string | undefined {
  return (
    ...
    normalizeToken(readCookieValue(req, ADMIN_AUTH_COOKIE_NAME))
  );
}

function readCookieValue(req: IncomingMessage, name: string): string | undefined {
  const cookieHeader = readHeaderValue(req, "cookie");
...
function setAdminAuthCookie(res: ServerResponse, token: string): void {
  const encoded = encodeURIComponent(token);
  res.setHeader(
    "Set-Cookie",
    `${ADMIN_AUTH_COOKIE_NAME}=${encoded}; Max-Age=${ADMIN_AUTH_COOKIE_MAX_AGE_SECONDS}; Path=/; HttpOnly; SameSite=Lax`,
  );
}

function clearAdminAuthCookie(res: ServerResponse): void {
  res.setHeader(
    "Set-Cookie",
    `${ADMIN_AUTH_COOKIE_NAME}=; Max-Age=0; Path=/; HttpOnly; SameSite=Lax`,
  );
}
...
function readNonNegativeIntQuery(
  value: string | null,
  label: string,
  fallback: number,
  strict: boolean,
  ...
  const parsed = Number.parseInt(trimmed, 10);
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > maxValue) {
    if (strict) throw new RequestValidationError(`${label} must be an integer in range 0..${maxValue}`, 400);
    return fallback;
  }
  return parsed;
}
